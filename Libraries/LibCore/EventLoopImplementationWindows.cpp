...
static void arm_master_timer(ThreadData& thread_data)
{
    ...
    for (auto& it : thread_data.timers) {
        if (!earliest.has_value() || it.value->deadline < *earliest)
            earliest = it.value->deadline;
    }
    ...
    // Measured in 0.1μs intervals; negative means relative to now. Clamp to at least one interval so an
    // already-passed deadline stays a relative time.
    due_time.QuadPart = -AK::max<i64>((*earliest - MonotonicTime::now()).to_nanoseconds() / 100, 1);
    ...
    if (success) {
        for (ULONG i = 0; i < entries_removed; i++) {
            ...
            if (packet->type == CompletionType::Timer) {
                ...
                // Fire every due timer, in registration order for equal deadlines. (This cannot rely on kernel
                // wakeup order: simultaneously signaled wait completion packets are delivered in LIFO order.)
                auto now = MonotonicTime::now();
                Vector<EventLoopTimer*> due_timers;
                for (auto& it : thread_data->timers) {
                    if (it.value->deadline <= now)
                        due_timers.append(it.value.ptr());
                }
                quick_sort(due_timers, [](auto* a, auto* b) {
                ...
                for (auto* timer : due_timers) {
                    ...
                    if (timer->is_periodic)
                        timer->deadline = now + AK::Duration::from_milliseconds(timer->interval_milliseconds);
                    else
                        thread_data->timers.remove(reinterpret_cast<intptr_t>(timer));
                }
...
intptr_t EventLoopManagerWindows::register_timer(EventReceiver& object, int milliseconds, bool should_reload)
{
    ...
    timer->deadline = MonotonicTime::now() + AK::Duration::from_milliseconds(milliseconds);
    timer->interval_milliseconds = milliseconds;
    timer->is_periodic = should_reload;
    timer->sequence = thread_data->next_timer_sequence++;

    auto timer_id = reinterpret_cast<intptr_t>(timer.ptr());
...
void EventLoopManagerWindows::unregister_timer(intptr_t timer_id)
{
    if (auto* thread_data = ThreadData::the()) {
        if (thread_data->timers.remove(timer_id))
            arm_master_timer(*thread_data);
    }
}
