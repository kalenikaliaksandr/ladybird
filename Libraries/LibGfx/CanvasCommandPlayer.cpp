...
static bool is_valid(CompositingAndBlendingOperator op)
{
    return static_cast<int>(op) >= static_cast<int>(CompositingAndBlendingOperator::Normal)
        && static_cast<int>(op) <= static_cast<int>(CompositingAndBlendingOperator::PlusLighter);
}

static bool is_valid(WindingRule rule)
{
    return rule == WindingRule::Nonzero || rule == WindingRule::EvenOdd;
}

static bool is_valid(ScalingMode mode)
{
    switch (mode) {
...
static bool is_valid(Path::CapStyle cap)
{
    switch (cap) {
    case Path::CapStyle::Butt:
    case Path::CapStyle::Round:
    case Path::CapStyle::Square:
        return true;
    }
    return false;
}

static bool is_valid(Path::JoinStyle join)
{
    switch (join) {
    case Path::JoinStyle::Miter:
    case Path::JoinStyle::Round:
    case Path::JoinStyle::Bevel:
        return true;
    }
    return false;
}

CanvasCommandPlayer::CanvasCommandPlayer(RefPtr<SkiaBackendContext> skia_backend_context)
...
RefPtr<PaintingSurface> CanvasCommandPlayer::surface() const
{
    return m_surface;
}

bool CanvasCommandPlayer::play(CanvasCommandList const& command_list)
{
...
void CanvasCommandPlayer::play_command(CanvasCommands::Initialize const& command)
{
    ...
    VERIFY(is_valid_bitmap_format(static_cast<u32>(command.format)));
    VERIFY(command.format != BitmapFormat::Invalid);
    VERIFY(is_valid_alpha_type(static_cast<u32>(command.alpha_type)));

    m_surface = PaintingSurface::create_with_size(command.size, command.format, command.alpha_type, m_skia_backend_context);
