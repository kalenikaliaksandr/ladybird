...
ErrorOr<void> encode(Encoder& encoder, Gfx::DecodedImageFrame const& frame)
{
    ...
    if (!bitmap.is_valid())
        return Error::from_string_literal("DecodedImageFrame IPC failed to create image-frame bitmap");
...
ErrorOr<Gfx::DecodedImageFrame> decode(Decoder& decoder)
{
    ...
    if (!bitmap.is_valid() || !bitmap.bitmap())
        return Error::from_string_literal("DecodedImageFrame IPC contained invalid image-frame bitmap");
