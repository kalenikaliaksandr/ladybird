...
public:
    ...
    // The source frame's pixels must stay immutable after the call: implementations may
    // cache textures by bitmap identity, so mutable sources must be snapshotted into a
    // fresh frame by the caller.
