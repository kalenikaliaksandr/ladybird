...
void PathImplSkia::move_to(Gfx::FloatPoint const& point)
{
    ...
    mutate_path(*m_path, [&](auto& builder) {
...
void PathImplSkia::glyph_run(GlyphRun const& glyph_run)
{
    ...
    for (auto const& glyph : glyph_run.glyphs()) {
        ...
        if (!glyph_path.has_value())
            continue;
        builder.addPath(*glyph_path, glyph.position.x(), glyph.position.y() + font_ascent);
    }
...
static NonnullOwnPtr<PathImpl> place_text_along_impl(SkPath const& path, Font const& font, size_t length_in_code_points, TextToGlyphs&& text_to_glyphs)
{
    ...
    SkPathBuilder output_path_builder(output_path->sk_path());
    ...
    for (size_t i = 0; i < length_in_code_points; ++i) {
        ...
        SkScalar advance;
        sk_font.getWidths(SkSpan { &glyph, 1 }, SkSpan { &advance, 1 });
        ...
        output_path_builder.addPath(*glyph_path, matrix);

        accumulated_distance += advance;
    }

    output_path->sk_path() = output_path_builder.detach();
    return output_path;
}
...
NonnullOwnPtr<PathImpl> PathImplSkia::place_text_along(Utf8View const& text, Font const& font) const
{
    ...
    return place_text_along_impl(*m_path, font, length_in_code_points, [&](auto const& sk_font, auto const& run_buffer) {
        sk_font.textToGlyphs(text.as_string().characters_without_null_termination(), text.as_string().length(), SkTextEncoding::kUTF8, SkSpan { run_buffer.glyphs, length_in_code_points });
    });
}
...
NonnullOwnPtr<PathImpl> PathImplSkia::place_text_along(Utf16View const& text, Font const& font) const
{
    ...
    return place_text_along_impl(*m_path, font, length_in_code_points, [&](auto const& sk_font, auto const& run_buffer) {
        sk_font.textToGlyphs(text.utf16_span().data(), text.length_in_code_units() * sizeof(char16_t), SkTextEncoding::kUTF16, SkSpan { run_buffer.glyphs, length_in_code_points });
    });
}
