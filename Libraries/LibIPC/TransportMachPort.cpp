...
// The default Mach port queue limit is 5 messages. Increase it to avoid
...
ErrorOr<NonnullOwnPtr<TransportMachPort>> TransportMachPort::from_socket(NonnullOwnPtr<Core::LocalSocket> socket)
{
    ...
    // The local send right is not needed — it was only created so the bootstrap
    // server has a right to hand out. It stays alive until function return
    // (destroyed after the TransportMachPort constructor runs initialize()).
    (void)our_send;

    // Synchronize: ensure peer has looked up our port before we proceed.
    // The peer must hold a send right before initialize() registers
    // MACH_NOTIFY_NO_SENDERS, otherwise the notification could fire
    // prematurely when our local send right (our_send) is dropped at
    // function return.
    u8 ack = 1;
...
ErrorOr<TransportMachPort::Paired> TransportMachPort::create_paired()
{
    ...
    // Local: receives on A, sends to B — fully initialized transport
    // Remote: receives on B, sends to A — just the raw port rights as a handle
    return Paired {
...
void TransportMachPort::wake_io_thread()
{
    ...
    // Zero-timeout send — if the wakeup port's queue is full, the IO thread
    // is already awake and will check the send queue on its next iteration.
...
{
    // The IO thread alternates between draining the outbound send queue and
    // blocking on mach_msg() to receive inbound messages or kernel notifications.
    ...
    for (;;) {
        ...
        // Dispatch based on which port in the set received the message.
        if (header->msgh_local_port == m_wakeup_receive_port.port()) {
            // Wakeup message from main thread — loop back to drain send queue.
            continue;
        }
        ...
        if (header->msgh_local_port == m_receive_port.port()) {
            if (header->msgh_id == IPC_DATA_MESSAGE_ID) {
                ...
                // Safety net: fires if a send right we hold becomes a dead name
                // (the peer's receive port was destroyed). Not our primary detection
                // mechanism (NO_SENDERS is), but handled defensively.
...
{
    // Mach message layout (always "complex" — carries port descriptors):
    //
    // ┌─────────────────────────┐
    // │ mach_msg_header_t       │  destination port, message ID, size
    // ├─────────────────────────┤
    // │ mach_msg_body_t         │  descriptor count
    // ├─────────────────────────┤
    // │ port descriptors (0..N) │  Mach port rights (file ports, IPC handles)
    // ├─────────────────────────┤
    // │ OOL descriptor          │  out-of-line payload (kernel copies the data)
    // └─────────────────────────┘
...
{
    auto receive_right = release_receive_right();
    auto send_right = release_send_right();
