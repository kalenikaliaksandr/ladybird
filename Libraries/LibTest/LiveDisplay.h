...
// Returns a tagged handle for the given stdio stream. (The CRT's fileno() namespace differs from the native handle
// namespace on Windows, so this cannot be a plain fd.)
inline Core::SystemHandleRef system_handle_for_stream(FILE* stream)
{
#if defined(AK_OS_WINDOWS)
    return Core::SystemHandleRef::from_handle(reinterpret_cast<void*>(_get_osfhandle(fileno(stream))), Core::HandleKind::File);
#else
    return Core::SystemHandleRef::from_fd(fileno(stream));
#endif
}

inline bool stdout_is_tty()
{
    return Core::System::isatty(system_handle_for_stream(stdout)).value_or(false);
}

inline size_t query_terminal_width(Core::SystemHandleRef handle, size_t fallback = 80)
{
    if (auto size = Core::System::terminal_size(handle); !size.is_error() && size.value().columns > 0)
...
public:
    ...
    bool begin(Options options)
    {
        ...
        (void)Core::System::enable_ansi_escape_sequence_processing(system_handle_for_stream(m_output));
    ...
    {
        m_terminal_width = query_terminal_width(system_handle_for_stream(m_output ? m_output : stdout));
    }
