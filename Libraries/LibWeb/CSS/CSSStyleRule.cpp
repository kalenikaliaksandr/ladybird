    ...
    if (parsed_selectors.has_value()) {
        ...
        if (auto* sheet = parent_style_sheet()) {
            if (auto style_sheet_list = sheet->style_sheet_list()) {
                style_sheet_list->document().style_computer().invalidate_rule_cache(style_sheet_list->document_or_shadow_root(), StyleComputer::InvalidateRuleCacheReason::SetSelectorText);
