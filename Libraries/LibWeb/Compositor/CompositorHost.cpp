...
AsyncScrollEnqueueResult CompositorContextHandle::async_scroll_by(UniqueNodeID expected_document_id, Gfx::FloatPoint position,
    Gfx::FloatPoint delta_in_device_pixels, Gfx::IntRect viewport_rect, AsyncScrollInput input, AsyncScrollOperationTracking operation_tracking)
{
    return m_host.async_scroll_by(m_context_id, expected_document_id, position, delta_in_device_pixels, viewport_rect, input, operation_tracking);
}
...
Optional<PendingAsyncScrollUpdates> CompositorContextHandle::take_over_user_scroll(AsyncScrollNodeStableID id, UserScrollTakeoverReason reason)
{
    return m_host.take_over_user_scroll(m_context_id, id, reason);
}
