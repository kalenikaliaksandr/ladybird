...
struct UpdateDisplayListIPCCommand {
    NonnullRefPtr<Painting::DisplayList> display_list;
    Painting::DisplayListResourceTransactionTransport resource_transaction;
    Painting::ScrollStateSnapshot scroll_state_snapshot;
};
...
using CompositorCommand = Variant<UpdateDisplayListCommand, UpdateDisplayListIPCCommand, AsyncScrollByCommand, ViewportScrollbarDragCommand,
        ...
        while (true) {
            ...
            while (true) {
                ...
                command->command.visit(
                    ...
                    [this, &context](UpdateDisplayListIPCCommand& cmd) {
                        dbgln_if(COMPOSITOR_DEBUG, "[Compositor] Compositor processing display list transport update (deferred_async_present={})",
                            context.has_deferred_async_scroll_present);
                        if (auto result = context.display_list_resource_storage.apply_transport_transaction(move(cmd.resource_transaction)); result.is_error()) {
                            dbgln("Failed to reconstruct display-list resources from compositor IPC: {}", result.error());
                            return;
                        }
                        install_display_list_update(context, move(cmd.display_list), move(cmd.scroll_state_snapshot));
                    },
    ...
    void present_frame(ContextState& context, Gfx::IntRect viewport_rect, PresentFrameDelivery delivery)
    {
        ...
        if (context.cached_display_list && context.backing_store_manager.is_valid()) {
            ...
            presentation_mode.visit(
                [this, &context, viewport_rect, rendered_bitmap_id, delivery](PresentToUI) {
                    if (context.presents_to_client) {
...
void CompositorThread::update_display_list(
    ...
    Painting::ScrollStateSnapshot&& scroll_state_snapshot)
{
    m_thread_data->enqueue_command(context_id, UpdateDisplayListIPCCommand { move(display_list), move(resource_transaction), move(scroll_state_snapshot) });
}
...
{
    m_thread_data->enqueue_command(context_id, ScreenshotCommand { move(target), request_id });
}
