...
Optional<SnapAxisChoice> choose_snap_offset_for_axis(Vector<SnapPositionCandidate> const& candidates, SnapAxisSelection const& selection, CSSPixels snapport_size, CSS::ScrollSnapStrictness strictness, Optional<CSSPixels> cross_axis_offset, Optional<SnapAreaID> only_area)
{
...
Vector<SnapAreaID> snap_areas_at_offset(Vector<SnapPositionCandidate> const& candidates, CSSPixels offset, CSSPixels cross_axis_offset)
{
    Vector<SnapAreaID> areas;
    for (auto const& candidate : candidates) {
...
SnapDestination select_snap_destination(SnapContainerData const& data, CSSPixelPoint destination, SnapSelectionStrategy const& strategy)
{
    auto snap_axes = data.axes;
    ...
    auto const& snapport = data.snapport_size;
    auto const& candidates = data.candidates;
    ...
    auto choose_x = [&](Optional<CSSPixels> cross_axis_offset, Optional<SnapAreaID> only_area = {}) {
        return choose_snap_offset_for_axis(candidates.x_candidates, x_selection, snapport.width(), data.strictness, cross_axis_offset, only_area);
    };
    auto choose_y = [&](Optional<CSSPixels> cross_axis_offset, Optional<SnapAreaID> only_area = {}) {
        return choose_snap_offset_for_axis(candidates.y_candidates, y_selection, snapport.height(), data.strictness, cross_axis_offset, only_area);
    };
