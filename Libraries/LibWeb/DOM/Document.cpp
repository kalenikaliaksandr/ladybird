...
void Document::update_scrollable_overflow()
{
    ...
    if (any_has_scrollable_overflow_flipped) {
        set_needs_accumulated_visual_contexts_reconcile();
    } else if (!accumulated_visual_contexts_refresh_pending()) {
...
void Document::update_paint_and_hit_testing_properties_if_needed()
{
    ...
    if (accumulated_visual_contexts_refresh_pending()) {
        bool full_rebuild = m_needs_accumulated_visual_contexts_update;
