...
static void relayout_svg_root(Layout::SVGSVGBox& svg_root)
{
    ...
    svg_context.run(Layout::LayoutInput::from_available_space(
        Layout::AvailableSpace(Layout::AvailableSize::make_definite(content_width), Layout::AvailableSize::make_definite(content_height))));
    ...
    for (size_t layout_pass = 0; layout_pass < max_container_query_layout_passes; ++layout_pass) {
...

        {
            ...
            if (m_layout_root->first_child() && m_layout_root->first_child()->is_svg_svg_box()) {
                ...
                svg_formatting_context.run(Layout::LayoutInput::from_available_space(available_space));
            } else {
                Layout::BlockFormattingContext root_formatting_context(layout_state, Layout::LayoutMode::Normal, *m_layout_root, nullptr);
                root_formatting_context.run(Layout::LayoutInput::from_available_space(available_space));
            }
        }
