...
Utf16String Document::dump_hit_test_display_list()
{
    update_layout(UpdateLayoutReason::DumpDisplayList);

    auto const* hit_test_display_list = ensure_hit_test_display_list();
    if (!hit_test_display_list)
        return "No hit test display list"_utf16;

    StringBuilder builder;
    hit_test_display_list->dump(builder);
    return Utf16String::from_utf8_without_validation(builder.string_view());
}
