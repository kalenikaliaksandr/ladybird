...
static CSSPixelPoint determine_the_scroll_into_view_position(Element& target, CSSPixelRect target_bounding_border_box, Element::ScrollLogicalPosition block, Element::ScrollLogicalPosition inline_, Node& scrolling_box)
{
    ...
    } else if (auto* layout_node = scrolling_box.layout_node(); layout_node && layout_node->paintable_ptr()) {
        auto const& paintable_box = *layout_node->paintable_ptr();
        current_scroll_position = Painting::scroll_offset(*layout_node);
        scrolling_box_rect = paintable_box.transform_rect_to_viewport(Painting::scroll_snapport_rect(*layout_node), Painting::AccumulatedVisualContextTree::IncludeVisualViewportTransform::No);
    } else {
        return {};
    }
