...
CanvasRenderingContext2D::CanvasRenderingContext2D(JS::Realm& realm, HTMLCanvasElement& element, Bindings::CanvasRenderingContext2DSettings context_attributes)
    ...
    , m_element(element)
    , m_canvas_context_id(Painting::allocate_display_list_resource_id<Painting::CanvasContextId>())
...
{
    // Solid-color fills in the default compositing state take the dedicated FillRect
    // op; everything else goes through the general path pipeline. fillRect is the
    // hottest canvas call, so skipping the path construction is worth the check.
    auto& state = drawing_state();
    auto solid_color = state.fill_style.as_color();
    if (solid_color.has_value() && solid_color->alpha() > 0
        && width >= 0 && height >= 0
        && state.global_alpha == 1.0f
        && !state.filter.has_value()
        && state.current_compositing_and_blending_operator == Gfx::CompositingAndBlendingOperator::SourceOver
        && state.shadow_blur == 0.0f && state.shadow_offset_x == 0.0f && state.shadow_offset_y == 0.0f) {
        if (auto* recorder = this->recorder()) {
            auto rect = Gfx::FloatRect { x, y, width, height };
            recorder->append(Painting::CanvasCommands::FillRect { .rect = rect, .color = *solid_color });
            did_draw(rect);
        }
        return;
    }
...
void CanvasRenderingContext2D::clear_rect(float x, float y, float width, float height)
{
    ...
    if (auto* recorder = this->recorder()) {
        auto rect = Gfx::FloatRect(x, y, width, height);
        recorder->append(Painting::CanvasCommands::ClearRect { .rect = rect, .color = clear_color() });
        did_draw(rect);
    }
}
...
WebIDL::ExceptionOr<void> CanvasRenderingContext2D::draw_image_internal(CanvasImageSource const& image, float source_x, float source_y, float source_width, float source_height, float destination_x, float destination_y, float destination_width, float destination_height)
{
    ...
    // A canvas source hosted in the same compositor context is sampled in the Compositor via a
    // DrawCanvasContext op instead of being read back into WebContent. Preparing our backing
    // storage first rebinds us to the current compositor context and initializes it.
    (void)recorder();
    GC::Ptr<CanvasRenderingContext2D> remote_canvas_source;
    Gfx::IntRect source_bitmap_rect;
    if (auto* destination_context = m_surface_initialized ? compositor_context() : nullptr) {
        if (auto const* canvas_source = image.get_pointer<GC::Ref<HTMLCanvasElement>>()) {
            if (auto source_context = (*canvas_source)->canvas_rendering_context_2d()) {
                source_context->rebind_host_if_needed();
                if (source_context->m_surface_initialized && source_context->compositor_context() == destination_context) {
                    remote_canvas_source = source_context;
                    source_bitmap_rect = { {}, source_context->m_size };
                }
            }
        }
    }
    ...
    if (!remote_canvas_source) {
        frame = canvas_image_source_frame(image);
        if (!frame.has_value())
            return {};
        source_bitmap_rect = frame->rect();
    }
    ...
    if (auto* recorder = this->recorder()) {
        if (remote_canvas_source) {
            // Temporal correctness: flush the source's pending commands, record the sampling
            // op, then flush it immediately. Global FIFO order then matches call order, so
            // later mutations of the source cannot be sampled. (Self-draw is well-defined:
            // the player snapshots the source surface before drawing.)
            remote_canvas_source->flush_recorded_commands();
            recorder->append(Painting::CanvasCommands::DrawCanvasContext {
                .source_context_id = remote_canvas_source->canvas_context_id(),
            ...
            recorder->append(Painting::CanvasCommands::DrawBitmap {
                .frame = *frame,
                .dst_rect = destination_rect,
                .src_rect = source_rect.to_rounded<int>(),
                .scaling_mode = scaling_mode,
...
Painting::CanvasCommandList* CanvasRenderingContext2D::recorder()
{
    if (is_context_lost())
        return nullptr;
    rebind_host_if_needed();
    // 2D canvas is always rasterized in the Compositor; without a host there is nowhere to
    // record to (only happens for embedders with no compositor, which never render a canvas).
...
Compositor::CompositorContextHandle* CanvasRenderingContext2D::compositor_context()
{
    auto navigable = m_element->document().navigable();
...
void CanvasRenderingContext2D::rebind_host_if_needed()
{
    if (!m_surface_initialized || !m_compositor_host_id.has_value())
        return;

    // The Initialize lives on the context named by m_compositor_host_id. If the canvas was
    // adopted into a different navigable, its navigable's context went away (so we now resolve
    // to the page context), or the page context was reset on compositor reconnect, that context
    // changed and further deltas would replay against a player that never saw the Initialize.
    auto* current = compositor_context();
    if (current == nullptr || current->id() != *m_compositor_host_id)
...
void CanvasRenderingContext2D::flush_recorded_commands()
{
    ...
    auto* compositor_context = this->compositor_context();
    if (!compositor_context)
        return;

    if (!m_compositor_host_id.has_value())
        VERIFY(compositor_context->create_canvas_context(m_canvas_context_id, { .type = Compositor::CanvasContextType::Context2D, .canvas_id = m_element->ensure_canvas_id() }));
    m_compositor_host_id = compositor_context->id();

    auto commands = move(m_commands);
    m_commands = {};
    compositor_context->update_canvas_commands(m_canvas_context_id, move(commands));
}
...
RefPtr<Gfx::Bitmap> CanvasRenderingContext2D::read_pixels(Gfx::IntRect const& rect)
{
    ...
    if (!compositor_context)
        return nullptr;
    return compositor_context->get_canvas_pixels(m_canvas_context_id, rect);
}
...
{
    // Nothing was lost if the context never had backing storage.
    if (!m_surface_initialized)
        return;
...
{
    if (m_surface_initialized || m_size.is_empty())
        return;
    ...
    m_commands.append(Painting::CanvasCommands::Initialize { .size = surface_size, .format = color_type, .alpha_type = Gfx::AlphaType::Premultiplied });
    ...
    if (!m_context_attributes.alpha)
        m_commands.append(Painting::CanvasCommands::ClearRect { .rect = Gfx::FloatRect { {}, surface_size.to_type<float>() }, .color = clear_color() });
}

void CanvasRenderingContext2D::discard_backing_storage()
{
    m_commands = {};
    if (auto* compositor_context = this->compositor_context())
        compositor_context->destroy_canvas_context(m_canvas_context_id);
    m_surface_initialized = false;
    m_compositor_host_id = {};
}
...
void CanvasRenderingContext2D::stroke_internal(Gfx::Path path)
{
    ...
    recorder->append(Painting::CanvasCommands::StrokePath {
        .path = move(path),
        .style = Painting::to_canvas_paint_style(*paint_style),
...
void CanvasRenderingContext2D::fill_internal(Gfx::Path path, Gfx::WindingRule winding_rule)
{
    ...
    auto bounding_box = path.bounding_box();
    recorder->append(Painting::CanvasCommands::FillPath {
        .path = move(path),
        .style = Painting::to_canvas_paint_style(*paint_style),
...
WebIDL::ExceptionOr<void> CanvasRenderingContext2D::put_image_data(ImageData& image_data, float dx, float dy)
{
    ...
    if (auto* recorder = this->recorder())
        TRY(put_pixels_from_an_image_data_onto_a_bitmap(image_data, *recorder, dx, dy, 0, 0, image_data.width(), image_data.height()));
...
WebIDL::ExceptionOr<void> CanvasRenderingContext2D::put_image_data(ImageData& image_data, float x, float y, float dirty_x, float dirty_y, float dirty_width, float dirty_height)
{
    ...
    // dirtyHeight.
    if (auto* recorder = this->recorder())
        TRY(put_pixels_from_an_image_data_onto_a_bitmap(image_data, *recorder, x, y, dirty_x, dirty_y, dirty_width, dirty_height));

    return {};
}

// https://html.spec.whatwg.org/multipage/canvas.html#dom-context2d-putimagedata-common
WebIDL::ExceptionOr<void> CanvasRenderingContext2D::put_pixels_from_an_image_data_onto_a_bitmap(ImageData& image_data, Painting::CanvasCommandList& recorder, float dx, float dy, float dirty_x, float dirty_y, float dirty_width, float dirty_height)
{
    ...
    recorder.append(Painting::CanvasCommands::Save {});
    recorder.append(Painting::CanvasCommands::SetTransform { .transform = {} });
    recorder.append(Painting::CanvasCommands::DrawBitmap {
        .frame = Gfx::DecodedImageFrame { *bitmap_snapshot, Gfx::AlphaType::Unpremultiplied },
        .dst_rect = dst_rect,
        .src_rect = bitmap_snapshot->rect(),
    });
    recorder.append(Painting::CanvasCommands::Restore {});
...
{
    auto* recorder = m_surface_initialized ? this->recorder() : nullptr;

    // 1. Clear canvas's bitmap to transparent black.
    if (recorder)
        recorder->append(Painting::CanvasCommands::ClearRect { .rect = Gfx::FloatRect { {}, m_size.to_type<float>() }, .color = clear_color() });
    ...
    if (recorder) {
        recorder->append(Painting::CanvasCommands::Reset {});
...
void CanvasRenderingContext2D::clip_internal(Gfx::Path& path, Gfx::WindingRule winding_rule)
{
    ...
    recorder->append(Painting::CanvasCommands::ClipPath { .path = path.clone(), .winding_rule = winding_rule });
}
...
void CanvasRenderingContext2D::paint_shadow_for_fill_internal(Gfx::Path const& path, Gfx::WindingRule winding_rule)
{
    ...
    recorder->append(Painting::CanvasCommands::SetTransform { .transform = transform });
    recorder->append(Painting::CanvasCommands::FillPath {
        .path = path.clone(),
