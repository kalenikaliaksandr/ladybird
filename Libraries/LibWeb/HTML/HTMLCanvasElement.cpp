...
RefPtr<Gfx::Bitmap> HTMLCanvasElement::get_bitmap_from_surface()
{
    ...
    if (auto* webgl_context = m_context.visit(
            [](GC::Ref<WebGL::WebGLRenderingContext>& context) -> WebGL::WebGLRenderingContextBase* { return context.ptr(); },
            [](GC::Ref<WebGL::WebGL2RenderingContext>& context) -> WebGL::WebGLRenderingContextBase* { return context.ptr(); },
            [](auto&) -> WebGL::WebGLRenderingContextBase* { return nullptr; })) {
        return webgl_context->context().read_back_drawing_buffer();
    }
...
bool HTMLCanvasElement::paints_remote_context() const
{
    return m_context.has<GC::Ref<WebGL::WebGLRenderingContext>>() || m_context.has<GC::Ref<WebGL::WebGL2RenderingContext>>();
}

void HTMLCanvasElement::notify_compositor_connection_lost()
{
    m_context.visit(
        [](GC::Ref<WebGL::WebGLRenderingContext>& context) { context->lose_context_from_compositor_loss(); },
        [](GC::Ref<WebGL::WebGL2RenderingContext>& context) { context->lose_context_from_compositor_loss(); },
        [](auto&) { });
}
