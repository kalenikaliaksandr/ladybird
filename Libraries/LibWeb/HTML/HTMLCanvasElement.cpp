...
{
    // Only the 2D context creates its remote context (and with it the Compositor-side
    // surface) on demand; a WebGL context gets its drawing buffer at getContext() time.
    if (auto context = canvas_rendering_context_2d())
