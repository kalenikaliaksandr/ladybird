...
Painting::ExternalContentSource& HTMLMediaElement::video_content_source()
{
    if (!m_video_content_source)
        m_video_content_source = Painting::ExternalContentSource::create();
    return *m_video_content_source;
}
...
void HTMLMediaElement::update_video_frame_and_timeline()
{
    ...
    if (m_selected_video_track_sink) {
        ...
        if (sink_update_result == Media::DisplayingVideoSinkUpdateResult::NewFrameAvailable) {
            video_content_source().update(m_selected_video_track_sink->current_frame());
