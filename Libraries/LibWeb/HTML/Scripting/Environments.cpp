...
RunScriptDecision can_run_script(JS::Realm const& realm)
{
    ...
    if (auto const* window = as_if<HTML::Window>(realm.global_object())) {
        ...
        // AD-HOC: We allow tasks for destroyed documents to run so that microtasks
        //         queued during the fetch of a new document in a navigation can still
        //         be processed, even after the previous document, the one that initiated
        //         the fetch, has been destroyed.
