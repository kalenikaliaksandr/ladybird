...
// element's own subtree. Both members are plain values, resolved by the ancestor formatting
// context before the algorithm runs (the static position rect is already translated into the
// actual containing block's coordinate space); a partial relayout can replay the algorithm
// from a saved copy of these inputs without re-running the ancestor's layout.
