...
void BlockFormattingContext::layout_block_level_box(Box const& box, BlockContainer const& block_container, CSSPixels& bottom_of_lowest_margin_box, LayoutInput const& layout_input)
{
    ...
    // In the document state, the layout entry point creates the document element's used values,
    // which makes its width definite before any formatting context runs, and that creation also
    // derives used values for its containing block chain, which may include an anonymous box
    // between the viewport and the document element (e.g. the table wrapper around a document
    // element with table display). Both are laid out here regardless. Throwaway states for
    // measurement passes have no such pre-created boxes.
