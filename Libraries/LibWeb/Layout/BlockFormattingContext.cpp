...
// Text markers fit snug against the list item; non-text markers position themselves at 50% of the font size.
static CSSPixels marker_distance_for(ListItemMarkerBox const& marker)
{
...
void BlockFormattingContext::layout_block_level_box(Box const& box, BlockContainer const& block_container, CSSPixels& bottom_of_lowest_margin_box, LayoutInput const& layout_input)
{
    ...
    // An inside-positioned marker shifts the list item's content box towards the inline end.
    // Marker dimensions are position-independent, so the shift is known before placement and
    // can be folded into the initial horizontal position.
    CSSPixels list_item_marker_inside_shift = 0;
    if (is_list_item_box_without_css_content && li_box->marker()) {
        ...
        if (marker.list_style_position() == CSS::ListStylePosition::Inside && box.computed_values().direction() == CSS::Direction::Ltr)
            list_item_marker_inside_shift = m_state.get(marker).content_width() + marker_distance_for(marker);
    }
    ...
    place_block_level_element_in_normal_flow_horizontally(box, available_space, list_item_marker_inside_shift);
...
void BlockFormattingContext::place_block_level_element_in_normal_flow_horizontally(Box const& child_box, AvailableSpace const& available_space, CSSPixels additional_inline_shift)
{
    auto& box_state = m_state.get_mutable(child_box);
    ...
    box_state.set_content_x(x + additional_inline_shift);
}
...
void BlockFormattingContext::layout_list_item_marker(ListItemBox const& list_item_box, SpaceUsedByFloats const& inline_space_used_before_list_item_elements_formatted)
{
    ...
    auto marker_distance = marker_distance_for(marker);
    ...
    if (marker.list_style_position() == CSS::ListStylePosition::Inside) {
        ...
        // NB: In LTR, the list item's x offset was already shifted by the marker's inline size
