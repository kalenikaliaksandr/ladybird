...
static bool should_layout_builtin_list_item_marker(ListItemBox const& list_item_box)
{
    if (!list_item_box.marker())
        return false;

    if (auto const* element = as_if<DOM::Element>(static_cast<Box const&>(list_item_box).dom_node())) {
        if (auto const computed_properties = element->computed_properties(CSS::PseudoElement::Marker))
...
void BlockFormattingContext::run(LayoutInput const& layout_input)
{
    ...
    if (auto const* list_item_box = as_if<ListItemBox>(root()); list_item_box && should_layout_builtin_list_item_marker(*list_item_box)) {
        auto const& marker = *list_item_box->marker();
        if (!m_state.get(marker).is_placed()) {
...
void BlockFormattingContext::layout_block_level_box(Box const& box, BlockContainer const& block_container, CSSPixels& bottom_of_lowest_margin_box, LayoutInput const& layout_input)
{
    ...
    auto should_layout_list_item_marker = list_item_box && should_layout_builtin_list_item_marker(*list_item_box);

    if (should_layout_list_item_marker) {
        ensure_sizes_correct_for_left_offset_calculation(*list_item_box);
    ...
    if (should_layout_list_item_marker && !m_state.get(*list_item_box->marker()).is_placed())
