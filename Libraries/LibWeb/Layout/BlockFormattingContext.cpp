...
{
    auto const original_y_offset = m_y_offset_of_current_block_container;

    CSSPixels dummy_bottom = 0;
    CSSPixels block_bottom = line_builder.current_block_offset();
    {
        TemporaryChange<Optional<CSSPixels>> change { m_y_offset_of_current_block_container, line_builder.current_block_offset() };
        layout_block_level_box(box, containing_block, dummy_bottom, layout_input);
        block_bottom = m_y_offset_of_current_block_container.value_or(line_builder.current_block_offset());
    }

    VERIFY(m_y_offset_of_current_block_container == original_y_offset);
    line_builder.append_block_level_box(box, block_bottom);
}
