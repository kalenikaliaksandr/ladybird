...
CSSPixelSize FormattingContext::solve_replaced_size_constraint(CSSPixels input_width, CSSPixels input_height, Box const& box, AvailableSpace const& available_space, Optional<CSSPixels> percentage_resolution_block_size) const
{
    ...
    auto min_height = box.computed_values().min_height().is_auto() ? 0 : calculate_inner_height(box, available_space, box.computed_values().min_height(), percentage_resolution_block_size);
    auto specified_max_height = should_treat_max_height_as_none(box, available_space.height) ? input_height : calculate_inner_height(box, available_space, box.computed_values().max_height(), percentage_resolution_block_size);
    ...
    TableWrapperWidthMode table_wrapper_width_mode)
{
    ...
    auto wrapper_percentage_basis = percentage_basis_for_child_context(containing_block_state);
    auto const& table_wrapper_state = throwaway_state.create(box, wrapper_percentage_basis.width, wrapper_percentage_basis.height);
    auto table_percentage_basis = percentage_basis_for_child_context(table_wrapper_state);
    auto& table_box_state = throwaway_state.create(*table_box, table_percentage_basis.width, table_percentage_basis.height);
    ...
    context->run_until_width_calculation(
        LayoutInput { table_box_state.available_inner_space_or_constraints_from(available_space), table_percentage_basis.width, table_percentage_basis.height },
...
CSSPixels FormattingContext::compute_table_box_height_inside_table_wrapper(Box const& box, AvailableSpace const& available_space)
{
    ...
    LayoutState throwaway_state(box);
    auto const& containing_block_state = throwaway_state.populate_node_from(m_state, *box.containing_block());
    auto wrapper_percentage_basis = percentage_basis_for_child_context(containing_block_state);
    throwaway_state.create(box, wrapper_percentage_basis.width, wrapper_percentage_basis.height);

    auto context = create_independent_formatting_context_if_needed(throwaway_state, LayoutMode::IntrinsicSizing, box);
    VERIFY(context);
    context->run(LayoutInput { m_state.get(box).available_inner_space_or_constraints_from(available_space), wrapper_percentage_basis.width, wrapper_percentage_basis.height });
...
FormattingContext::PercentageBasis FormattingContext::percentage_basis_for_child_context(
...
{
    auto const* containing_block_box = as_if<Box>(containing_block_used_values.node());
    ...
    // definite size of its own passes the basis it was given through. Anonymous table cells are
    // the exception: they are proper containing blocks with their own size semantics.
    ...
    return {
        containing_block_used_values.has_definite_width()
            ? Optional<CSSPixels> { containing_block_used_values.content_width() }
            : should_forward_indefinite_basis ? inherited_percentage_basis_width
                                              : Optional<CSSPixels> {},
        containing_block_used_values.has_definite_height()
            ? Optional<CSSPixels> { containing_block_used_values.content_height() }
            : should_forward_indefinite_basis ? inherited_percentage_basis_height
                                              : Optional<CSSPixels> {},
    };
}
...
FormattingContext::PercentageBasis FormattingContext::percentage_basis_for_child_context(LayoutState::UsedValues const& containing_block_used_values)
{
    return percentage_basis_for_child_context(
        containing_block_used_values,
        containing_block_used_values.percentage_basis_width(),
        containing_block_used_values.percentage_basis_height());
}

// https://quirks.spec.whatwg.org/#the-percentage-height-calculation-quirk
Optional<CSSPixels> FormattingContext::percentage_resolution_block_size_for_child_context(
    LayoutState::UsedValues const& containing_block_used_values,
...
{
    auto percentage_basis = percentage_basis_for_child_context(
        containing_block_used_values,
        containing_block_layout_input.percentage_basis_width,
        containing_block_layout_input.percentage_basis_height);
    return LayoutInput {
        move(available_space),
        percentage_basis.width,
        percentage_basis.height,
        percentage_resolution_block_size_for_child_context(containing_block_used_values, containing_block_layout_input.percentage_resolution_block_size),
    };
}
...
CSSPixels FormattingContext::tentative_width_for_replaced_element(Box const& box, CSS::Size const& computed_width, AvailableSpace const& available_space, Optional<CSSPixels> percentage_resolution_block_size) const
{
    ...
    if ((computed_height.is_auto() && computed_width.is_auto() && !intrinsic.has_width() && intrinsic.has_height() && box.has_preferred_aspect_ratio())
        ...
        return compute_height_for_replaced_element(box, available_space, percentage_resolution_block_size) * box.preferred_aspect_ratio().value();
    }
...
CSSPixels FormattingContext::compute_width_for_replaced_element(Box const& box, AvailableSpace const& available_space, Optional<CSSPixels> percentage_resolution_block_size) const
{
    ...
    if (computed_width.is_auto() && computed_height.is_auto() && box.has_preferred_aspect_ratio()) {
        ...
        CSSPixels h = tentative_height_for_replaced_element(box, computed_height, available_space, percentage_resolution_block_size);
        used_width = solve_replaced_size_constraint(w, h, box, available_space, percentage_resolution_block_size).width();
    }
...
CSSPixels FormattingContext::tentative_height_for_replaced_element(Box const& box, CSS::Size const& computed_height, AvailableSpace const& available_space, Optional<CSSPixels> percentage_resolution_block_size) const
{
    auto intrinsic = box.auto_content_box_size();
    ...
    return calculate_inner_height(box, available_space, computed_height, percentage_resolution_block_size);
}

CSSPixels FormattingContext::compute_height_for_replaced_element(Box const& box, AvailableSpace const& available_space, Optional<CSSPixels> percentage_resolution_block_size) const
{
    ...
    CSSPixels used_height = tentative_height_for_replaced_element(box, computed_height, available_space, percentage_resolution_block_size);
    ...
    if ((computed_width.is_auto() && computed_height.is_auto() && box.has_preferred_aspect_ratio())) {
        ...
        if (auto intrinsic = box.auto_content_box_size(); intrinsic.has_width() || !intrinsic.has_height()) {
            CSSPixels w = tentative_width_for_replaced_element(box, computed_width, available_space, percentage_resolution_block_size);
            CSSPixels h = used_height;
            used_height = solve_replaced_size_constraint(w, h, box, available_space, percentage_resolution_block_size).height();
        }
    }
    ...
    if (!should_treat_max_height_as_none(box, available_space.height)) {
        ...
        auto max_height = calculate_inner_height(box, available_space, computed_max_height, percentage_resolution_block_size);
        if (used_height > max_height)
            used_height = tentative_height_for_replaced_element(box, computed_max_height, available_space, percentage_resolution_block_size);
    }
    ...
    if (!computed_min_height.is_auto()) {
        auto min_height = calculate_inner_height(box, available_space, computed_min_height, percentage_resolution_block_size);
        if (used_height < min_height)
            used_height = tentative_height_for_replaced_element(box, computed_min_height, available_space, percentage_resolution_block_size);
    }
...
void FormattingContext::compute_width_for_absolutely_positioned_replaced_element(Box const& box, AvailableSpace const& available_space)
{
    ...
    // The available space of an absolutely positioned box is its definite containing block
    // rect, so quirks-mode percentage height resolution cannot be reached from here.
...
void FormattingContext::layout_absolutely_positioned_element(Box& box)
{
    ...
    auto independent_formatting_context = layout_inside(box, LayoutMode::Normal, LayoutInput { box_state.available_inner_space_or_constraints_from(available_space), containing_block_width, containing_block_height });
...
CSSPixels FormattingContext::calculate_min_content_width(Layout::Box const& box) const
{
    ...
    auto containing_block_percentage_basis = percentage_basis_for_child_context(containing_block_state);
    auto& box_state = throwaway_state.create(box, containing_block_percentage_basis.width, containing_block_percentage_basis.height);
    ...
    context->run(LayoutInput { available_space, containing_block_percentage_basis.width, containing_block_percentage_basis.height });
