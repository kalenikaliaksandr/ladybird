...
public:
    ...
    // context: the saved inputs pin everything the algorithm consumes from outside the box's
    // subtree, so they stand in for the ancestor's run. Partial relayout uses this to
    // re-resolve a boundary's own size and position.
