    ...
    for (;;) {
        ...
        case InlineLevelIterator::Item::Type::AbsolutelyPositionedElement:
            if (auto const* box = as_if<Box>(*item.node)) {
                // Enclosing inline boxes' start edges make this line a non-zero-height line box even
                // before any fragment lands on it (CSS 2 § 9.4.2); those edges arrive either still
                // unattached in the iterator or restashed onto this item from skipped whitespace.
