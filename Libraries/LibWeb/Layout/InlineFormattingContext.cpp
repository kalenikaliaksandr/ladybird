...
CSSPixels InlineFormattingContext::leftmost_inline_offset_at(CSSPixels y) const
{
    ...
    auto intrusions = parent().float_state().intrusion_by_floats_into_box(m_containing_block_used_values, y, parent(), parent().root());
...
CSSPixels InlineFormattingContext::total_available_width() const
{
    return m_available_space.has_value() ? m_available_space->width.to_px_or_zero() : 0;
}
...
bool InlineFormattingContext::any_floats_present_at_block_offset(CSSPixels block_offset) const
{
    ...
    return parent().float_state().has_float_at_root_y(y_in_root);
}

bool InlineFormattingContext::can_fit_new_line_at_block_offset(CSSPixels block_offset) const
{
    // FIXME: Respect inline direction.
    auto const& float_state = parent().float_state();
    auto line_height = containing_block().computed_values().line_height();
    auto available_width = m_available_space->width.to_px_or_zero();
    return float_state.can_fit_line_at_box_y(m_containing_block_used_values, block_offset, line_height, available_width, parent(), parent().root());
}
