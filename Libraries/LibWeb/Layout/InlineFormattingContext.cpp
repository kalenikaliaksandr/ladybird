...
void InlineFormattingContext::compute_inline_box_pieces()
{
    ...
    bool block_axis_is_reversed = containing_block_computed_values.block_axis_is_reverse();
    ...
    auto inline_start_edge = inline_axis_is_horizontal
        ? (inline_axis_is_reversed ? Edge::Right : Edge::Left)
        : (inline_axis_is_reversed ? Edge::Bottom : Edge::Top);
    auto inline_end_edge = inline_axis_is_horizontal
        ? (inline_axis_is_reversed ? Edge::Left : Edge::Right)
        : (inline_axis_is_reversed ? Edge::Top : Edge::Bottom);

    auto edge_bits = [&](bool is_first, bool is_last) -> u8 {
        u8 edges = block_axis_edges;
        if (is_first)
            edges |= to_underlying(inline_start_edge);
        if (is_last)
            edges |= to_underlying(inline_end_edge);
        return edges;
    };
    ...
    for (auto node_index : emission_order) {
        ...
        CSSPixels border_padding_inline_start = 0;
        CSSPixels border_padding_inline_end = 0;
        CSSPixels border_padding_block_start = 0;
        CSSPixels border_padding_block_end = 0;
        CSSPixels margin_inline_start = 0;
        CSSPixels margin_inline_end = 0;
        if (used_values) {
            if (inline_axis_is_horizontal) {
                border_padding_inline_start = inline_axis_is_reversed ? used_values->border_box_right() : used_values->border_box_left();
                border_padding_inline_end = inline_axis_is_reversed ? used_values->border_box_left() : used_values->border_box_right();
                border_padding_block_start = used_values->border_box_top();
                border_padding_block_end = used_values->border_box_bottom();
                margin_inline_start = inline_axis_is_reversed ? used_values->margin_right : used_values->margin_left;
                margin_inline_end = inline_axis_is_reversed ? used_values->margin_left : used_values->margin_right;
            } else {
                border_padding_inline_start = inline_axis_is_reversed ? used_values->border_box_bottom() : used_values->border_box_top();
                border_padding_inline_end = inline_axis_is_reversed ? used_values->border_box_top() : used_values->border_box_bottom();
                border_padding_block_start = block_axis_is_reversed ? used_values->border_box_right() : used_values->border_box_left();
                border_padding_block_end = block_axis_is_reversed ? used_values->border_box_left() : used_values->border_box_right();
                margin_inline_start = inline_axis_is_reversed ? used_values->margin_bottom : used_values->margin_top;
                margin_inline_end = inline_axis_is_reversed ? used_values->margin_top : used_values->margin_bottom;
            }
        }
        ...
        for (auto& line : per_node.lines) {
            ...
            CSSPixels border_box_inline_start = line.inline_start - (is_first ? border_padding_inline_start : 0);
            CSSPixels border_box_inline_end = line.inline_end + (is_last ? border_padding_inline_end : 0);
            CSSPixels border_box_block_start = content_block_start - border_padding_block_start;
            CSSPixels border_box_block_length = content_block_length + border_padding_block_start + border_padding_block_end;
            ...
            staged_pieces.append({ .piece = {
                                       ...
                                       .edges = edge_bits(is_first, is_last),
                                   },
                .discovery_index = node_index });
            ...
            if (per_node.parent_index.has_value()) {
                ...
                auto contribution_start = border_box_inline_start - (is_first ? margin_inline_start : 0);
                auto contribution_end = border_box_inline_end + (is_last ? margin_inline_end : 0);
    ...
    for (size_t i = 0; i < content_less_nodes.size(); ++i) {
        ...
        staged_pieces.append({ .piece = {
                                   ...
                                   .border_box_rect = { {}, { 0, node.computed_values().line_height() } },
                                   .edges = edge_bits(true, true),
                                   .is_placeholder = true,
                               },
