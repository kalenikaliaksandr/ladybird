...
void InlineFormattingContext::run(LayoutInput const& layout_input)
{
    ...
    bool has_block_level_phantom_line = false;

    for (auto& line_box : m_containing_block_used_values.line_boxes) {
        has_block_level_phantom_line |= line_box.has_block_level_box();
        content_height += line_box.height();
    }
    if (has_block_level_phantom_line)
        content_height = m_containing_block_used_values.line_boxes.is_empty() ? CSSPixels(0) : m_containing_block_used_values.line_boxes.last().bottom();
