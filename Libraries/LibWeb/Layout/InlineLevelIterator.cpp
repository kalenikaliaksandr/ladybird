...
InlineLevelIterator::InlineLevelIterator(Layout::InlineFormattingContext& inline_formatting_context, Layout::BlockContainer const& containing_block, LayoutState::UsedValues const& containing_block_used_values, LayoutMode layout_mode)
...
void InlineLevelIterator::enter_node_with_box_model_metrics(Layout::NodeWithStyleAndBoxModelMetrics const& node)
{
    ...
    auto& used_values = m_inline_formatting_context.mutable_used_values_for(node);
...
Optional<InlineLevelIterator::Item> InlineLevelIterator::generate_next_item()
{
    ...
    auto const& box_state = m_inline_formatting_context.used_values_for(box);
