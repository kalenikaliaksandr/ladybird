...
void InlineLevelIterator::enter_node_with_box_model_metrics(Layout::NodeWithStyleAndBoxModelMetrics const& node)
{
    ...
    m_inline_formatting_context.compute_inset(node, m_containing_block_used_values.content_size());
...
Optional<InlineLevelIterator::Item> InlineLevelIterator::generate_next_item()
{
    ...
    if (is<Layout::ListItemBox>(*m_current_node)
        && m_current_node->display().is_inline_outside()
        && m_current_node->display().is_flow_inside()) {
        skip_to_next();
        return generate_next_item();
    }
    ...
    if (is<Layout::ListItemMarkerBox>(*m_current_node)) {
        ...
        if (!list_item || !list_item->display().is_inline_outside() || !list_item->display().is_flow_inside()) {
            skip_to_next();
            return generate_next_item();
        }
    }
