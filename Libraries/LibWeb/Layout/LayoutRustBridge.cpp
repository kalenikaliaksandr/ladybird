...
#undef LIBWEB_LAZY_STYLE_SCHEMA_ENTRY
        ...
        using StyleValueData = CSS::StyleValueFFI::StyleValueData;
        using LengthPayload = decltype(((StyleValueData*)nullptr)->length);
        using PercentagePayload = decltype(((StyleValueData*)nullptr)->percentage);
        static constexpr RustFFI::FfiStyleValueSchema style_value_schema {
            .style_value_size = sizeof(StyleValueData),
            .tag_offset = offsetof(StyleValueData, tag),
            .length_value_offset = offsetof(StyleValueData, length) + offsetof(LengthPayload, value),
            .length_unit_offset = offsetof(StyleValueData, length) + offsetof(LengthPayload, unit),
            .percentage_value_offset = offsetof(StyleValueData, percentage) + offsetof(PercentagePayload, value),
            .length_tag = to_underlying(StyleValueData::Tag::Length),
            .percentage_tag = to_underlying(StyleValueData::Tag::Percentage),
            .calculated_tag = to_underlying(StyleValueData::Tag::Calculated),
        };
        static constexpr size_t length_unit_count = to_underlying(CSS::LengthUnit::Vw) + 1;
        static Array<double, length_unit_count> const absolute_length_to_px_ratios = [] {
            Array<double, length_unit_count> ratios;
            for (size_t index = 0; index < ratios.size(); ++index) {
                auto unit = static_cast<CSS::LengthUnit>(index);
                ratios[index] = CSS::is_absolute(unit)
                    ? CSS::ratio_between_units(unit, CSS::LengthUnit::Px)
                    : AK::NaN<double>;
            }
            return ratios;
        }();
        RustFFI::rust_layout_register_style_value_schema(
            style_value_schema,
            absolute_length_to_px_ratios.data(),
            absolute_length_to_px_ratios.size());
        return true;
    }();
    (void)registered;
}
...
RustFFI::FfiAdditionalBoxFacts build_layout_box_facts(NodeWithStyle const& node)
{
...
Optional<RustFFI::FfiFormattingContextType> formatting_context_type_created_by_box(Box const& box)
{
    auto type = RustFFI::rust_layout_formatting_context_type_for_box({
        ...
        .can_have_children = box.can_have_children(),
        .creates_block_formatting_context = creates_block_formatting_context(box),
    });
...
void LayoutRustBridge::run_root_layout(Box& viewport, NodeWithStyleAndBoxModelMetrics* document_element_layout_node, CSSPixels viewport_inline_size, CSSPixels viewport_block_size, u32 node_count, bool should_collect_devtools_layout_data)
{
...
RustFFI::FfiLayoutFcCallbacks LayoutRustBridge::formatting_context_callbacks(Node const* subtree_root)
{
    ...
    return {
        ...
        .build_box_facts = [](void*, void* node) {
            ...
            RustFFI::FfiAdditionalBoxFacts facts {};
...
static RustFFI::FfiResidualStyleValues decode_residual_style(RustFFI::FfiStylePayloads const& payloads)
{
    auto group = [&]<typename Group>() -> Group const& {
        auto const* payload = payloads.groups[layout_style_group_index(Group::style_group_index)];
        VERIFY(payload);
