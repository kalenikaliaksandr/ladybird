...
LayoutState::FormattingContextScope::FormattingContextScope(LayoutState& state, NodeWithStyle const& root)
    : m_state(&state)
    , m_root(&root)
{
    m_state->push_formatting_context_root(root);
}

LayoutState::FormattingContextScope::FormattingContextScope(FormattingContextScope&& other)
...
LayoutState::FormattingContextScope::~FormattingContextScope()
{
    if (m_state)
        m_state->pop_formatting_context_root(*m_root);
}

LayoutState::FormattingContextScope LayoutState::enter_formatting_context(NodeWithStyle const& root)
{
    return FormattingContextScope { *this, root };
}

void LayoutState::push_formatting_context_root(NodeWithStyle const& root)
{
    m_formatting_context_root_stack.append(&root);
}

void LayoutState::pop_formatting_context_root(NodeWithStyle const& root)
{
    VERIFY(!m_formatting_context_root_stack.is_empty());
    VERIFY(m_formatting_context_root_stack.last() == &root);
    m_formatting_context_root_stack.take_last();
}

void LayoutState::verify_formatting_context_boundary_read(NodeWithStyle const& node, StringView operation) const
{
    ...
    if (auto const* containing_block = node.containing_block()) {
        if (&formatting_context_root == containing_block || formatting_context_root.is_inclusive_ancestor_of(*containing_block))
            return;
        dbgln("LayoutState {} crossed formatting context boundary: box={}, containing block={}, FC root={}",
            operation, node.debug_description(), containing_block->debug_description(), formatting_context_root.debug_description());
    } else {
        dbgln("LayoutState {} crossed formatting context boundary: box={}, containing block=(none), FC root={}",
            operation, node.debug_description(), formatting_context_root.debug_description());
    }
...
LayoutState::UsedValues& LayoutState::ensure_used_values_for(NodeWithStyle const& node)
{
    ...
    if (m_subtree_root == &node) {
        // For the subtree root, ancestor values are provided as LayoutInput rather than
        // reached through the throwaway state's containing-block chain.
    ...
    auto& used_values = m_used_values_store.allocate(index);
    auto percentage_basis = containing_block_used_values
        ? percentage_basis_for_containing_block(*containing_block_used_values)
        : PercentageBasis {};
    used_values.set_node(node, containing_block_used_values, percentage_basis.width, percentage_basis.height);
    return used_values;
}

LayoutState::UsedValues const* LayoutState::try_get(NodeWithStyle const& node) const
{
    verify_formatting_context_boundary_read(node, "try_get"sv);
...
{
    verify_formatting_context_boundary_read(node, "try_get_mutable"sv);
    return m_used_values_store.get(node.layout_index());
}
...
LayoutState::UsedValues const* LayoutState::try_get_existing(NodeWithStyle const& node) const
{
    return m_used_values_store.get(node.layout_index());
}
...
LayoutState::UsedValues& LayoutState::UsedValues::operator=(UsedValues const& other)
{
    ...
    m_percentage_basis_width = other.m_percentage_basis_width;
    m_percentage_basis_height = other.m_percentage_basis_height;
...
LayoutState::PercentageBasis LayoutState::percentage_basis_for_containing_block(UsedValues const& containing_block_used_values)
{
    if (auto const& grid_area_size = containing_block_used_values.grid_area_size(); grid_area_size.has_value()) {
        return {
    ...
    auto const& computed_height = containing_block_used_values.node().computed_values().height();
    auto height_establishes_definite_containing_block = !computed_height.is_auto()
        && !computed_height.is_intrinsic_sizing_constraint()
        && (!computed_height.contains_percentage() || containing_block_used_values.percentage_basis_height().has_value());
    ...
    return {
        containing_block_used_values.has_definite_width()
            ...
            : should_forward_indefinite_basis ? containing_block_used_values.percentage_basis_width()
                                              : Optional<CSSPixels> {},
        containing_block_used_values.has_definite_height()
                || height_establishes_definite_containing_block
            ? Optional<CSSPixels> { containing_block_used_values.content_height() }
            : should_forward_indefinite_basis ? containing_block_used_values.percentage_basis_height()
                                              : Optional<CSSPixels> {},
    };
}
...
void LayoutState::UsedValues::set_node(NodeWithStyle const& node, UsedValues const* containing_block_used_values, Optional<CSSPixels> percentage_basis_width, Optional<CSSPixels> percentage_basis_height)
{
    ...
    m_percentage_basis_width = percentage_basis_width;
    m_percentage_basis_height = percentage_basis_height;
    ...
    auto containing_block_size_for_axis = [&](bool width) {
        return width ? percentage_basis_width_or_zero() : percentage_basis_height_or_zero();
    };
    ...
    auto adjust_for_box_sizing = [&](CSSPixels unadjusted_pixels, CSS::Size const& computed_size, bool width) -> CSSPixels {
        ...
        if (width) {
            border_and_padding = computed_values.border_left().width
                + computed_values.padding().left().to_px_or_zero(percentage_basis_width_or_zero())
                + computed_values.border_right().width
                + computed_values.padding().right().to_px_or_zero(percentage_basis_width_or_zero());
        } else {
            border_and_padding = computed_values.border_top().width
                + computed_values.padding().top().to_px_or_zero(percentage_basis_width_or_zero())
                + computed_values.border_bottom().width
                + computed_values.padding().bottom().to_px_or_zero(percentage_basis_width_or_zero());
        }
    ...
    auto is_definite_size = [&](CSS::Size const& size, CSSPixels& resolved_definite_size, bool width) {
        ...
        auto containing_block_has_definite_size = width ? m_percentage_basis_width.has_value() : m_percentage_basis_height.has_value();
