...
static CSSPixelRect measure_scrollable_overflow(Box const& box, ContainedBoxesMap const& contained_boxes_map)
{
    ...
    // Replaced SVG viewports clip their content, and their descendants' geometry
    // is in viewport user units rather than CSS pixels, so SVG content never
    // contributes scrollable overflow.
    if (is<SVGSVGBox>(box)) {
