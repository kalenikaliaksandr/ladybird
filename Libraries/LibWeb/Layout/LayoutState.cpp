...
LayoutState::UsedValues& LayoutState::create(NodeWithStyle const& node, Optional<CSSPixels> percentage_basis_width, Optional<CSSPixels> percentage_basis_height)
{
    ...
    // A list item's marker is placed by the list item itself, which is also its containing
    // block, so the marker's used values are created together with the list item's, with the
    // list item's basis passed down. List items are never anonymous, so no basis forwarding
    // is needed here: the marker's basis is the list item's definite sizes.
...
LayoutState::UsedValues& LayoutState::ensure_used_values_for(NodeWithStyle const& node)
{
    ...
    auto& used_values = m_used_values_store.allocate(index);
    // Boxes reached lazily here, as containing blocks of an explicitly created box, have not
    // entered layout themselves; nobody has passed a percentage basis down for them.
