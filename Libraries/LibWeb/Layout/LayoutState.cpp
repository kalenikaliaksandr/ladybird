...
static Painting::LineBoxData line_box_data_for(LineBox const& line_box, size_t line_index)
{
    if (line_box.fragments().is_empty())
        return { line_index, {} };
    return { line_index, united_fragment_rect_for_line_box(line_box) };
}

static CSSPixelRect rect_for_line_box(LineBox const& line_box, CSSPixels containing_block_content_width)
{
    // Blank lines (e.g. between consecutive forced breaks in a textarea) have no fragments to
    // derive a rect from; they cover the containing block's content width at their block position.
    if (line_box.fragments().is_empty())
...
void LayoutState::resolve_relative_positions()
{
    ...
    m_used_values_store.for_each([&](UsedValues& used_values) {
        ...
        for (auto& piece : paintable_with_lines->inline_box_pieces()) {
            ...
            auto accumulated = accumulated_relative_insets_from_inline_ancestor_chain(piece_node, nullptr);
            if (!accumulated.offset.is_zero())
                piece.border_box_rect.translate_by(accumulated.offset);
        }
    });
}
        ...
        if (auto* paintable_box = paintable.ptr()) {
            ...
            if (auto* paintable_with_lines = as_if<Painting::PaintableWithLines>(*paintable_box)) {
                ...
                for (size_t line_index = 0; line_index < line_boxes.size(); ++line_index) {
                    ...
                    Painting::LineRecord line_record {
                        .rect = rect_for_line_box(line_box, used_values.content_width()),
                        .first_fragment_index = static_cast<u32>(paintable_with_lines->fragments().size()),
                        .fragment_count = 0,
                    };
                    ...
                    line_record.fragment_count = static_cast<u32>(paintable_with_lines->fragments().size()) - line_record.first_fragment_index;
