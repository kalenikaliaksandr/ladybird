...
static bool is_fragmented_inline_node(Node const& node)
{
...
static InlineAncestorChainRelativeOffset accumulated_relative_insets_from_inline_ancestor_chain(Node const* first_ancestor, Node const* stop_at)
{
    ...
    for (auto const* ancestor = first_ancestor; ancestor && ancestor != stop_at; ancestor = ancestor->parent()) {
        ...
        if (!ancestor->display().is_inline_outside() || !ancestor->display().is_flow_inside())
            break;
        result.found_fragmented_inline_node |= is_fragmented_inline_node(*ancestor);
...
void LayoutState::resolve_relative_positions()
{
    ...
    m_used_values_store.for_each([&](UsedValues& used_values) {
        ...
        for (auto& paintable : node.paintables()) {
            ...
            if (!inline_paintable || !is_fragmented_inline_node(inline_paintable->layout_node()))
                continue;
...
{
    if (auto const* paintable_with_lines = as_if<Painting::PaintableWithLines>(paintable); paintable_with_lines && is_fragmented_inline_node(paintable.layout_node()))
...
static void build_paint_tree(Node& node, Painting::Paintable* fallback_parent_paintable = nullptr, HashMap<size_t, Painting::Paintable*> const* parent_paintable_by_line_index = nullptr)
{
    ...
    if (is_fragmented_inline_node(node)) {
        for (auto& paintable : node.paintables()) {
    ...
    for (auto child = node.first_child(); child; child = child->next_sibling()) {
        ...
        auto* fallback_for_children = node.first_paintable() ? node.first_paintable().ptr() : (is_fragmented_inline_node(node) ? fallback_parent_paintable : nullptr);
        build_paint_tree(*child, fallback_for_children, is_fragmented_inline_node(node) ? &paintable_by_line_index : nullptr);
    }
}
...
void LayoutState::commit(Box& root)
{
    ...
    root.for_each_in_inclusive_subtree([&](Node& node) {
        ...
        if (is_fragmented_inline_node(node) && node.dom_node())
    ...
    auto try_to_relocate_fragment_in_inline_node = [&](auto& fragment, Painting::LineBoxData line_box_data, Painting::Paintable::FragmentationState fragmentation_state) -> bool {
        for (auto const* parent = fragment.layout_node().parent(); parent; parent = parent->parent()) {
            if (!parent->display().is_inline_outside() || !parent->display().is_flow_inside())
                break;
            if (is_fragmented_inline_node(*parent)) {
    ...
    m_used_values_store.for_each([&](UsedValues& used_values) {
        ...
        if (!paintable && !is_fragmented_inline_node(node))
    ...
    for (auto const& paintable : inline_node_paintables.values()) {
        for (auto const* ancestor = paintable->layout_node().parent(); ancestor; ancestor = ancestor->parent()) {
            if (!is_fragmented_inline_node(*ancestor) || ancestor->dom_node())
                break;
    ...
    m_used_values_store.for_each([&](UsedValues& used_values) {
        ...
        if (!node.is_box() || is_fragmented_inline_node(node))
            return;
    ...
    for (auto const& weak_paintable_with_lines : inline_node_paintables) {
        ...
        if (!paintable_with_lines)
            continue;
        if (!is_fragmented_inline_node(paintable_with_lines->layout_node()))
            continue;
        ...
        paintable_with_lines->for_each_in_inclusive_subtree_of_type<Painting::PaintableWithLines>([&](auto& paintable) {
            if (paintable.line_index() != line_index)
                return TraversalDecision::Continue;
            if (is<BlockContainer>(paintable.layout_node()) && !is_fragmented_inline_node(paintable.layout_node()))
