...
{
    // Splice the rebuilt paint subtree exactly where the replaced paintable stands: paint and
    // hit-test order between siblings with equal stacking follow paintable tree order, so
    // reattaching a partially relaid subtree at the end of the parent's children would move
    // it on top of later siblings.
