...
void Node::set_needs_layout_update(DOM::SetNeedsLayoutReason reason, LayoutUpdatePropagation propagation)
{
    ...
    if (propagation == LayoutUpdatePropagation::BoundarySelfOnly) {
        // The boundary keeps its ancestors' dirty flags clean, but their cached fragment
        // trees still contain this subtree's fragments, so their epochs must advance.
        for (auto* ancestor = parent(); ancestor; ancestor = ancestor->parent()) {
            if (auto* box = as_if<Box>(ancestor))
                box->bump_fragment_cache_epoch();
        }
    ...
    // Fragment cache epochs have no such boundary: absolutely positioned and SVG
    // descendants produce fragments inside ancestor run trees, so every box ancestor's
    // epoch must advance.
