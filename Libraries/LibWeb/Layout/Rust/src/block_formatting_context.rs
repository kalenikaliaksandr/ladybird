...
use super::{
    FcFrame, FfiChildLayoutResult, FfiFlexAxis, FfiFormattingContextType, FfiLayoutFcCallbacks,
...
use crate::layout_state::{LayoutState, StaticPositionAlignment, StaticPositionRect};
...
struct FloatingBox<'pass> {
    box_: Node,
    used_values: &'pass UsedValuesCore,
    side: FloatSide,
...
impl<'pass> BlockFormattingContext<'pass> {
    pub(crate) fn new(state: &'pass LayoutState, root: Node, layout_mode: u8, callbacks: FfiLayoutFcCallbacks) -> Self {
        Self {
            state,
            root,
            layout_mode,
    ...
    fn create_used_values(&self, node: Node, constraints: ContainingBlockConstraints) -> &'pass UsedValuesCore {
    ...
    fn compute_inline_size(
        ...
        if self.used(node).inline_size_constraint.get() != FfiSizeConstraint::None {
            return;
        }
    ...
    fn layout_inside(
        ...
        match super::layout_inside_child(
            frame,
            Some(self),
            None,
            node,
            ...
            super::ChildLayoutOutcome::Skipped => None,
            super::ChildLayoutOutcome::Created(result) => Some(result),
    ...
    fn finish_inside_layout(&self, frame: &mut FcFrame<'pass>, node: Node) {
    ...
    fn layout_block_level_box(
        ...
        if self.try_used_pointer(node).is_some() {
            assert!(
                self.state.may_reuse_precreated_used_values(facts.layout_index),
        ...
        let child_result = if has_independent_formatting_context {
            ...
            let result = self.layout_inside(frame, node, inside_layout_input, true);
        ...
        *bottom_of_lowest_margin_box = (*bottom_of_lowest_margin_box)
        ...
        if has_independent_formatting_context && child_result.is_some() {
            self.finish_inside_layout(frame, node);
        }
    }
    ...
    fn layout_block_level_children(
        ...
        if self.layout_mode == layout_mode::INTRINSIC_SIZING && !self.used(block_container).has_definite_inline_size() {
            ...
            if self.used(block_container).inline_size_constraint.get() == FfiSizeConstraint::None {
                let sizing = self.sizing();
    ...
    fn layout_fieldset_with_rendered_legend(&self, frame: &mut FcFrame<'pass>, fieldset: Node, input: LayoutInput) {
        ...
        if self.layout_mode == layout_mode::INTRINSIC_SIZING && !self.used(fieldset).has_definite_inline_size() {
            ...
            if self.used(fieldset).inline_size_constraint.get() == FfiSizeConstraint::None {
                let sizing = self.sizing();
                if !sizing.should_treat_max_inline_size_as_none(
                    fieldset,
                    available_space.inline_size,
    ...
    pub(crate) fn layout_table_caption(
        ...
        if child_was_created {
            self.finish_inside_layout(frame, caption);
        }
        result
    }
    ...
    pub(crate) fn layout_floating_box(
        ...
        let root_index = self.facts(self.root).layout_index;
        let mut rare = self.state.block_rare_data_mut(root_index);
    ...
    fn layout_inline_children(
        ...
        if context.facts(block_container).is_anonymous {
            let non_anonymous = context.callbacks.non_anonymous_containing_block(block_container);
        ...
        if !self.used(block_container).has_definite_inline_size() {
            ...
            if self.used(block_container).inline_size_constraint.get() == FfiSizeConstraint::None {
                ...
                let min_is_auto = style.min_width().is_auto()
                    ...
                    || (style.min_width().is_max_content() && input.available_space.inline_size.is_max_content())
                    || (style.min_width().is_min_content() && input.available_space.inline_size.is_min_content());
                if !min_is_auto {
    ...
    fn compute_automatic_block_size_for_block_level_element(
        ...
        if facts.children_are_inline {
            let line_data = self.state.line_data(facts.layout_index);
    ...
    pub(crate) fn greatest_child_inline_size_including_floats(&self, node: Node) -> CssPixels {
        ...
        let facts = self.facts(node);
        if facts.children_are_inline {
            if let Some(data) = self.state.line_data(facts.layout_index) {
...
pub(crate) fn automatic_block_size_for_bfc_root(
    ...
    if facts.children_are_inline {
        ...
        let line_data = state.line_data(facts.layout_index);
