...
use super::inline::line_builder::LineBuilder;
use super::inline::{FfiCssPixelRect, FfiSpaceUsedByFloats, InlineFormattingContext};
use super::sizing::SizingContext;
use super::{
    FfiChildLayoutResult, FfiFlexAxis, FfiFormattingContextType, FfiLayoutFcCallbacks,
...
fn round_css_pixels(value: CssPixels) -> CssPixels {
    let half = CssPixels::from_raw(crate::css_pixels::FIXED_POINT_DENOMINATOR >> 1);
...
pub(crate) struct BlockFormattingContext {
    ...
    pending_table_box_content_offset_in_wrapper: Cell<LogicalOffset>,
...
impl BlockFormattingContext {
    pub(crate) fn new(
        ...
        Self {
            ...
            pending_table_box_content_offset_in_wrapper: Cell::new(LogicalOffset::default()),
    ...
    fn used(&self, node: Node) -> &UsedValuesCore {
        let pointer = self.used_pointer(node);
    ...
    fn used_mut(&self, node: Node) -> &mut UsedValuesCore {
        let pointer = self.used_pointer(node);
        // SAFETY: Layout serializes mutation of each used-values entry.
        unsafe { &mut *pointer }
    }
    ...
    fn is_ancestor_of(&self, ancestor: Node, node: Node) -> bool {
        // SAFETY: Both tree nodes remain live for the pass.
    ...
    fn register_contained_abspos_child(&self, node: Node, block_offset: CssPixels) {
        let static_position = FfiStaticPositionRect {
            rect: LogicalRect {
    ...
    fn containing_block_rect(&self, node: Node, position: FfiCssPixelPoint) -> CssPixelRect {
        ...
        CssPixelRect {
            x: position.x,
            y: position.y,
            width: used.content_inline_size,
            height: used.content_block_size,
        }
    }

    fn margin_box_rect(used: &UsedValuesCore) -> CssPixelRect {
        CssPixelRect {
            x: -used.margin_left - used.border_box_left(false),
            y: -used.margin_top - used.border_box_top(false),
            width: used.margin_box_inline_size(false),
            height: used.margin_box_block_size(false),
        }
    }
    ...
    fn compute_inline_size(
        ...
        if available_space.inline_size.is_definite() && self.box_should_avoid_floats_because_it_establishes_fc(node) {
            ...
            let box_in_root_rect = CssPixelRect {
                ...
                height: self.used(node).content_block_size,
            };
        ...
        if self.used(node).inline_size_constraint != FfiSizeConstraint::None {
            return;
        }
    ...
    pub(crate) fn available_inline_space(
        ...
        let bands = self.bands.borrow();
        assert!(!bands.is_empty());
        let mut intrusions = FfiSpaceUsedByFloats::default();
    ...
    fn intrusions_for_band_into_rect(&self, band: FloatBand, rect_in_root: CssPixelRect) -> FfiSpaceUsedByFloats {
        // Deliberately read the root inline size at query time. It can still be stale
        // while intrinsic sizing is in progress.
        let root_content_inline_size = self.used(self.root).content_inline_size;
        FfiSpaceUsedByFloats {
    ...
    fn place_float(
        ...
        loop {
            ...
            if available_space.inline_size.is_max_content()
                || available_space.inline_size.is_indefinite()
                ...
                || !has_floats_present
            {
                return FloatPlacement {
                    ...
                    offset_from_edge: if side == FloatSide::Left {
                        intrusions.left + used.margin_left + used.border_box_left(false)
                    } else {
                        intrusions.right + used.content_inline_size + used.margin_right + used.border_box_right(false)
                    },
                };
            }
    ...
    fn update_lowest_floating_descendant_bottom_margin_edge(&self) {
        let lowest = self
            ...
            .max();
        let root_index = self.facts(self.root).layout_index;
        state_mut(self.state)
            .block_rare_data_mut(root_index)
    ...
    fn border_box_left_of_box_avoiding_floats(
        ...
        if used.margin_left >= CssPixels::default() {
            return space_used_by_floats.left.max(used.margin_left);
        }
        ...
        space_used_by_floats.left + used.margin_left
    }
    ...
    fn compute_normal_flow_inline_offset(
        ...
        if self.box_should_avoid_floats_because_it_establishes_fc(node) {
            ...
            inline_offset = self.border_box_left_of_box_avoiding_floats(node, used, space) - used.margin_left;
        }

        let containing_block = self.containing_block(node);
        let containing_text_align = self.style(containing_block).text_align;
        if containing_text_align == text_align::LIBWEB_CENTER {
            inline_offset += available_inline_size_within_containing_block / 2 - used.content_inline_size / 2;
        } else if containing_text_align == text_align::LIBWEB_RIGHT {
            ...
            inline_offset += used.margin_left + used.border_box_left(false);
        }
        inline_offset
    }
    ...
    fn layout_list_item_marker(
        &self,
        list_item: Node,
        inline_space_used_before_list_item_elements_formatted: FfiSpaceUsedByFloats,
    ) {
        ...
        let marker_block_size = marker_used.content_block_size;
        let marker_inline_size = marker_used.content_inline_size;
        ...
        let marker_inline_offset = if list_item_style.direction == direction::LTR {
            ...
            list_item_used.content_inline_size
        ...
        if marker_facts.marker_list_style_position == list_style_position::INSIDE {
            ...
            self.used_mut(list_item).set_content_inline_size(
                self.used(list_item).content_inline_size - marker_inline_size - marker_distance,
            );
        }
        ...
        if marker_style.line_height > self.used(list_item).content_block_size {
    ...
    fn child_layout_input(
        ...
        containing_input: FfiLayoutInput,
        available_space: AvailableSpace,
    ) -> FfiLayoutInput {
        FfiLayoutInput {
            available_space,
            ...
            has_content_box_position_in_bfc_root: containing_input.has_content_box_position_in_bfc_root,
    ...
    fn layout_block_level_box(
        ...
        let block_container_inline_size = self.used(block_container).content_inline_size;
        if !facts.may_reuse_precreated_used_values || self.try_used_pointer(node).is_null() {
        ...
        assert!(input.has_content_box_position_in_bfc_root);
        let containing_block_position_in_root = input.content_box_position_in_bfc_root;
        ...
        if facts.is_floating {
            ...
            self.layout_floating_box(node, block_container, input, margin_top + block_offset, None);
        ...
        self.margin_state.borrow_mut().add_margin(self.used(node).margin_top);
        ...
        let box_opens_top_margin_group = !has_independent_formatting_context
            && self.used(node).border_top == CssPixels::default()
            && self.used(node).padding_top == CssPixels::default()
        ...
        if is_list_item_box_without_css_content && !marker.is_null() {
                ...
                && style.direction == direction::LTR
            {
                content_inline_offset +=
                    self.used(marker).content_inline_size + self.distance_between_marker_and_list_item(marker);
            }
        }
        ...
        if box_is_positioned_by_fieldset_layout {
            ...
            self.pending_table_box_content_offset_in_wrapper.set(LogicalOffset {
                inline_offset: content_inline_offset,
                block_offset: content_block_offset,
            });
        ...
        let mut inline_space_used_before_children_formatted = FfiSpaceUsedByFloats::default();
        if is_list_item_box_without_css_content && !marker.is_null() {
            let marker_block_offset =
                ((self.style(marker).line_height - self.used(marker).content_block_size) / 2).max(CssPixels::default());
            ...
            inline_space_used_before_children_formatted = self.intrusion_by_floats_into_rect(
                CssPixelRect {
                    ...
                    width: list_item_used.content_inline_size,
                    height: list_item_used.content_block_size,
                }
                .into(),
                marker_block_offset,
                marker_block_offset,
        ...
        let child_result = if has_independent_formatting_context {
            ...
            let inside_layout_input = FfiLayoutInput {
                ...
                has_table_grid_min_border_box_block_size: is_table_formatting_context
                    && input.has_table_grid_min_border_box_block_size,
                table_grid_min_border_box_block_size: input.table_grid_min_border_box_block_size,
            };
            let result = self.layout_inside(node, inside_layout_input, true);
            if is_table_formatting_context {
                let pending = self.pending_table_box_content_offset_in_wrapper.get();
                pending_position = Some(FfiCssPixelPoint {
            ...
            if container_facts.is_table_wrapper && style.display.is_table_inside() {
                let used = self.used_mut(node);
                used.margin_left = used.margin_left.max(CssPixels::default());
                used.margin_right = used.margin_right.max(CssPixels::default());
            }
            ...
            {
                self.used_mut(node)
                    .set_content_inline_size(result.automatic_content_inline_size);
            }
            result
        } else {
            ...
            if self.used(node).border_top > CssPixels::default() || self.used(node).padding_top > CssPixels::default() {
            ...
            let inside_layout_input = FfiLayoutInput {
                has_content_box_position_in_bfc_root: true,
                content_box_position_in_bfc_root: FfiCssPixelPoint {
            ...
            if facts.children_are_inline {
                self.layout_inline_children(node, inside_layout_input, space_available_for_children);
            } else {
                self.layout_block_level_children(node, inside_layout_input, space_available_for_children);
            }
        ...
        if !style.display.is_table_inside() {
            self.resolve_used_block_size_if_treated_as_auto(
                ...
                available_space_for_block_size_resolution,
                input.containing_block_constraints,
                child_result.map(|result| result.automatic_content_block_size),
            );
        }
        ...
        if has_independent_formatting_context || !self.margins_collapse_through(node) {
            ...
            self.block_offset_of_current_block_container.set(Some(
                used.content_offset.y + used.content_block_size + used.border_box_bottom(false),
            ));
        }
        self.margin_state
            .borrow_mut()
            .box_last_in_flow_child_margin_bottom_collapsed = false;
        self.margin_state.borrow_mut().add_margin(self.used(node).margin_bottom);
        self.margin_state.borrow_mut().update_open_top_margin_group();
        ...
        self.compute_inset(
            node,
            LogicalSize {
                inline_size: block_container_used.content_inline_size,
                block_size: block_container_used.content_block_size,
            },
        );
        let used = self.used(node);
        *bottom_of_lowest_margin_box = (*bottom_of_lowest_margin_box)
            .max(used.content_offset.y + used.content_block_size + used.margin_box_bottom(false));

        if has_independent_formatting_context && child_result.is_some() {
    ...
    fn layout_block_level_children(
        ...
        for child in self.children(block_container) {
            self.layout_block_level_box(child, block_container, &mut bottom_of_lowest_margin_box, child_input);
        }
    ...
    fn layout_fieldset_with_rendered_legend(&self, fieldset: Node, input: FfiLayoutInput) {
        let available_space = input.available_space;
        ...
        // Lay out the legend to determine its dimensions.
        {
            ...
            self.layout_block_level_box(legend, fieldset, &mut dummy_bottom, child_input);
            self.block_offset_of_current_block_container.set(saved);
        }
        ...
        let extra_top = effective_border - self.used(fieldset).border_top;
        ...
        let mut bottom_of_lowest_margin_box = CssPixels::default();
        {
            ...
            for child in self.children(fieldset) {
                if child != legend {
                    self.layout_block_level_box(child, fieldset, &mut bottom_of_lowest_margin_box, child_input);
                }
            }
            self.block_offset_of_current_block_container.set(saved);
        }
        ...
        let fieldset_border_box_block_start_in_content =
            -(self.used(fieldset).border_top + self.used(fieldset).padding_top);
    ...
    pub(crate) fn run(&self, input: FfiLayoutInput) {
        let available_space = input.available_space;
        ...
        let root_input = FfiLayoutInput {
            has_content_box_position_in_bfc_root: true,
            content_box_position_in_bfc_root: FfiCssPixelPoint::default(),
            ..input
        };
        let root_facts = self.facts(self.root);
        if root_facts.is_fieldset_box && !root_facts.rendered_legend.is_null() {
            self.layout_fieldset_with_rendered_legend(self.root, root_input);
            return;
        }
        if root_facts.children_are_inline {
            self.layout_inline_children(self.root, root_input, available_space);
        } else {
            self.layout_block_level_children(self.root, root_input, available_space);
        }
        ...
        if collapsed_margin != CssPixels::default() {
            for child in self.children(self.root).into_iter().rev() {
                ...
                self.used_mut(child).margin_bottom = collapsed_margin;
                break;
            }
    ...
    pub(crate) fn parent_context_did_dimension_child_root_box(&self) {
        ...
        for floating_box in floats {
            ...
            let inline_offset = if floating_box.side == FloatSide::Left {
                ...
                let float_containing_block_inline_size = match used.inline_size_constraint {
                    FfiSizeConstraint::MinContent => CssPixels::default(),
                    // Preserve the MaxContent saturation quirk from the C++ fixed-point subtraction.
                    FfiSizeConstraint::MaxContent => CssPixels::from_raw(i32::MAX),
                    FfiSizeConstraint::None => floating_box.percentage_basis_inline_size.unwrap_or_default(),
                };
    ...
    pub(crate) fn pending_table_box_content_offset_in_wrapper(&self) -> LogicalOffset {
        self.pending_table_box_content_offset_in_wrapper.get()
    }

    pub(crate) fn set_pending_table_box_content_offset_in_wrapper(&self, offset: LogicalOffset) {
        self.pending_table_box_content_offset_in_wrapper.set(offset);
    }
    ...
    pub(crate) fn layout_table_caption(
        ...
        let mut child_was_created = false;
        if formatting_context_type_created_by_box(self.facts(caption)).is_some() {
            ...
            if is_block_context {
                ...
                if phase == 1 {
                    caption_offset.block_offset = self.used(table_box).margin_box_block_size(false)
                        + self.used(caption).margin_top
                        + self.used(caption).border_box_top(false);
                }
            }

            let child_result = self.layout_inside(
                caption,
                FfiLayoutInput {
                    available_space: inner_available_space,
            ...
            child_was_created = child_result.is_some();
            if is_block_context && let Some(child_result) = child_result {
                if self
                    .sizing()
        ...
        let result = CaptionLayoutResult {
            ...
            pending_table_block_offset: if phase == 0 {
                self.used(caption).content_block_size + self.used(caption).margin_box_bottom(false)
            } else {
                CssPixels::default()
            },
        };
    ...
    pub(crate) fn layout_interrupting_block_inside_inline_context(
        ...
        // SAFETY: The builder remains live for this bounded parent call.
        let current_block_offset = unsafe { (*line_builder).current_block_offset() };
        let saved = self
            .block_offset_of_current_block_container
            .replace(Some(current_block_offset));
        let mut dummy_bottom = CssPixels::default();
        self.layout_block_level_box(node, containing_block, &mut dummy_bottom, input);
        // SAFETY: The builder remains live and no reference escaped.
        let block_bottom = self
            .block_offset_of_current_block_container
            .get()
            .unwrap_or_else(|| unsafe { (*line_builder).current_block_offset() });
        self.block_offset_of_current_block_container.set(saved);
        // SAFETY: This is the same bounded re-entry used by the old C ABI export.
        unsafe {
    ...
    pub(crate) fn layout_floating_box(
        ...
        line_builder: Option<*mut LineBuilder>,
    ) {
        let available_space = input.available_space;
        assert!(self.facts(node).is_floating);
        let block_container_inline_size = self.used(block_container).content_inline_size;
        self.resolve_vertical_box_model_metrics(node, block_container_inline_size);
        let containing_block_rect = self.containing_block_rect(block_container, input.content_box_position_in_bfc_root);
        ...
        let mut margin_box_ceiling = if let Some(line_builder) = line_builder {
            // SAFETY: The builder remains live for this bounded parent call.
            unsafe { (*line_builder).ceiling_for_float_to_be_inserted_here(node) }
        } else {
            block_offset
        };
        ...
        let floating_box = FloatingBox {
            ...
            top_margin_edge: content_block_offset - self.used(node).margin_top - self.used(node).border_box_top(false),
            bottom_margin_edge: content_block_offset
                + self.used(node).content_block_size
            ...
            percentage_basis_inline_size: input
                .containing_block_constraints
                .has_percentage_basis_inline_size
                .then_some(input.containing_block_constraints.percentage_basis_inline_size),
        };
        ...
        let root_index = self.facts(self.root).layout_index;
        let rare = state_mut(self.state).block_rare_data_mut(root_index);
    ...
    fn layout_inline_children(
        ...
        context.style(block_container);
        if context.facts(block_container).is_anonymous {
            // SAFETY: The host returns the live percentage/style owner.
            let non_anonymous = unsafe {
                (context.callbacks.non_anonymous_containing_block)(context.callbacks.context, block_container)
            };
            if !non_anonymous.is_null() {
                context.style(non_anonymous);
            }
        }
        crate::ffi_stats::exclude_root_fact_builds(fact_build_counts);
        context.run();
    ...
    fn compute_automatic_block_size_for_block_level_element(
        ...
        if facts.children_are_inline {
            let line_data = state_mut(self.state).line_data(facts.layout_index);
            if let Some(last_line) = line_data.and_then(|data| data.line_boxes.last()) {
                ...
                if last_line.has_block_level_box {
                    ...
                    let used = self.used(node);
                    if used.padding_bottom == CssPixels::default() && used.border_bottom == CssPixels::default() {
        ...
        if !facts.children_are_inline {
            ...
            for child in self.children(node).into_iter().rev() {
                ...
                return (child_used.content_offset.y
                    + child_used.content_block_size
    ...
    pub(crate) fn greatest_child_inline_size_including_floats(&self, node: Node) -> CssPixels {
        ...
        let inline_size_to_make_room_for_float_margin_box = |floating_box: &FloatingBox| {
            ...
            if floating_box.side == FloatSide::Left {
                ...
                floating_box.offset_from_edge + used.margin_left + used.border_box_left(false)
            }
        };
        ...
        {
            let mut inline_space = FfiSpaceUsedByFloats::default();
        ...
        let facts = self.facts(node);
        if facts.children_are_inline {
            if let Some(data) = state_mut(self.state).line_data(facts.layout_index) {
                for line in &data.line_boxes {
                    let mut inline_size_here = super::inline::line_physical_horizontal_extent(line);
            ...
            for child in self.children(node) {
                ...
                let child_used = self.try_used_pointer(child);
                if !child_used.is_null() {
                    // SAFETY: Non-null state entries remain stable.
                    max_inline_size = max_inline_size.max(unsafe { (*child_used).margin_box_inline_size(false) });
                }
            }
        }
        max_inline_size
    ...
    pub(crate) fn automatic_content_inline_size(&self) -> CssPixels {
        let root_facts = self.facts(self.root);
        if root_facts.children_are_inline {
            return self.used(self.root).content_inline_size;
        }
        if root_facts.is_table_wrapper {
            let mut stack = vec![self.root];
            while let Some(node) = stack.pop() {
                for child in self.children(node).into_iter().rev() {
                    if self.style(child).display.is_table_inside() {
                        return self.used(child).border_box_inline_size(false);
                    }
                    stack.push(child);
                }
            }
...
pub(crate) fn automatic_block_size_for_bfc_root(
    ...
    let used_pointer = state_mut(state).used_values(&callbacks, root);
    // SAFETY: State-owned used values remain stable.
    ...
    if facts.children_are_inline {
        ...
        if let Some(last_line) = state_mut(state)
            .line_data(facts.layout_index)
            .and_then(|data| data.line_boxes.last())
        {
        ...
        while !child.is_null() {
            ...
            bump(FfiOp::NavigationCallback);
            // SAFETY: The host owns the tree.
            child = unsafe { (callbacks.navigation.next_sibling)(callbacks.navigation.context, child) };
        }
    }
    ...
    if let Some(lowest) = state_mut(state)
        .block_rare_data(facts.layout_index)
