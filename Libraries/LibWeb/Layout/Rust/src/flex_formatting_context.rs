...
use super::{
    FcFrame, FfiFlexAxis, FfiFlexLayoutData, FfiFlexLayoutItem, FfiFlexLayoutItemRect, FfiFlexLayoutLine,
    FfiFlexSizeProperty, FfiLayoutFcCallbacks,
};
use crate::box_facts::BoxFactsView;
use crate::css_enums::{
    align_content, align_items, align_self, display_inside, flex_direction, flex_wrap, justify_content, layout_mode,
    writing_mode,
};
use crate::css_pixels::{CssPixels, clamp_to_max_dimension_value, css_clamp};
use crate::geometry::{AvailableSize, AvailableSpace, ContainingBlockConstraints, LayoutInput, to_physical};
...
impl<'pass> FlexItem<'pass> {
    fn new(box_: Node, used_values: &'pass UsedValuesCore) -> Self {
        Self {
            box_,
            used_values,
...
pub(super) struct FlexFormattingContext<'pass> {
    ...
    flex_container_state: &'pass UsedValuesCore,
...
impl<'pass> FlexFormattingContext<'pass> {
    pub(super) fn new(frame: &FcFrame<'pass>) -> Self {
    ...
    fn item_used(&self, index: usize) -> &'pass UsedValuesCore {
        self.flex_items[index].used_values
    }

    fn item_used_mut(&self, index: usize) -> &'pass UsedValuesCore {
        self.flex_items[index].used_values
    }
    ...
    fn create_used_values(&self, node: Node) -> &'pass UsedValuesCore {
    ...
    fn layout_inside_item(&mut self, frame: &mut FcFrame<'pass>, index: usize) {
        ...
        match super::layout_inside_child(frame, None, None, node, layout_mode::NORMAL, input, false) {
            super::ChildLayoutOutcome::Created(_) => super::finish_child_layout(frame, node),
            super::ChildLayoutOutcome::ReenterCurrent => self.run(frame, input),
            super::ChildLayoutOutcome::Skipped => {}
        }
    ...
    pub(super) fn run(&mut self, frame: &mut FcFrame<'pass>, layout_input: LayoutInput) {
