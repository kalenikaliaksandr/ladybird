...
use crate::box_facts::{BoxFactsView, FfiAdditionalBoxFacts};
...
use crate::layout_state::{
    FfiCommitSink, LayoutState, LayoutStatePurpose, StaticPositionAlignment, StaticPositionRect,
};
use crate::node_data::{NodeData, NodeFlag, NodeSlotId};
...
pub(crate) mod abspos {
    ...
    use super::{FfiFlexAxis, FfiLayoutFcCallbacks};
    use crate::box_facts::BoxFactsView;
    ...
    use crate::ffi_stats::{FfiOp, bump};
    use crate::geometry::{
        AvailableSize, AvailableSpace, ContainingBlockConstraints, LayoutInput, LogicalOffset, LogicalRect, LogicalSize,
    };
    use crate::layout_state::{
        AbsposAlignment, AbsposAxisMode, AbsposContainingBlockInfo, AbsposLayoutInputs, LayoutState,
        StaticPositionAlignment, StaticPositionRect,
    };
    ...
    impl<'a, 'pass> AbsposEngine<'a, 'pass> {
        ...
        fn facts(&self, node: Node) -> BoxFactsView<'pass> {
            self.state.box_facts(&self.callbacks, node)
        }

        fn used_pointer(&self, node: Node) -> &'pass UsedValuesCore {
            self.state.used_values(&self.callbacks, node)
        }

        fn try_used_pointer(&self, node: Node) -> Option<&'pass UsedValuesCore> {
            self.state.try_used_values(&self.callbacks, node)
        }

        fn used(&self, node: Node) -> &'pass UsedValuesCore {
            self.used_pointer(node)
        }

        fn used_mut(&self, node: Node) -> &'pass UsedValuesCore {
            self.used_pointer(node)
        }
        ...
        fn line_fragments(&self, node: Node) -> Vec<LineFragmentFacts> {
            ...
            let Some(lines) = self.state.line_data(facts.layout_index) else {
                return fragments;
            };
                ...
                if facts.is_box && !facts.is_anonymous {
                    ...
                    if let Some(child_used) = child_used_pointer {
                        ...
                        let border_box_origin = FfiCssPixelPoint {
                            x: child_offset.x
                                - child_used.border_left_collapsed(collapsed)
                                - child_used.padding_left.get(),
                            y: child_offset.y
                                - child_used.border_top_collapsed(collapsed)
                                - child_used.padding_top.get(),
                        };
                        add_fragment_rect(
                            PhysicalRect {
        ...
        fn base_containing_block_info(&self, node: Node) -> AbsposContainingBlockInfo {
                ...
                && let Some(rect) = self.compute_inline_containing_block_rect(inline_containing_block, containing_block)
            {
                return AbsposContainingBlockInfo {
                    ...
                    inline_alignment: AbsposAlignment::Normal,
    ...
    impl<'pass> AbsposEngine<'_, 'pass> {
        fn layout_element(
            &self,
            frame: &mut super::FcFrame<'pass>,
            parent_block: Option<&block::BlockFormattingContext<'pass>>,
            ...
            let inner_available_space = self
                .used(node)
                .available_inner_space_or_constraints_from(available_space);
            let has_independent_context = match super::layout_inside_child(
                frame,
                parent_block,
                parent_grid,
                node,
                ...
                super::ChildLayoutOutcome::Created(_) => true,
                super::ChildLayoutOutcome::Skipped => false,
                ...
                super::ChildLayoutOutcome::ReenterCurrent => {
            ...
            if self.layout_mode == layout_mode::NORMAL && !is_measurement {
                bump(FfiOp::AbsposSavedInputsSet);
                self.state
            ...
            if has_independent_context {
                super::finish_child_layout(frame, node);
            }
        }
        ...
        pub(crate) fn layout_children(
            ...
            loop {
                ...
                let Some(child) = self.state.take_next_contained_abspos_child(box_) else {
                    break;
                };
        ...
        fn replay(&self, frame: &mut super::FcFrame<'pass>, node: Node) {
    ...
    pub(super) fn layout_children_for_instance(instance: &mut super::FormattingContextInstance, box_: Node) {
        ...
        let super::FormattingContextInstance { frame, implementation } = instance;
        let (parent_block, parent_grid) = match &*implementation {
            super::FcImpl::Block(context) => (Some(context.as_ref()), None),
            super::FcImpl::Grid(context) => (None, Some(context.as_ref())),
            _ => (None, None),
        };
    ...
    pub(super) fn replay_for_instance(instance: &mut super::FormattingContextInstance, node: Node) {
        assert!(matches!(instance.implementation, super::FcImpl::AbsposReplay));
        AbsposEngine::new(
            instance.state,
            instance.callbacks,
...
pub(crate) mod sizing {
    ...
    use super::{FcImpl, FfiFlexAxis, FfiFlexSizeProperty, FfiLayoutFcCallbacks, FormattingContextInstance};
    use crate::box_facts::BoxFactsView;
    use crate::css_enums::{box_sizing, layout_mode};
    use crate::css_pixels::{CssPixels, clamp_to_max_dimension_value};
    use crate::ffi_stats::{FfiOp, bump};
    use crate::geometry::{AvailableSize, AvailableSpace, ContainingBlockConstraints, LayoutInput};
    use crate::layout_node_arena::{IntrinsicSizeCacheKey, IntrinsicSizeCacheKind};
    use crate::layout_state::{LayoutState, LayoutStatePurpose};
    ...
    pub(crate) fn preferred_aspect_ratio(facts: BoxFactsView<'_>) -> Option<PixelFraction> {
    ...
    impl MeasurementState {
        ...
        pub(crate) fn root_used(&self) -> &UsedValuesCore {
            self.state.used_values(&self.callbacks, self.root)
        }

        fn run(&self, node: Node, input: LayoutInput) -> super::FfiChildLayoutResult {
        ...
        pub(crate) fn run_with_layout_mode(
            ...
            super::run_formatting_context(&mut context, input, None, None);
            super::FfiChildLayoutResult {
    ...
    impl<'pass> SizingContext<'pass> {
        ...
        fn has_children(&self, node: Node) -> bool {
            !self.callbacks.first_child(node).is_null()
        }
        ...
        pub(crate) fn constraints_for_child_context(
            ...
            let should_forward_indefinite_basis = facts.is_box
                ...
                && used.inline_size_constraint.get() == FfiSizeConstraint::None
                && used.block_size_constraint.get() == FfiSizeConstraint::None;
        ...
        pub(crate) fn calculate_min_content_inline_size(
            ...
            root.inline_size_constraint.set(FfiSizeConstraint::MinContent);
        ...
        pub(crate) fn calculate_max_content_inline_size(
            ...
            let measurement = MeasurementState::create(self.callbacks, node, constraints);
            let root = measurement.root_used();
            root.inline_size_constraint.set(FfiSizeConstraint::MaxContent);
            root.has_definite_inline_size.set(false);
            let block_size = if root.has_definite_block_size() {
                AvailableSize::definite(root.content_block_size.get())
            } else {
        ...
        pub(crate) fn calculate_min_content_block_size(
            ...
            let measurement = MeasurementState::create(self.callbacks, node, constraints);
            let root = measurement.root_used();
            root.block_size_constraint.set(FfiSizeConstraint::MinContent);
        ...
        pub(crate) fn calculate_max_content_block_size(
            ...
            let measurement = MeasurementState::create(self.callbacks, node, constraints);
            let root = measurement.root_used();
            root.block_size_constraint.set(FfiSizeConstraint::MaxContent);
            root.has_definite_block_size.set(false);
            root.set_content_inline_size(inline_size);
            let result = measurement.run(
                node,
        ...
        fn table_box_inside_wrapper_impl(&self, wrapper: Node, count_navigation: bool) -> Node {
            fn find(context: &SizingContext, parent: Node, count_navigation: bool) -> Option<Node> {
                let mut child = if count_navigation {
                    ...
                    context.callbacks.first_child_without_counter(parent)
                };
                while !child.is_null() {
                    ...
                    child = if count_navigation {
                        ...
                        context.callbacks.next_sibling_without_counter(child)
                    };
                }
                None
            }
        ...
        pub(crate) fn compute_table_box_block_size_inside_wrapper(
            ...
            let table_used_block_size =
                table_used.border_box_block_size(table_used.uses_collapsing_borders_model.get());
...
pub(crate) fn register_contained_abspos_child(
    ...
    loop {
        ...
        let facts = state.box_facts(callbacks, target);
        if containing_block.is_null() || formatting_context_type_created_by_box(facts).is_some() {
            break;
        }
        target = containing_block;
    }
    let child_facts = state.box_facts(callbacks, child);
    bump(FfiOp::AbsposRegister);
    state.register_contained_abspos_child(target, child, child_facts.layout_index, static_position_rect);
}
...
pub(crate) fn box_baseline(
    ...
    let facts = state.box_facts(callbacks, box_);
...
pub(crate) fn compute_and_store_baselines(
    ...
    if line_count > 0 {
        let baseline_for_line_box = |line_index: usize, baseline_set: BaselineSet| -> CssPixels {
            let (has_block_level_box, block_start, baseline, fragment_count, fragment_node) = {
                let line = &state.line_data(facts.layout_index).unwrap().line_boxes[line_index];
                (
        ...
        while first_line_index < line_count {
            let is_empty = state.line_data(facts.layout_index).unwrap().line_boxes[first_line_index].is_empty();
            if !is_empty {
                break;
            }
            first_line_index += 1;
        ...
        while last_line_index > 0 {
            let is_empty = state.line_data(facts.layout_index).unwrap().line_boxes[last_line_index].is_empty();
            if !is_empty {
                break;
            }
            last_line_index -= 1;
    ...
    if callbacks.first_child(box_).is_null() || facts.children_are_inline {
        return;
    }
...
pub type FfiBuildBoxFactsCallback = unsafe extern "C" fn(*mut c_void, *mut c_void) -> FfiAdditionalBoxFacts;
pub type FfiBuildStyleSnapshotCallback =
    unsafe extern "C" fn(*mut c_void, *const c_void) -> crate::style_facts::FfiStyleSnapshot;
...
pub struct FfiLayoutFcCallbacks {
    ...
    pub decode_residual_style: crate::style_facts::FfiDecodeResidualStyleCallback,
...
impl FfiLayoutFcCallbacks {
    ...
    fn node_data_pointer(&self, node: *mut c_void) -> *mut NodeData {
        assert!(!node.is_null());
        // SAFETY: Every shell passed here comes from the single Layout::Node
        // inheritance chain and outlives the synchronous layout pass. C++
        // registered the displacement from that chain in this process, and
        // NodeArena keeps m_data valid under the same pass-long borrow as the
        // arena handle.
        let data = unsafe {
            node.cast::<u8>()
    ...
    pub(crate) fn node_data(&self, node: *mut c_void) -> &NodeData {
        // SAFETY: node_data_pointer() returns the live arena allocation for
        // this shell, and callers only borrow it synchronously.
        unsafe { &*self.node_data_pointer(node) }
    }

    pub(crate) fn saved_abspos_layout_inputs(
        &self,
        node: *mut c_void,
    ) -> Option<crate::layout_state::AbsposLayoutInputs> {
        let data = self.node_data_pointer(node);
        // SAFETY: node_data_pointer() returns a live arena slot.
        assert!(unsafe { crate::box_facts::kind_is_box((*data).kind) });
        self.arena().saved_abspos_layout_inputs(data)
    }

    pub(crate) fn set_saved_abspos_layout_inputs(
        &self,
        node: *mut c_void,
        inputs: Option<crate::layout_state::AbsposLayoutInputs>,
    ) {
        let data = self.node_data_pointer(node);
        // Match prepare_node's former as_if<Box>() guard.
        // SAFETY: node_data_pointer() returns a live arena slot.
        if !unsafe { crate::box_facts::kind_is_box((*data).kind) } {
            return;
        }
        self.arena().set_saved_abspos_layout_inputs(data, inputs);
    }

    fn shell_for_slot(&self, slot: NodeSlotId) -> *mut c_void {
        let data = self.arena().data_or_null(slot);
        if data.is_null() {
            return std::ptr::null_mut();
        }
        // SAFETY: A live arena slot always retains its C++ shell back-pointer.
        unsafe { (*data).shell }
    }

    pub(crate) fn parent(&self, node: *mut c_void) -> *mut c_void {
        bump(FfiOp::NodeQuery);
        self.parent_without_counter(node)
    }

    pub(crate) fn parent_without_counter(&self, node: *mut c_void) -> *mut c_void {
        self.shell_for_slot(self.node_data(node).parent)
    }

    pub(crate) fn first_child(&self, node: *mut c_void) -> *mut c_void {
        bump(FfiOp::NodeQuery);
        self.first_child_without_counter(node)
    }

    pub(crate) fn first_child_without_counter(&self, node: *mut c_void) -> *mut c_void {
        self.shell_for_slot(self.node_data(node).first_child)
    }

    pub(crate) fn next_sibling(&self, node: *mut c_void) -> *mut c_void {
        bump(FfiOp::NodeQuery);
        self.next_sibling_without_counter(node)
    }

    pub(crate) fn next_sibling_without_counter(&self, node: *mut c_void) -> *mut c_void {
        self.shell_for_slot(self.node_data(node).next_sibling)
    }

    pub(crate) fn containing_block(&self, node: *mut c_void) -> *mut c_void {
        bump(FfiOp::NodeQuery);
        self.containing_block_without_counter(node)
    }

    fn containing_block_without_counter(&self, node: *mut c_void) -> *mut c_void {
        self.shell_for_slot(self.node_data(node).containing_block)
    }

    pub(crate) fn inline_containing_block(&self, node: *mut c_void) -> *mut c_void {
        bump(FfiOp::NodeQuery);
        self.shell_for_slot(self.node_data(node).inline_containing_block)
    }

    pub(crate) fn is_ancestor(&self, ancestor: *mut c_void, mut node: *mut c_void) -> bool {
        bump(FfiOp::NodeQuery);
        while !node.is_null() {
            if node == ancestor {
                return true;
            }
            node = self.parent_without_counter(node);
        }
        false
    }

    pub(crate) fn non_anonymous_containing_block(&self, node: *mut c_void) -> *mut c_void {
        bump(FfiOp::NodeQuery);
        let mut containing_block = self.containing_block_without_counter(node);
        assert!(!containing_block.is_null());
        while self.node_data(containing_block).flags & NodeFlag::Anonymous as u32 != 0 {
            containing_block = self.containing_block_without_counter(containing_block);
            assert!(!containing_block.is_null());
        }
        containing_block
    }
...
pub(crate) struct FcFrame<'pass> {
    ...
    pub(crate) child_contexts: HashMap<usize, Box<FormattingContextInstance<'pass>>>,
}

#[derive(Clone, Copy, Default)]
struct FcParents<'parent, 'pass> {
    block: Option<&'parent block::BlockFormattingContext<'pass>>,
    grid: Option<&'parent grid::GridFormattingContext<'pass>>,
}

enum FcImpl<'pass> {
    Block(Box<block::BlockFormattingContext<'pass>>),
    Flex(Box<flex::FlexFormattingContext<'pass>>),
    Grid(Box<grid::GridFormattingContext<'pass>>),
    Table(Box<table::TableFormattingContext<'pass>>),
    Svg(Box<svg::SvgFormattingContext<'pass>>),
    ReplacedWithChildren,
    InternalReplaced,
    InternalDummy,
    AbsposReplay,
...
pub(crate) fn formatting_context_type_created_by_box(facts: BoxFactsView<'_>) -> Option<FfiFormattingContextType> {
    if facts.is_svg_svg_box {
...
pub extern "C" fn rust_layout_formatting_context_type_for_box(facts: FfiFormattingContextArenaFacts) -> u8 {
    abort_on_panic(|| {
        ...
        let is_replaced_box = crate::box_facts::kind_is_replaced_box(data.kind);
        let fc_type = if data.kind == crate::node_data::NodeKind::SVGSVGBox {
            Some(FfiFormattingContextType::Svg)
        } else if is_replaced_box && facts.can_have_children {
            Some(FfiFormattingContextType::ReplacedWithChildren)
        } else if is_replaced_box {
            Some(FfiFormattingContextType::InternalReplaced)
        } else if !facts.can_have_children {
            None
        } else if crate::box_facts::has_flag(data, NodeFlag::IsReplacedElement)
            && data.table_display_before != crate::node_data::FfiTableDisplay::Other
        {
            Some(if crate::box_facts::kind_is_block_container(data.kind) {
                FfiFormattingContextType::Block
            } else {
                FfiFormattingContextType::InternalReplaced
            })
        } else if crate::box_facts::has_display_flag(data, crate::node_data::NodeDisplayFlag::FlexInside) {
            Some(FfiFormattingContextType::Flex)
        } else if data.table_display == crate::node_data::FfiTableDisplay::TableRoot {
            Some(FfiFormattingContextType::Table)
        } else if crate::box_facts::has_display_flag(data, crate::node_data::NodeDisplayFlag::GridInside) {
            Some(FfiFormattingContextType::Grid)
        } else if crate::box_facts::has_display_flag(data, crate::node_data::NodeDisplayFlag::MathInside)
            || facts.creates_block_formatting_context
        {
            Some(FfiFormattingContextType::Block)
        } else if crate::box_facts::has_flag(data, NodeFlag::ChildrenAreInline)
            || matches!(
                data.table_display,
                crate::node_data::FfiTableDisplay::TableColumn
                    | crate::node_data::FfiTableDisplay::TableColumnGroup
                    | crate::node_data::FfiTableDisplay::TableRow
                    | crate::node_data::FfiTableDisplay::TableRowGroup
                    | crate::node_data::FfiTableDisplay::TableHeaderGroup
                    | crate::node_data::FfiTableDisplay::TableFooterGroup
            )
        {
            None
        } else if !crate::box_facts::has_display_flag(data, crate::node_data::NodeDisplayFlag::FlowInside) {
            Some(FfiFormattingContextType::InternalDummy)
        } else {
            None
        };
        fc_type.map(|type_| type_ as u8).unwrap_or(NO_FORMATTING_CONTEXT)
    })
}
...
fn create_formatting_context<'pass>(
    ...
    bump(FfiOp::FcCreate);
    assert!(!box_.is_null());
    let root_facts = state.box_facts(&callbacks, box_);
    if fc_type == FfiFormattingContextType::Block as u8
        && state.mark_bfc_root_fact_builds_excluded(root_facts.layout_index)
    {
        let _ = state.style_facts(&callbacks, box_);
    ...
    let implementation =
        match fc_type {
            type_ if type_ == FfiFormattingContextType::Block as u8 => FcImpl::Block(Box::new(
                block::BlockFormattingContext::new(state, box_, layout_mode, callbacks),
            )),
            type_ if type_ == FfiFormattingContextType::Flex as u8 => {
                FcImpl::Flex(Box::new(flex::FlexFormattingContext::new(&frame)))
            }
            type_ if type_ == FfiFormattingContextType::Grid as u8 => {
                FcImpl::Grid(Box::new(grid::GridFormattingContext::new(
                    state,
                    box_,
                    parents.grid,
                    layout_mode,
            ...
            type_ if type_ == FfiFormattingContextType::Table as u8 => {
                ...
                FcImpl::Table(Box::new(table::TableFormattingContext::new(
                    &frame,
                    pending_table_offset,
                )))
            }
            type_ if type_ == FfiFormattingContextType::Svg as u8 => FcImpl::Svg(Box::new(
                svg::SvgFormattingContext::new(state, box_, layout_mode, callbacks),
            )),
            type_ if type_ == FfiFormattingContextType::ReplacedWithChildren as u8 => FcImpl::ReplacedWithChildren,
            type_ if type_ == FfiFormattingContextType::InternalReplaced as u8 => FcImpl::InternalReplaced,
            type_ if type_ == FfiFormattingContextType::InternalDummy as u8 => FcImpl::InternalDummy,
            type_ if type_ == FfiFormattingContextType::AbsposReplay as u8 => FcImpl::AbsposReplay,
            _ => panic!("no Rust implementation for this formatting context"),
        };
    Box::new(FormattingContextInstance { frame, implementation })
}

fn register_table_abspos_descendants(frame: &mut FcFrame, parent: *mut c_void) {
    let mut child = frame.callbacks.first_child(parent);
    while !child.is_null() {
        let next = frame.callbacks.next_sibling(child);
        let facts = frame.state.box_facts(&frame.callbacks, child);
        if facts.is_box {
            if facts.is_absolutely_positioned {
...
fn parent_did_dimension(instance: &mut FormattingContextInstance) {
    if let FcImpl::Block(context) = &instance.implementation {
        ...
        if instance.callbacks.node_data(instance.box_).table_display != crate::node_data::FfiTableDisplay::TableCell {
...
impl Drop for FormattingContextInstance<'_> {
    fn drop(&mut self) {
        ...
        context.parent_context_did_dimension_child_root_box();
        // The table formatting context handles cell abspos layout after vertical alignment.
        if self.callbacks.node_data(self.box_).table_display != crate::node_data::FfiTableDisplay::TableCell {
            let box_ = self.box_;
...
fn run_formatting_context<'pass>(
    ...
    input: LayoutInput,
    parent_block: Option<&block::BlockFormattingContext<'pass>>,
    _parent_grid: Option<&grid::GridFormattingContext<'pass>>,
) {
    bump(FfiOp::FcRun);
    ...
    match implementation {
        ...
        FcImpl::ReplacedWithChildren => {
            replaced_with_children::run(frame, input);
        }
        FcImpl::InternalReplaced | FcImpl::InternalDummy => replaced_with_children::run_noop(frame),
...
pub(crate) fn layout_inside_child<'pass>(
    ...
    parent_block: Option<&block::BlockFormattingContext<'pass>>,
    parent_grid: Option<&grid::GridFormattingContext<'pass>>,
    child: *mut c_void,
    layout_mode: u8,
    input: LayoutInput,
    force_independent_context_run: bool,
) -> ChildLayoutOutcome {
    assert!(!frame.child_contexts.contains_key(&(child as usize)));
    ...
    frame.child_contexts.insert(child as usize, context);
    ChildLayoutOutcome::Created(result)
}

pub(crate) fn finish_child_layout(frame: &mut FcFrame, child: *mut c_void) {
    let mut child_context = frame
        .child_contexts
...
pub(crate) fn discard_child_layout(frame: &mut FcFrame, child: *mut c_void) {
    frame
        .child_contexts
        .remove(&(child as usize))
        .expect("missing child formatting context");
...
pub extern "C" fn rust_layout_run_root_layout(
    root: *mut c_void,
    document_element_layout_node: *mut c_void,
    ...
    abort_on_panic(|| {
        ...
        let root_constraints = crate::geometry::ContainingBlockConstraints {
        ...
        if !first_child.is_null() && state.box_facts(&callbacks, first_child).is_svg_svg_box {
        ...
        let input = LayoutInput {
            ...
            containing_block_constraints: crate::geometry::ContainingBlockConstraints::default(),
            has_content_box_position_in_bfc_root: false,
            content_box_position_in_bfc_root: FfiCssPixelPoint::default(),
            table_grid_min_border_box_block_size: None,
        };
