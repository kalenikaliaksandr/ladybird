...
use crate::geometry::{AvailableSpace, ContainingBlockConstraints, LayoutInput};
use crate::layout_state::{LayoutState, LineData, StaticPositionAlignment, StaticPositionRect};
use crate::style_facts::StyleValues;
use crate::used_values::{FfiCssPixelPoint, UsedValuesCore};
use crate::{
    ffi_stats::FfiOp,
    ffi_stats::bump,
    formatting_context::{FcFrame, FfiLayoutFcCallbacks, block::BlockFormattingContext, sizing::SizingContext},
};
...
#[path = "line_box_fragment.rs"]
pub(crate) mod fragment;
#[path = "inline_level_iterator.rs"]
...
pub(crate) mod pieces {
    ...
    use crate::css_pixels::CssPixels;
    use crate::formatting_context::inline::{InlineFormattingContext, Node};
...
pub(crate) struct InlineFormattingContext<'context, 'pass> {
    ...
    pub(crate) containing_used_values: &'pass UsedValuesCore,
...
impl<'context, 'pass> InlineFormattingContext<'context, 'pass> {
    pub(crate) fn new_with_rust_parent(
        ...
        let containing_facts = state.box_facts(&callbacks, containing_block);
        assert!(containing_facts.has_layout_index);
        let containing_used_values = state.used_values(&callbacks, containing_block);
        let line_data = state.line_data_cell(containing_facts.layout_index);
        Self {
    ...
    pub(crate) fn facts(&self, node: Node) -> BoxFactsView<'pass> {
    ...
    pub(crate) fn containing_used(&self) -> &'pass UsedValuesCore {
        self.containing_used_values
    }

    pub(crate) fn used(&self, node: Node) -> &'pass UsedValuesCore {
        self.state.used_values(&self.callbacks, node)
    }

    pub(crate) fn used_mut(&self, node: Node) -> &'pass UsedValuesCore {
        self.state.used_values(&self.callbacks, node)
    }

    pub(crate) fn try_used_pointer(&self, node: Node) -> Option<&'pass UsedValuesCore> {
    ...
    fn layout_inside(&mut self, node: Node, available_space: AvailableSpace) -> bool {
    ...
    fn finish_inside_layout(&mut self, node: Node) {
        super::finish_child_layout(&mut self.frame.borrow_mut(), node);
    }
    ...
    pub(crate) fn dimension_box_on_line(&mut self, node: Node) {
        ...
        if facts.is_fragmented_inline {
            ...
            unsafe {
                (self.callbacks.report_unexpected_fragmented_inline)(self.callbacks.context, node);
            }
            return;
        }
    ...
    fn clear_floating_boxes(&self, node: Node) -> bool {
        self.parent
            .clear_floating_boxes(node, Some(self), self.input.content_box_position_in_bfc_root)
    }
    ...
    pub(crate) fn generate_line_boxes(&mut self) {
        ...
        let mut iterator = iterator::InlineLevelIterator::new(self);
        self.fragmented_inlines_in_pre_order = iterator.take_visited_fragmented_inlines();
        let mut line_builder = line_builder::LineBuilder::new(self);
        ...
        if self.text_overflow_applies() {
            ellipsis::apply(self.line_data_mut().line_boxes.as_mut_slice(), self);
        }
...
impl line_box::LineBoxTextProvider for InlineFormattingContext<'_, '_> {
...
impl ellipsis::EllipsisFontProvider for InlineFormattingContext<'_, '_> {
    fn font_glyph_width(&self, font: *const c_void, code_point: u32) -> f32 {
...
pub struct FfiCommittedFragment {
    ...
    pub glyphs: *const iterator::text::FfiDrawGlyph,
...
pub(crate) fn push_line_data(
    state: &crate::layout_state::LayoutState,
    layout_index: u32,
    ...
    let Some(mut data) = state.line_data_mut_if_present(layout_index) else {
        return false;
    };
