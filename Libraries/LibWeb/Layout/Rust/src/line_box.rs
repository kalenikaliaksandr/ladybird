...
impl LineBoxData {
    ...
    pub(crate) fn add_static_position_marker(&mut self, box_: *mut c_void, preceded_by_inline_box_start_edges: bool) {
    ...
    fn calculate_or_trim_trailing_whitespace(
        &mut self,
        provider: &mut impl LineBoxTextProvider,
        should_remove: bool,
    ) -> CssPixels {
        ...
        while last_text_length != 0 {
            ...
            if last_character > 0x7f || !(last_character as u8).is_ascii_whitespace() {
                break;
            }
    ...
    pub(crate) fn trailing_whitespace_inline_size(&mut self, provider: &mut impl LineBoxTextProvider) -> CssPixels {
        self.calculate_or_trim_trailing_whitespace(provider, false)
    }

    pub(crate) fn trim_trailing_whitespace(&mut self, provider: &mut impl LineBoxTextProvider) {
