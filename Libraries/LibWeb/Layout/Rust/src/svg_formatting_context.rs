...
use super::{FcFrame, FfiLayoutFcCallbacks};
...
impl<'pass> SvgFormattingContext<'pass> {
    ...
    fn first_child(&self, node: *mut c_void) -> *mut c_void {
        self.callbacks.first_child_without_counter(node)
    }

    fn next_sibling(&self, node: *mut c_void) -> *mut c_void {
        self.callbacks.next_sibling_without_counter(node)
    }

    fn parent(&self, node: *mut c_void) -> *mut c_void {
        self.callbacks.parent_without_counter(node)
    }
    ...
    fn style_facts(&self, node: *mut c_void) -> StyleValues<'_> {
        self.state.style_facts(&self.callbacks, node)
    }

    fn used_values(&self, node: *mut c_void) -> &'pass UsedValuesCore {
        self.state.used_values(&self.callbacks, node)
    }

    fn create_used_values(&self, node: *mut c_void) -> &'pass UsedValuesCore {
    ...
    fn computed_transforms(&self, node: *mut c_void) -> Option<FfiSvgComputedTransforms> {
        let index = self.state.box_facts(&self.callbacks, node).layout_index;
    ...
    fn first_child_matching(
        ...
        self.for_each_child(node, |child| {
            if result.is_none()
                && self.state.box_facts(&self.callbacks, child).is_box
    ...
    pub(super) fn run(&mut self, frame: &mut FcFrame, input: LayoutInput) {
        ...
        if self.state.box_facts(&self.callbacks, self.box_).is_svg_svg_box
        ...
        while !child.is_null() {
            let next = self.next_sibling(child);
            if self.state.box_facts(&self.callbacks, child).is_box {
    ...
    fn layout_svg_element(
        ...
        if facts.is_fit_to_view_box {
        ...
        } else if facts.is_foreign_object_box && self.state.box_facts(&self.callbacks, child).is_block_container {
            ...
            assert!(matches!(
                super::layout_inside_child(frame, None, None, child, self.layout_mode, child_input, true,),
                super::ChildLayoutOutcome::Created(_)
            ));
            ...
            super::discard_child_layout(frame, child);
    ...
    fn layout_nested_viewport(
        ...
        if self.state.box_facts(&self.callbacks, viewport).is_svg_svg_box
    ...
    fn layout_graphics_element(
        ...
        while !child.is_null() {
            let next = self.next_sibling(child);
            if self.state.box_facts(&self.callbacks, child).is_box && self.svg_facts(child).is_pattern_box {
    ...
    fn layout_path_like_element(&mut self, frame: &mut FcFrame, graphics_box: *mut c_void, input: LayoutInput) {
        let transforms = self
    ...
    fn layout_mask_or_clip(&mut self, frame: &mut FcFrame, resource: *mut c_void) {
