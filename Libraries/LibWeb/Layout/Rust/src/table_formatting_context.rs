...
mod grid {
    ...
    fn matching_children<'pass, T: TableTree<'pass>>(
        tree: &T,
        parent: Node,
        predicate: impl Fn(crate::box_facts::BoxFactsView<'pass>) -> bool,
    ) -> Vec<Node> {
...
use super::{FcFrame, FfiBorderData, FfiChildLayoutResult, FfiLayoutFcCallbacks, MeasuredCellContent, block};
...
use crate::geometry::{AvailableSize, AvailableSizeType, AvailableSpace, ContainingBlockConstraints, LayoutInput};
...
pub(crate) trait TableTree<'pass> {
    ...
    fn box_facts(&self, node: Node) -> BoxFactsView<'pass>;
    fn style_facts(&self, node: Node) -> StyleValues<'pass>;
    fn table_facts(&self, node: Node) -> super::FfiTableBoxFacts;
}
...
impl<'pass> TableFormattingContext<'pass> {
    pub(super) fn new(frame: &FcFrame<'pass>, pending_table_offset: Option<crate::geometry::LogicalOffset>) -> Self {
        Self {
            state: frame.state,
    ...
    fn matching_children(&mut self, parent: Node, predicate: impl Fn(BoxFactsView<'pass>) -> bool) -> Vec<Node> {
        let mut children = Vec::new();
    ...
    fn used_values(&self, node: Node) -> &'pass UsedValuesCore {
    ...
    fn set_cell_coordinates(&self, cell: Cell) {
        bump(FfiOp::SetTableCellCoordinates);
        self.state
    ...
    fn layout_inside_cell(
        ...
        parent: Option<&block::BlockFormattingContext<'pass>>,
        cell: Cell,
        ...
        match super::layout_inside_child(frame, None, None, cell.box_, self.layout_mode, layout_input, false) {
    ...
    fn finish_child_layout(&self, frame: &mut FcFrame<'pass>, cell: Node) {
        super::finish_child_layout(frame, cell);
    }
    ...
    fn measure_cell(&self, cell: Cell, used: &UsedValuesCore, inner: AvailableSpace) -> Option<MeasuredCellContent> {
        ...
        let facts = self.state.box_facts(&self.callbacks, cell.box_);
    ...
    fn compute_table_block_size(
        ...
        for cell_index in 0..self.cells.len() {
            ...
            } else if let Some(result) = self.layout_inside_cell(frame, parent, cell, inner) {
                used.set_content_block_size(result.automatic_content_block_size);
                self.finish_child_layout(frame, cell.box_);
            }
        ...
        for cell_index in 0..self.cells.len() {
            ...
            if self.layout_inside_cell(frame, parent, cell, inner).is_some() {
                self.finish_child_layout(frame, cell.box_);
            }
    ...
    fn run_caption_layout(
        ...
        for caption in self.matching_children(self.table_box, |facts| facts.is_table_caption) {
            ...
            let result = if let Some(parent) = parent {
                ...
                block::BlockFormattingContext::new(self.state, self.table_box, self.layout_mode, self.callbacks)
    ...
    pub(super) fn run(
        ...
        let mut captions = self.run_caption_layout(frame, parent, CAPTION_SIDE_TOP, caption_available);
        ...
        captions += self.run_caption_layout(frame, parent, CAPTION_SIDE_BOTTOM, caption_available);
    ...
    pub(super) fn pending_table_offset(&self) -> crate::geometry::LogicalOffset {
        self.pending_table_offset
    }

    pub(super) fn should_publish_pending_table_offset(&self) -> bool {
        self.should_publish_pending_table_offset
    }
}
