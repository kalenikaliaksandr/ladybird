...
void SVGFormattingContext::run(LayoutInput const& layout_input)
{
    ...
    // A root materialized from its paintable (partial SVG relayout) already carries the
    // final size this branch would derive, and its position.
    if (document.document_element() == context_box().dom_node() && !document.is_decoded_svg() && !svg_box_state.is_placed()) {
    ...
    // NOTE: The root box's own offset is neither read nor written in here. A root whose
    //       geometry must be mapped through a non-identity parent viewBox transform (a nested
    //       viewport without its own viewBox, or a mask/clip/pattern box) is positioned by the
    //       SVG-internal caller that created us, after this run() returns; every other caller
    //       passes an identity transform and positions the root itself.
...
void SVGFormattingContext::layout_svg_element(Box const& child, LayoutInput const& layout_input, Gfx::AffineTransform const& parent_svg_transform)
{
    if (is<SVG::SVGFitToViewBox>(child.dom_node())) {
        ...
        // Placed after the inside layout so placement is the box's last geometry write, and
        // before mask/clip layout, which reads this offset for objectBoundingBox units.
        // Placement also finalizes the box's floats and absolutely positioned children.
...
void SVGFormattingContext::layout_nested_viewport(Box const& viewport, Gfx::AffineTransform const& parent_svg_transform)
{
    ...
    if (!has_own_view_box) {
        // For a nested viewport without a viewBox, the geometry above is still in the outer
        // viewport's user units; its run() derives the viewport size and transforms from
        // those raw values, and the mapped geometry becomes final only afterwards.
        // FIXME: Avoid converting SVG box to floats.
        Gfx::FloatRect nested_rect {
            { nested_viewport_x.to_float(), nested_viewport_y.to_float() },
            { nested_viewport_width.to_float(), nested_viewport_height.to_float() }
...
void SVGFormattingContext::layout_mask_or_clip(SVGBox const& mask_or_clip)
{
    ...
    // The geometry above is in the pre-transform space the nested run() derives its viewport
    // size and transforms from; the box's final, mapped geometry is assigned afterwards.
    // FIXME: Avoid converting SVG box to floats.
