...
void SVGFormattingContext::run(LayoutInput const& layout_input)
{
    ...
    // NOTE: A root whose geometry must be mapped through a non-identity parent viewBox
    //       transform (a nested viewport without its own viewBox, or a mask/clip/pattern box)
    //       is placed by the SVG-internal caller that created us, after this run() returns;
    //       every other caller passes an identity transform and places the root itself.
    //       Nothing in this run() reads the root box's own offset — children are positioned
    //       through m_current_viewbox_transform, and the viewport size above is derived from
    //       the raw, unmapped content size.
