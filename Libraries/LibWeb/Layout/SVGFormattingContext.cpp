...
{
    auto& graphics_box_state = m_state.create(graphics_box, layout_input.percentage_basis_width, layout_input.percentage_basis_height);
    auto svg_transform = parent_svg_transform;
...
void SVGFormattingContext::layout_mask_or_clip(SVGBox const& mask_or_clip)
{
    ...
    // Masks and clips are laid out once per referencing element, since their viewport depends
    // on the element that references them, so their used values may already exist here.
    // FIXME: Lay out masks and clips into per-reference state instead of overwriting shared values.
    auto* existing_layout_state = m_state.try_get_mutable(mask_or_clip);
    auto& layout_state = existing_layout_state ? *existing_layout_state : m_state.create(mask_or_clip);
    auto parent_viewbox_transform = m_current_viewbox_transform;
