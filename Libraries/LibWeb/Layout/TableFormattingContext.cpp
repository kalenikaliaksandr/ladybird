    ...
    for (auto child = table_box().first_child(); child; child = child->next_sibling()) {
        ...
        if (auto caption_context = create_independent_formatting_context_if_needed(m_state, m_layout_mode, child_box)) {
            ...
            if (block_context) {
                ...
                // The caption is the root of block_context, so it sits at that context's origin.
                block_context->compute_width(child_box, caption_available_space, caption_constraints, { 0, 0 });
                inner_available_space = m_state.get(child_box).available_inner_space_or_constraints_from(caption_available_space);

                // The caption's final offset (in table wrapper coordinates) is already known
        ...
        if (phase == CSS::CaptionSide::Top) {
            // The table box sits below the top captions.
            ...
            // A caption whose principal box is not a block container (e.g. an SVG root) has
            // its box model metrics resolved by its inside layout, so it can only be placed
            // after that ran, with the same border-box-aligned inline position as the
            // block-container branch above.
...
void TableFormattingContext::position_row_boxes()
{
    ...
    // NOTE: Rows and row groups have the table wrapper as their containing block; the table
    //       box is not positioned until this run() returns, so they are positioned against
    //       its pending offset in the wrapper.
