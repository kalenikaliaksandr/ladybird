...
void TreeBuilder::update_layout_tree(DOM::Node& dom_node, TreeBuilder::Context& context, MustCreateSubtree must_create_subtree)
{
    ...
    if (may_replace_existing_layout_node && !m_current_rebuild_root) {
        ...
        // A fresh subtree that is not replacing a box in place - a ShadowRoot direct child or
        // a top-layer element revealed from display:none marks itself, so no ancestor rebuild
        // covers it. Nothing would lay the new boxes out on the partial path, so report it as
        // escaping the rebuild roots.
        m_layout_tree_update_escaped_rebuild_roots = true;
    }
    ...
    if (dom_node.is_document()) {
        ...
        if (may_replace_existing_layout_node) {
            ...
            // references at the replacement so the paint tree has no dead references until the
            // next layout commit rebuilds the lists; a subtree relayout that skips the containing
            // block never rebuilds them at all.
