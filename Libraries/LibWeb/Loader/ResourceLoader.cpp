...
template<typename LoadCounterChangeCallback, typename FileHandler, typename ErrorHandler>
static void request_file_for_load(LoadRequest& request, int& pending_loads, LoadCounterChangeCallback& on_load_counter_change, FileHandler on_file, ErrorHandler on_error)
{
    ...
    FileRequest file_request(url.file_path(), [request, &pending_loads, &on_load_counter_change, on_file, on_error, url](ErrorOr<i32> file_or_error) mutable {
        --pending_loads;
...
static void request_about_for_load(GC::Heap& heap, LoadRequest const& request, Callback callback)
{
    auto const& url = request.url().value();
...
static void request_resource_for_load(LoadRequest const& request, ResourceHandler on_resource, ErrorHandler on_error)
{
    auto const& url = request.url().value();
...
void ResourceLoader::load(LoadRequest& request, GC::Root<SuccessCallback> success_callback, GC::Root<ErrorCallback> error_callback, Optional<u32> timeout, GC::Root<TimeoutCallback> timeout_callback)
{
    ...
    if (url.scheme() == "about") {
        request_about_for_load(m_heap, request, [success_callback, request](ReadonlyBytes data, Requests::RequestTimingInfo const& timing_info, HTTP::HeaderMap const& response_headers) {
            log_success(request);
...
void ResourceLoader::load_unbuffered(LoadRequest& request, GC::Root<OnHeadersReceived> on_headers_received, GC::Root<OnDataReceived> on_data_received, GC::Root<OnComplete> on_complete)
{
    ...
    if (url.scheme() == "about"sv) {
        request_about_for_load(m_heap, request, [on_headers_received, on_data_received, on_complete, request](ReadonlyBytes data, Requests::RequestTimingInfo const& timing_info, HTTP::HeaderMap const& response_headers) {
            log_success(request);
