...
void paint_background(DisplayListRecordingContext& context, PaintableBox const& paintable_box, CSS::ImageRendering image_rendering, ResolvedBackground resolved_background, BorderRadiiData const& border_radii)
{
    ...
    for (auto& layer : resolved_background.layers.in_reverse()) {
        ...
        if (is<CSS::ImageStyleValue>(image) && (repeat_x || repeat_y) && !repeat_x_has_gap && !repeat_y_has_gap) {
            ...
            if (auto bitmap_promise = image_style_value.current_frame_bitmap(dest_rect); bitmap_promise && bitmap_promise->is_resolved()) {
