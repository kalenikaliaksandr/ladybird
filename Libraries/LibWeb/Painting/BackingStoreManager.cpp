...
void BackingStoreManager::resize_backing_stores_if_needed(WindowResizingInProgress window_resize_in_progress)
{
    ...
    // For iframe navigables the backing store's size directly determines the bitmap dimensions
    // read by DrawExternalContent in the parent's display list, so we must size it exactly to
    // the viewport. Padding only helps the top-level traversable avoid churn during window
    // resizes where the UI process crops to the viewport anyway.
