...
Layout::Node const* nearest_svg_viewport_of(Layout::Node const& layout_node)
{
    for (auto const* ancestor = layout_node.parent(); ancestor; ancestor = ancestor->parent()) {
        if (has_committed_box(*ancestor) && svg_viewport_transform(*ancestor).has_value())
            return ancestor;
    }
    return nullptr;
}
...
static Gfx::FloatRect svg_svg_box_view_box_or_viewport_rect(Layout::Box const& svg_svg_box)
{
    ...
    if (has_committed_box(svg_svg_box)) {
        auto viewport_size = svg_viewport_size(svg_svg_box);
...
{
    return Layout::RustFFI::layout_arena_paintable_slot_for_node(node.arena_handle(), Layout::Node::slot_id(&node));
}

Layout::RustFFI::PaintableData const* committed_row(Layout::Node const& node)
{
    auto slot = committed_row_slot(node);
    if (slot.index == Layout::RustFFI::PaintableSlotId_INVALID.index)
        return nullptr;
    return Layout::RustFFI::layout_arena_paintable_row(node.arena_handle(), slot);
}

bool has_committed_box(Layout::Node const& node)
{
    return committed_row_slot(node).index != Layout::RustFFI::PaintableSlotId_INVALID.index;
}
...
{
    return committed_row(node) && as<Layout::NodeWithStyle>(node).has_css_transform();
}
...
{
    if (!committed_row(node) || !is_positioned(node))
        return {};
...
{
    auto const* row = committed_row(block);
    if (!row)
        return {};
...
{
    auto const* row = committed_row(block);
    if (!row || !should_paint_cursor(block))
        return {};
    auto const& styled_block = as<Layout::NodeWithStyle>(block);
...
{
    if (!committed_row(node) || has_content(node))
        return {};
    auto const& styled_node = as<Layout::NodeWithStyle>(node);
...
{
    auto const* row = committed_row(containing_block);
    if (!row || !is_paintable_with_lines(containing_block))
        return;
