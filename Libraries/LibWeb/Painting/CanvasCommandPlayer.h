...
class WEB_API CanvasCommandPlayer {
    AK_MAKE_NONCOPYABLE(CanvasCommandPlayer);
    AK_MAKE_NONMOVABLE(CanvasCommandPlayer);

public:
    explicit CanvasCommandPlayer(RefPtr<Gfx::SkiaBackendContext>);
    ~CanvasCommandPlayer();
...
private:
    ...
    void play_command(CanvasCommands::DrawCanvasContext const&);

    NonnullRefPtr<Gfx::PaintStyle> resolve_paint_style(CanvasPaintStyle const&) const;
    DisplayListResourceStorage const& resource_storage() const { return *m_current_resource_storage; }
