...
{
    if (m_slot.index == Layout::RustFFI::PaintableSlotId_INVALID.index)
        return nullptr;
    return static_cast<Layout::Node*>(Layout::RustFFI::layout_arena_paintable_event_dispatch_node_shell(m_arena->handle(), m_slot));
}

RefPtr<Paintable> ChromeWidget::paintable() const
{
    auto* node = layout_node();
    return node ? node->paintable() : nullptr;
}
...
{
    auto const* paintable = node.paintable_ptr();
    if (!paintable || !has_resizer(node))
        return {};
    auto padding_rect = paintable->absolute_padding_box_rect();
...
bool resizer_contains(Layout::Node const& node, CSSPixelPoint adjusted_position, ChromeMetrics const& metrics)
{
    ...
    handle_rect->inflate(0, bottom_left_resizer ? 0 : paintable->box_model().border.right, paintable->box_model().border.bottom, bottom_left_resizer ? paintable->box_model().border.left : 0);
...
Optional<CSSPixelRect> absolute_scrollbar_rect(Layout::Node const& node, ScrollDirection direction, bool with_gutter, ChromeMetrics const& metrics)
{
    ...
    CSSPixelRect scrollbar_rect = paintable->absolute_padding_box_rect();
...
Optional<ScrollbarData> compute_scrollbar_data(Layout::Node const& node, ScrollDirection direction, ChromeMetrics const& metrics, ScrollStateSnapshot const* scroll_state_snapshot, ScrollbarSizing scrollbar_sizing)
{
    ...
    auto scrollable_overflow_rect = paintable->scrollable_overflow_rect();
    ...
    CSSPixels scrollport_size = paintable->absolute_padding_box_rect().primary_size_for_orientation(orientation);
    ...
    if (scroll_state_snapshot) {
        auto own_offset = scroll_state_snapshot->device_offset_for_index(paintable->own_scroll_node_index());
        auto device_scroll_offset = is_horizontal ? -own_offset.x() : -own_offset.y();
        auto device_pixels_per_css_pixel = static_cast<float>(paintable->document().page().client().device_pixels_per_css_pixel());
