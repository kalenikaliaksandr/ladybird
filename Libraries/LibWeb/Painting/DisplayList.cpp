...
void DisplayList::append(DisplayListCommand&& command, RefPtr<EffectiveRenderState const> context)
{
...
static RefPtr<EffectiveRenderState const> find_common_ancestor(RefPtr<EffectiveRenderState const> a, RefPtr<EffectiveRenderState const> b)
{
    if (!a || !b)
        return {};
...
void DisplayListPlayer::execute_impl(DisplayList& display_list, ScrollStateSnapshot const& scroll_state, RefPtr<Gfx::PaintingSurface> surface)
{
    ...
    auto for_each_node_from_common_ancestor_to_target = [](this auto const& self, RefPtr<EffectiveRenderState const> common_ancestor, RefPtr<EffectiveRenderState const> node, auto&& callback) -> void {
    ...
    auto apply_effective_render_state = [&](EffectiveRenderState const& node) {
        save({});
        ...
        if (auto const& perspective = node.perspective(); perspective.has_value()) {
            auto matrix = scale_matrix_translation(perspective.value(), static_cast<float>(device_pixels_per_css_pixel));
        ...
        if (auto transform_frame = node.transform_frame(); transform_frame) {
            ...
            auto matrix = scale_matrix_translation(transform_frame->matrix(), static_cast<float>(device_pixels_per_css_pixel));
    ...
    RefPtr<EffectiveRenderState const> applied_context;
    size_t applied_depth = 0;

    auto switch_to_context = [&](RefPtr<EffectiveRenderState const> target_context) {
        ...
        for_each_node_from_common_ancestor_to_target(common_ancestor, target_context, [&](EffectiveRenderState const& node) {
            apply_effective_render_state(node);
            applied_depth++;
        });
