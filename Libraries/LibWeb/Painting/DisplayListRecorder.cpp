...
void DisplayListRecorder::replay_cached_commands(ReadonlySpan<DisplayListCommand> commands)
{
    for (auto const& cmd : commands) {
        auto cmd_copy = cmd;
        m_save_nesting_level += cmd_copy.visit([](auto const& c) -> int {
            if constexpr (requires { c.nesting_level_change; })
                return c.nesting_level_change;
            return 0;
        });
        m_display_list.append(move(cmd_copy), m_accumulated_visual_context);
    }
}
