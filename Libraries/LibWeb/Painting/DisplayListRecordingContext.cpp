...
{
    if (m_nested_visual_context_assignments.has_value()) [[unlikely]] {
        if (auto assignment = m_nested_visual_context_assignments->find(&paintable); assignment != m_nested_visual_context_assignments->end())
            return assignment->value.context;
        return Painting::VISUAL_VIEWPORT_NODE_INDEX;
    }
