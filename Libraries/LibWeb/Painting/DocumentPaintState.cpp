...
GC::Ptr<Selection::Selection> DocumentPaintState::selection(DOM::Document const& document) const
{
...
{
    Layout::RustFFI::layout_arena_selection_clear(m_layout_node_arena->handle(), viewport_paintable_slot(document));
}
...
void DocumentPaintState::recompute_selection_states(DOM::Document& document, DOM::Range& range)
{
    ...
    auto apply_entries = [&] {
        Layout::RustFFI::layout_arena_selection_apply(m_layout_node_arena->handle(), viewport_paintable_slot(document), entries.data(), entries.size(), range.start_offset(), range.end_offset());
    };
