...
NonnullRefPtr<HitTestDisplayList> HitTestDisplayList::create_from_rust_recording(u64 visual_context_tree_version, Layout::NodeArena& arena, ChromeWidgetRegistry& chrome_widget_registry)
{
    ...
    for (auto const& exported : exported_items) {
        ...
        case ChromeWidgetKind::ResizeHandle:
            (void)chrome_widget_registry.get_or_create_resize_handle(arena, paintable.rust_slot());
            break;
        case ChromeWidgetKind::HorizontalScrollbar:
            (void)chrome_widget_registry.get_or_create_scrollbar(arena, paintable.rust_slot(), ScrollDirection::Horizontal);
            break;
        case ChromeWidgetKind::VerticalScrollbar:
            (void)chrome_widget_registry.get_or_create_scrollbar(arena, paintable.rust_slot(), ScrollDirection::Vertical);
            break;
        }
...
RefPtr<ChromeWidget> HitTestDisplayList::chrome_widget_for_item(Item const& item) const
{
    ...
    case ChromeWidgetKind::None:
        return nullptr;
    case ChromeWidgetKind::ResizeHandle:
        return m_chrome_widget_registry->resize_handle(item.paintable->rust_slot());
    case ChromeWidgetKind::HorizontalScrollbar:
        return m_chrome_widget_registry->scrollbar(item.paintable->rust_slot(), ScrollDirection::Horizontal);
    case ChromeWidgetKind::VerticalScrollbar:
        return m_chrome_widget_registry->scrollbar(item.paintable->rust_slot(), ScrollDirection::Vertical);
    }
    VERIFY_NOT_REACHED();
}
