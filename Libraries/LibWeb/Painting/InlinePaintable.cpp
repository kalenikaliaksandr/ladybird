...
void InlinePaintable::paint(DisplayListRecordingContext& context, PaintPhase phase) const
{
    ...
    if (phase == PaintPhase::Background && is_visible()) {
        ...
        // If the body's background properties were propagated to the root element, do not
        // re-paint the body's background.
        bool background_is_propagated_to_root = layout_node_with_style_and_box_metrics().is_body()
    ...
    if (phase == PaintPhase::Foreground) {
        ...
        // block has already painted them. This box being visibility: hidden does not gate
        // them: descendants may set visibility: visible again, so each fragment is filtered
        // by its own node's visibility.
        if (is_self_painting()) {
...
void InlinePaintable::record_hit_test_items(DisplayListRecordingContext& context, PaintPhase phase) const
{
    ...
    // Only this box's own items are gated on its visibility; the fragments it owns may
    // belong to descendants that override visibility or pointer-events, so they are
    // filtered per fragment instead.
    bool box_is_hit_testable = is_visible() && visible_for_hit_testing();
    ...
    if (is_self_painting()) {
        ...
        auto append_fragments = [&](bool want_direct_content) {
            m_fragment_ownership_filter.for_each_owned_fragment_index(fragments.size(), [&](size_t index) {
                ...
                if (fragment.is_block_level_box())
                    return;
                bool is_direct_content = fragment.layout_node().nearest_fragmented_inline_ancestor() == &layout_node();
                if (is_direct_content != want_direct_content)
                    return;
        ...
        append_fragments(true);
        if (box_is_hit_testable && stacking_context())
            append_piece_boxes();
        append_fragments(false);
    }
}
