...
public:
    ...
    // Unions across pieces, relative to the content union's origin (this paintable's offset).
    ...
    // The fragments this box owns; assigned by the containing block's
    // PaintableWithLines::assign_fragment_ownership() and only meaningful while
    // is_self_painting().
