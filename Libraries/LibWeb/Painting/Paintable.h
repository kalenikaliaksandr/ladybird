...
public:
    ...
    // Most paintables contribute none and carry no allocation; a box that keeps contributing
    // reuses its list's buffer across reconciles. The tree identity tag recognizes lists orphaned
    // by a wholesale tree replacement, whose indices mean nothing anymore.
    void set_owned_visual_context_nodes(ReadonlySpan<VisualContextIndex> owned_nodes, u64 tree_identity)
    {
        m_owned_visual_context_nodes_tree_identity = tree_identity;
        if (owned_nodes.is_empty()) {
            m_owned_visual_context_nodes = nullptr;
        } else if (m_owned_visual_context_nodes) {
            m_owned_visual_context_nodes->clear_with_capacity();
            m_owned_visual_context_nodes->append(owned_nodes.data(), owned_nodes.size());
        } else {
            m_owned_visual_context_nodes = make<Vector<VisualContextIndex>>(owned_nodes);
        }
    }
    [[nodiscard]] u64 owned_visual_context_nodes_tree_identity() const { return m_owned_visual_context_nodes_tree_identity; }
