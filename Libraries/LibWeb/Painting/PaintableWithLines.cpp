...
void PaintableWithLines::reset_for_relayout()
{
    ...
    m_fragment_ownership_filter = { .include_everything = true };
...
void PaintableWithLines::assign_fragment_ownership()
{
    ...
    // Pieces are ordered by depth within a line, so ranges of boxes nested at different
    // depths can arrive out of order; iteration relies on sorted ranges.
...
void PaintableWithLines::paint(DisplayListRecordingContext& context, PaintPhase phase) const
{
    ...
    if (phase == PaintPhase::Foreground) {
        ...
        // visible again, so fragments are filtered by their own node's visibility instead.
...
void PaintableWithLines::paint_cursor(DisplayListRecordingContext& context, InlinePaintable const* owner) const
{
    ...
    if (!focused_text_control_is_editable) {
        // The editable element may sit anywhere between the cursor and this block (e.g. a
