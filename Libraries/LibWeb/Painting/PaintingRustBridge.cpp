...
static Layout::RustFFI::PaintableSlotId viewport_paintable_slot(DOM::Document const& document)
{
    auto const* viewport = document.unsafe_layout_node();
    VERIFY(viewport);
    return { Layout::Node::slot_id(viewport).index };
}

bool rust_assign_accumulated_visual_contexts(DOM::Document& document, bool forced_incompatible_rebuild)
{
    return Layout::RustFFI::layout_arena_assign_accumulated_visual_contexts(layout_arena_handle(document), viewport_paintable_slot(document), visual_context_host_callbacks(document), forced_incompatible_rebuild);
}
...
{
    Layout::RustFFI::layout_arena_build_stacking_context_tree(layout_arena_handle(document), viewport_paintable_slot(document));
}
...
Layout::RustFFI::FfiHitTestHostCallbacks hit_test_host_callbacks()
{
    return {
        ...
        .paintable_facts = [](void*, void* layout_node_shell) -> Layout::RustFFI::FfiHitTestPaintableFacts {
            ...
            facts.could_be_scrolled_horizontally = could_be_scrolled_by_wheel_event(layout_node, ScrollDirection::Horizontal);
            facts.could_be_scrolled_vertically = could_be_scrolled_by_wheel_event(layout_node, ScrollDirection::Vertical);
...
RefPtr<DisplayList> record_rust_display_list(DOM::Document& document, DisplayList const& placeholder_display_list, DisplayListResourceStorage& resource_storage, PaintCommandCacheMode cache_mode, HTML::PaintConfig const& config, InspectorOverlayInputs const& overlay_inputs)
{
    ...
    auto generation = Layout::RustFFI::layout_arena_record_display_list(arena, viewport_paintable_slot(document), hit_test_host_callbacks(), paint_host_callbacks(paint_host_context), visual_context_host_callbacks(document), inputs);
    if (generation == 0)
        return nullptr;
