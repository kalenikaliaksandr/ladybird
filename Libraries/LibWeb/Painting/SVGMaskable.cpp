...
static void build_nested_svg_visual_context_tree_for_subtree(AccumulatedVisualContextTree& visual_context_tree, NestedVisualContextAssignments& context_assignments, DevicePixelConverter const& converter, Paintable& paintable_box, VisualContextIndex inherited_state, float pixel_ratio)
{
    ...
    {
        auto& assignment = context_assignments.ensure(&paintable_box);
        for (auto const& mask_layer : paintable_box.mask_layer_presence(MaskLayerSet::SvgOnly)) {
            ...
            assignment.mask_nodes.append(own_state);
        }
        assignment.context = own_state;
    }

    paintable_box.for_each_child_of_type<Paintable>([&](Paintable& child) {
        build_nested_svg_visual_context_tree_for_subtree(visual_context_tree, context_assignments, converter, child, own_state, pixel_ratio);
        return IterationDecision::Continue;
    });
}

static AccumulatedVisualContextTree build_nested_svg_visual_context_tree(Paintable& root_paintable, Gfx::IntPoint content_offset, NestedVisualContextAssignments& context_assignments)
{
    ...
    build_nested_svg_visual_context_tree_for_subtree(visual_context_tree, context_assignments, converter, root_paintable, {}, pixel_ratio);
    ...
    bool is_clip_path)
{
    ...
    auto visual_context_tree = build_nested_svg_visual_context_tree(const_cast<Paintable&>(paintable), -mask_rect.location().to_type<int>(), context_assignments);
    ...
    paint_context.set_nested_visual_context_assignments(move(context_assignments));
