...
static Compositor::SnapAreaID snap_area_id_for(Layout::Node const& snap_area)
{
    auto* element = snap_area.generated_for_pseudo_element().has_value()
        ? snap_area.pseudo_element_generator().ptr()
        : as_if<DOM::Element>(snap_area.dom_node());
    return { element ? element->unique_id() : UniqueNodeID { 0 }, snap_area.generated_for_pseudo_element() };
}

static SnapAreaReference snap_area_reference_for(Compositor::SnapAreaID const& id)
{
    return { as_if<DOM::Element>(DOM::Node::from_unique_id(id.element_id)), id.pseudo_element };
}

SnappedAreas resolve_snapped_areas(Compositor::SnappedAreaIDs const& ids, DOM::Document const& document)
{
    auto resolve = [&](auto const& axis_ids) {
        Vector<SnapAreaReference> areas;
        for (auto const& id : axis_ids) {
            auto area = snap_area_reference_for(id);
            if (area.element && &area.element->document() == &document && area.element->is_connected())
                areas.append(move(area));
        }
        return areas;
    };
    return { resolve(ids.x), resolve(ids.y) };
}

static SnapDestination snap_destination_for(CSSPixelPoint destination, Optional<CSSPixels> x, Optional<CSSPixels> y, SnapAxes axes)
{
    return { .position = { x.value_or(destination.x()), y.value_or(destination.y()) },
        .snapped_x = x.has_value(),
        .snapped_y = y.has_value(),
        .evaluated_x = axes.x,
        .evaluated_y = axes.y };
}
...
static Optional<SnapCandidateCollection> collect_snap_position_candidates(Layout::Node const& snap_container, bool collect_x, bool collect_y)
{
    ...
    for_each_descendant_snap_area(snap_container, snap_container, [&](Layout::NodeWithStyle const& snap_area) {
        ...
        auto area_reference = snap_area_id_for(snap_area);
...
static Vector<SnapAreaReference> snap_area_references_at_offset(Vector<SnapPositionCandidate> const& candidates, CSSPixels offset, CSSPixels cross_axis_offset)
{
    Vector<SnapAreaReference> areas;
    for (auto const& id : Compositor::snap_areas_at_offset(candidates, offset, cross_axis_offset)) {
        auto area = snap_area_reference_for(id);
        if (area.element)
            areas.append(move(area));
    }
...
Optional<Compositor::SnapContainerData> collect_scroll_snap_data(Layout::Node const& container)
{
    auto axes = snap_axes_of_scroll_container(container);
    if (axes.is_empty())
        return {};
    auto collection = collect_snap_position_candidates(container, axes.x, axes.y);
    if (!collection.has_value())
        return {};
    return Compositor::SnapContainerData {
        .axes = axes,
        .strictness = collection->snap_type.strictness,
        .snapport_size = collection->snapport.size(),
        .min_scroll_offset = { collection->x_geometry.min_offset, collection->y_geometry.min_offset },
        .max_scroll_offset = { collection->x_geometry.max_offset, collection->y_geometry.max_offset },
        .candidates = move(collection->candidates),
    };
}
...
SnapDestination adjust_scroll_destination_for_snapping(Layout::Node const& container, CSSPixelPoint destination, SnapSelectionStrategy const& strategy)
{
    auto data = collect_scroll_snap_data(container);
    if (!data.has_value())
        return { destination };
    auto result = Compositor::select_snap_destination(*data, destination, strategy);
    return { result.position, result.snapped_x, result.snapped_y, result.evaluated_x, result.evaluated_y,
        resolve_snapped_areas(result.snapped_areas, container.document()) };
}
...
static SnappedAxisBoxes select_between_multiple_aligned_snap_areas(SnapAxisCandidates const& candidates, ResnapSelection const& selection, CSSPixelPoint scroll_position)
{
    ...
    auto aligned_boxes = [](Vector<SnapPositionCandidate> const& axis_candidates, Vector<SnapAreaReference> const& snapped_areas, CSSPixels cross_axis_offset) {
        ...
        for (size_t candidate_index = 0; candidate_index < axis_candidates.size(); ++candidate_index) {
            ...
            if (!candidate.area.is_valid() || !snapped_areas.contains_slow(snap_area_reference_for(candidate.area)))
                continue;
    ...
    auto remove_superseded_boxes = [&](Vector<size_t>& boxes, Vector<SnapPositionCandidate> const& axis_candidates) {
        auto keep_only_boxes_matching = [&](auto const& predicate) {
            if (!any_of(boxes, [&](size_t candidate_index) { return predicate(snap_area_reference_for(axis_candidates[candidate_index].area)); }))
                return false;
            boxes.remove_all_matching([&](size_t candidate_index) { return !predicate(snap_area_reference_for(axis_candidates[candidate_index].area)); });
            return true;
        };
        ...
        boxes.remove_all_matching([&](size_t candidate_index) {
            auto area = snap_area_reference_for(axis_candidates[candidate_index].area);
            if (area.pseudo_element.has_value())
                return false;
            return any_of(boxes_before_ancestor_removal, [&](size_t other_index) {
                auto other_area = snap_area_reference_for(axis_candidates[other_index].area);
                return area.element && other_area.element && other_area.element != area.element && area.element->is_inclusive_ancestor_of(*other_area.element);
            });
        });
    };
    ...
    auto boxes_contain_area = [](Vector<size_t> const& boxes, Vector<SnapPositionCandidate> const& axis_candidates, Compositor::SnapAreaID const& area) {
...
SnapDestination select_resnap_destination(Layout::Node const& snap_container, CSSPixelPoint current_offset, ResnapSelection const& selection)
{
    ...
    if (x_offset.has_value() && y_offset.has_value() && !chosen_offsets_are_mutually_visible(candidates, *x_offset, *y_offset)) {
        ...
        struct ResnapAxis {
            ...
            Optional<Compositor::SnapAreaID> area;
        ...
        auto area_contains_focus = [&](Optional<Compositor::SnapAreaID> const& area) {
            return area.has_value() && selection.focused_node && snap_area_contains_node(snap_area_reference_for(*area), *selection.focused_node);
        };
        auto area_contains_target = [&](Optional<Compositor::SnapAreaID> const& area) {
            return area.has_value() && selection.targeted_element && snap_area_contains_node(snap_area_reference_for(*area), *selection.targeted_element);
        };
    ...
    auto record_snapped_areas = [&](Optional<size_t> const& chosen_candidate, Vector<SnapPositionCandidate> const& axis_candidates, Vector<SnapAreaReference> const& previously_snapped_areas, CSSPixels axis_offset, CSSPixels axis_current_offset, CSSPixels cross_axis_offset) {
        auto areas = snap_area_references_at_offset(axis_candidates, axis_offset, cross_axis_offset);
        if (chosen_candidate.has_value() && axis_offset == axis_current_offset) {
            auto chosen_area = snap_area_reference_for(axis_candidates[*chosen_candidate].area);
