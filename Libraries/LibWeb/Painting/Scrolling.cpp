...
CSSPixelPoint minimum_scroll_offset(Layout::Node const& node)
{
    ...
    auto scrollport_rect = paintable->absolute_padding_box_rect();
    return {
...
CSSPixelRect scroll_snapport_rect(Layout::Node const& node)
{
    ...
    if (!paintable)
        return {};
    return scroll_snapport_rect(node, paintable->absolute_padding_box_rect());
}
...
ScrollHandled set_scroll_offset(Layout::Node& node, CSSPixelPoint offset)
{
    ...
    if (auto* paintable = node.paintable_ptr())
        paintable->set_needs_repaint(InvalidateDisplayList::No);
    return ScrollHandled::Yes;
}
...
void scroll_ancestor_to_offset_into_view(DOM::Node const& node, size_t offset)
{
