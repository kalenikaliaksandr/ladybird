...
class StackedRenderState : public AtomicRefCounted<StackedRenderState> {
public:
    static NonnullRefPtr<StackedRenderState> create(
        RefPtr<TransformFrame const> transform_frame,
        Optional<size_t> scroll_frame_id,
        RefPtr<ClipFrame const> clip_frame,
        RefPtr<StackedRenderState const> parent,
        Optional<Gfx::FloatMatrix4x4> perspective = {})
    {
        return adopt_ref(*new StackedRenderState(move(transform_frame), scroll_frame_id, move(clip_frame), move(parent), move(perspective)));
    }
    ...
    RefPtr<StackedRenderState const> parent() const { return m_parent; }
...
private:
    ...
    RefPtr<StackedRenderState const> m_parent;
    Optional<Gfx::FloatMatrix4x4> m_perspective;
    size_t m_depth;
};

// Key for StackedRenderState deduplication in the node cache.
// States with identical keys can be shared between different paintables.
struct StackedRenderStateKey {
    RefPtr<TransformFrame const> transform_frame;
    Optional<size_t> scroll_frame_id;
    RefPtr<ClipFrame const> clip_frame;
    RefPtr<StackedRenderState const> parent;

    bool operator==(StackedRenderStateKey const&) const = default;
};

}

template<>
struct AK::Traits<Web::Painting::StackedRenderStateKey> : public DefaultTraits<Web::Painting::StackedRenderStateKey> {
    static unsigned hash(Web::Painting::StackedRenderStateKey const& key)
    {
