...
static void paint_node(Paintable const& paintable, DisplayListRecordingContext& context, PaintPhase phase)
{
    ...
    paintable.before_paint(context);
    paintable.paint(context, phase);
    paintable.after_paint(context);
...
void StackingContext::paint(DisplayListRecordingContext& context) const
{
    ...
    bool needs_to_push_stacking_context = opacity != 1.0f
    ...
    context.display_list_recorder().set_context(paintable_box().stacked_render_state());
    ...
    auto context_before_children = context.display_list_recorder().context();
    ...
    context.display_list_recorder().set_context(context_before_children);
...
TraversalDecision StackingContext::hit_test(CSSPixelPoint position, HitTestType type, Function<TraversalDecision(HitTestResult)> const& callback) const
{
    ...
    if (paintable_box().stacked_render_state()) {
        local_position = paintable_box().stacked_render_state()->transform_point_for_hit_test(position, scroll_state);
    } else {
