...
void ViewportPaintable::assign_effective_render_states()
{
    ...
    HashMap<EffectiveRenderStateKey, RefPtr<EffectiveRenderState const>> node_cache;
    ...
    for_each_in_subtree_of_type<PaintableBox>([&](auto& paintable_box) {
        ...
        if (paintable_box.is_fixed_position()) {
            auto const* containing_block = paintable_box.containing_block();
            // If containing block is the viewport (no transform trapping), prevent scroll
            if (containing_block && is<ViewportPaintable>(*containing_block))
        ...
        RefPtr<EffectiveRenderState const> current_node;
        ...
        for (size_t i = visual_ancestors.size(); i > 0; --i) {
            ...
            if (perspective.has_value()) {
                ...
                current_node = EffectiveRenderState::create({}, {}, {}, current_node, move(perspective));
            }
            ...
            if (transform || scroll_id.has_value() || clip) {
                EffectiveRenderStateKey key { .transform_frame = transform, .scroll_frame_id = scroll_id, .clip_frame = clip, .parent = current_node };
                ...
                if (auto cached = node_cache.get(key); cached.has_value()) {
                    ...
                    current_node = EffectiveRenderState::create(transform, scroll_id, clip, current_node);
                    node_cache.set(key, current_node);
                }
            }
        }

        paintable_box.set_effective_render_state(current_node);
        return TraversalDecision::Continue;
    });
}
