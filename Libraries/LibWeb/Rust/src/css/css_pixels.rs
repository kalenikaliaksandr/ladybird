...
impl CssPixelFraction {
    ...
    #[cfg(test)]
    pub(crate) fn numerator_for_test(self) -> CssPixels {
        self.numerator
    }
}
...
mod tests {
    ...
    #[test]
    fn fraction_comparison_and_multiplication_stay_deferred() {
        let ten_thirds = CssPixelFraction::ratio_of(CssPixels::from_integer(10), CssPixels::from_integer(3));
        assert_eq!(
            CssPixels::from_integer(3).mul_by_fraction(ten_thirds),
            CssPixels::from_integer(10)
        );
        assert!(ten_thirds.is_at_least_one());
        let two_thirds = CssPixelFraction::ratio_of(CssPixels::from_integer(2), CssPixels::from_integer(3));
        assert!(!two_thirds.is_at_least_one());
        assert!(CssPixelFraction::one().is_at_least_one());

        let three_fifths = CssPixelFraction::ratio_of(CssPixels::from_integer(3), CssPixels::from_integer(5));
        assert_eq!(
            two_thirds.min(three_fifths).numerator_for_test().raw_value(),
            CssPixels::from_integer(3).raw_value()
        );
        assert_eq!(
            CssPixels::from_raw(100).mul_by_fraction(two_thirds).raw_value(),
            (100i64 * 128 / 192) as i32
        );
    }
