...
impl<'pass> BlockFormattingContext<'pass> {
    ...
    fn register_contained_abspos_child(&self, node: Node, block_offset: CssPixels, birth_box: Node) {
    ...
    fn layout_inside(
        ...
        match crate::layout::layout_inside_child(
            ...
            crate::layout::ChildLayoutOutcome::Skipped(_) => None,
    ...
    fn layout_block_level_box(
        ...
        let child_layout = if has_independent_formatting_context {
            ...
            if container_facts.is_table_wrapper()
                && style.display().is_table_inside()
                && let Some(child_layout) = &child_layout
            {
                self.table_box_in_wrapper_border_box_block_size.set(Some(
                    child_layout
                        .geometry
                        .border_box_block_size(child_layout.geometry.uses_collapsing_borders_model),
                ));
            }
            child_layout
        } else {
