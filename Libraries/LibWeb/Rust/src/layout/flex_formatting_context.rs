...
struct FlexFormattingContext<'pass> {
    ...
    /// The container's own metrics as this run knows them: captured at run
    /// entry and refreshed when the parent dimensions the container, with
    /// the container self-sizing setters writing through. The record copy
    /// stays until fragments are built from placement inputs.
...
impl<'pass> FlexFormattingContext<'pass> {
    ...
    fn container_has_definite_main_size(&self) -> bool {
        let metrics = self.container_metrics.get();
        if self.main_axis_is_horizontal() {
            metrics.has_definite_inline_size()
        } else {
            metrics.has_definite_block_size()
    ...
    fn container_has_definite_cross_size(&self) -> bool {
        let metrics = self.container_metrics.get();
        if self.cross_axis_is_horizontal() {
            metrics.has_definite_inline_size()
        } else {
    ...
    fn container_inner_main_size(&self) -> CssPixels {
        let metrics = self.container_metrics.get();
        if self.main_axis_is_horizontal() {
            metrics.content_inline_size
        } else {
    ...
    fn container_inner_cross_size(&self) -> CssPixels {
        let metrics = self.container_metrics.get();
        if self.cross_axis_is_horizontal() {
            metrics.content_inline_size
        } else {
    ...
    fn used_flex_basis_for_item(&self, index: usize) -> UsedFlexBasis<'pass> {
        ...
        if value.is_percentage() && !self.container_has_definite_main_size() {
            return UsedFlexBasis::Content;
        }
    ...
    fn adjust_main_size_through_aspect_ratio(
        ...
        let reference = self.container_inner_cross_size();
    ...
    fn adjust_cross_size_through_aspect_ratio(
        ...
        let ratio = self.facts(node).preferred_aspect_ratio().unwrap();
        let reference = self.container_inner_main_size();
    ...
    fn determine_flex_base_size(&mut self, index: usize) {
        ...
        let basis_is_definite = match basis {
            ...
            UsedFlexBasis::Size { value, .. } => {
                if value.is_auto() || value.is_none() || value.is_intrinsic_sizing_constraint() {
                    ...
                    !value.contains_percentage() || self.container_has_definite_main_size()
                } else {
                    debug_assert!(value.is_percentage());
                    self.container_has_definite_main_size()
                }
            }
        };
        ...
        if facts.has_auto_content_aspect_ratio() {
            if !basis_is_definite
                ...
                && self.container_has_definite_main_size()
            {
                flex_base_size = self.container_inner_main_size();
            }
            let (min_cross, _) = self.computed_cross_min_size(node);
            let (max_cross, _) = self.computed_cross_max_size(node);
    ...
    fn main_gap(&self) -> CssPixels {
        ...
        gap.to_px(self.container_inner_main_size())
    }
    ...
    fn cross_gap(&self) -> CssPixels {
        ...
        gap.to_px(self.container_inner_cross_size())
    }
    ...
    fn resolve_flexible_lengths_for_line(&mut self, line_index: usize) {
            ...
            .is_intrinsic_sizing_constraint()
        {
            ...
            AvailableSize::definite(self.container_inner_main_size())
        };
        ...
        for item_position in 0..item_count {
            ...
            if self.container_has_definite_main_size()
    ...
    fn determine_hypothetical_cross_size_of_item(&mut self, index: usize) {
        ...
        if let Some(ratio) = self.facts(node).preferred_aspect_ratio() {
            ...
            if replaced_with_only_natural_ratio && !self.flex_items[index].used_flex_basis_is_definite {
                self.flex_items[index].hypothetical_cross_size =
                    css_clamp(self.container_inner_cross_size(), clamp_min, clamp_max);
                return;
            }
    ...
    fn calculate_cross_size_of_each_flex_line(&mut self) {
        ...
        if self.is_single_line() && self.container_has_definite_cross_size() {
            self.flex_lines[0].cross_size = self.container_inner_cross_size();
            return;
        }
    ...
    fn handle_align_content_stretch(&mut self) {
        ...
        if !self.container_has_definite_cross_size()
        ...
        let container_size = self.container_inner_cross_size();
            ...
            if auto_margins == 0 && number_of_items > 0 {
                match justify {
                    ...
                    justify_content::STRETCH | justify_content::NORMAL | justify_content::FLEX_START => {
                        if self.is_direction_reverse() {
                            initial_offset = self.container_inner_main_size();
                        }
                    }
                    justify_content::END => {
                        initial_offset = self.container_inner_main_size();
                    ...
                    justify_content::CENTER => {
                        initial_offset = (self.container_inner_main_size() - used_main_space) / 2;
                        if self.is_direction_reverse() {
                            initial_offset = self.container_inner_main_size() - initial_offset;
                        }
                    }
                    ...
                    justify_content::SPACE_AROUND => {
                        ...
                        initial_offset = if self.is_direction_reverse() {
                            self.container_inner_main_size() - space_between_items / 2
                        } else {
                    ...
                    justify_content::SPACE_EVENLY => {
                        ...
                        initial_offset = if self.is_direction_reverse() {
                            self.container_inner_main_size() - space_between_items
                        } else {
                            space_between_items
                        };
                    }
    ...
    fn align_all_flex_lines(&mut self) {
        ...
        let container_cross_size = self.container_inner_cross_size();
        if self.is_single_line() {
    ...
    fn layout_inside_item(&mut self, run: &FormattingContextRun<'pass>, index: usize) {
        ...
        let container_inline_size = self.container_metrics.get().content_inline_size;
        let container_block_size = self.container_metrics.get().content_block_size;
    ...
    fn calculate_static_position_rect(&self, node: Node) -> StaticPositionRect {
        ...
        let main_size = self.container_inner_main_size();
        let cross_size = self.container_inner_cross_size();
    ...
    fn cross_size_transferred_from_definite_main_size(&self, index: usize) -> Option<CssPixels> {
        ...
        let ratio = self.facts(node).preferred_aspect_ratio()?;
        let main_size = if self.container_has_definite_main_size() {
