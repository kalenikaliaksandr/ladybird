...
pub(crate) struct RootSizingDirectives {
    /// The complete pre-run root-record state the spawning side declares,
    /// replacing direct writes onto the record: the style baseline with the
    /// parent's decisions applied. Seeded onto the record at run entry;
    /// the input-purity probe holds declaring runs to exactly this state.
    pub(crate) declared_root_metrics: Option<BoxMetrics>,
