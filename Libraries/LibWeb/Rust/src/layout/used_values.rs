...
impl UsedValues {
    ...
    /// Seals the metrics a box's own formatting context run is responsible
    /// for, called when that run returns. The parent still owns the offset
    /// family (insets, content offset, line coordinate) up to placement,
    /// where seal_committed_box_metrics covers the rest.
...
/// One enumeration of every plain-value cell of a UsedValues record,
/// generating the snapshot struct the run cache keys and replays with: at
/// probe time a capture joins the cache key (container runs hand their
/// roots sizes, definiteness, and cell padding through the record rather
/// than through LayoutInput), and at body end a capture becomes the state
/// a hit replays into the freshly handed root record.
macro_rules! used_values_cell_state {
    ($($field:ident: $type:ty),+ $(,)?) => {
        ...
        impl UsedValuesCellState {
            ...
            pub(crate) fn replay_into(&self, used: &UsedValues) {
