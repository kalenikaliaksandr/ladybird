...
mod tests {
    ...
    fn px(value: f64) -> CssPixels {
        CssPixels::nearest_value_for(value)
    }

    #[test]
    fn rounded_rect_rounds_far_edges_not_sizes() {
        let converter = DevicePixelConverter::new(1.5);
        let rect = converter.rounded_device_rect(CssPixelRect {
            x: px(1.0),
            y: px(1.0),
            width: px(1.0),
            height: px(1.0),
    ...
    #[test]
    fn enclosing_rect_floors_origin_and_ceils_size() {
        let converter = DevicePixelConverter::new(2.0);
        let rect = converter.enclosing_device_rect(CssPixelRect {
            x: px(0.75),
            y: px(0.25),
            width: px(0.3),
            height: px(0.7),
