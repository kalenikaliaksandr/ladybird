...
#[derive(Clone, Copy)]
#[repr(C)]
pub struct FfiDumpCallbacks {
    pub context: *mut c_void,
    pub describe_layout_node: unsafe extern "C" fn(*mut c_void, *mut c_void, *mut c_void),
    pub describe_dom_node: unsafe extern "C" fn(*mut c_void, *mut c_void, *mut c_void) -> bool,
    pub append_output: unsafe extern "C" fn(*mut c_void, *const u8, usize),
}

struct CallbackDescriber<'a> {
    arena: &'a LayoutNodeArena,
    callbacks: FfiDumpCallbacks,
}

impl crate::painting::dump::LayoutNodeDescriber for CallbackDescriber<'_> {
    fn describe(&self, node: NodeSlotId) -> String {
        let shell = self.arena.shell_if_live(node);
        if shell.is_null() {
            return "(detached)".to_string();
        }
        let mut description = String::new();
        // SAFETY: describe_layout_node writes into the String handed as the output context via
        // layout_arena_dump_append_string below, synchronously.
        unsafe {
            (self.callbacks.describe_layout_node)(self.callbacks.context, shell, (&raw mut description).cast());
        }
        description
    }

    fn is_live(&self, node: NodeSlotId) -> bool {
        !self.arena.shell_if_live(node).is_null()
    }
    ...
    fn describe_dom_node(&self, node: NodeSlotId) -> Option<String> {
        ...
        let mut description = String::new();
        // SAFETY: describe_dom_node writes into the String handed as the output context via
        // layout_arena_dump_append_string, synchronously.
        let has_dom_node =
            unsafe { (self.callbacks.describe_dom_node)(self.callbacks.context, shell, (&raw mut description).cast()) };
        has_dom_node.then_some(description)
    }
}

/// # Safety
///
/// `output` must point to the live Rust String provided to a dump callback, and `bytes` must
/// remain readable for this call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn layout_arena_dump_append_string(output: *mut c_void, bytes: *const u8, length: usize) {
    abort_on_panic(|| {
        // SAFETY: `output` is the String pointer handed out by CallbackDescriber::describe.
        let output = unsafe { &mut *output.cast::<String>() };
        if length == 0 {
            return;
        }
        // SAFETY: The C++ caller passes a live UTF-8 buffer for this synchronous call.
...
/// `arena` must be a live handle from `layout_arena_create`, used on the document thread.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn layout_arena_dump_visual_context_tree(
    arena: *mut c_void,
    viewport: PaintableSlotId,
    callbacks: FfiDumpCallbacks,
) {
    abort_on_panic(|| {
        let arena = unsafe { arena_from_handle(arena) };
        let paintables = arena.paintables().borrow();
        let describer = CallbackDescriber { arena, callbacks };
        let text = if paintables.is_live(viewport) {
            crate::painting::dump::dump_visual_context_tree(
                &paintables.visual_context,
                &paintables,
                &describer,
                viewport,
            )
        ...
        // SAFETY: append_output copies the bytes synchronously.
        unsafe { (callbacks.append_output)(callbacks.context, text.as_ptr(), text.len()) };
    });
}
...
pub unsafe extern "C" fn ladybird_web_record_image_paint_display_list(
    ...
    abort_on_panic(|| {
        ...
        match inputs.kind {
            0 => recorder.draw_scaled_decoded_image_frame(
                dest_rect,
                None,
            ...
            1 => recorder.paint_nested_display_list(
            ...
            2 => recorder.fill_rect_with_linear_gradient(
                dest_int_rect,
                &LinearGradientData {
            ...
            3 => recorder.fill_rect_with_radial_gradient(
                dest_int_rect,
                &RadialGradientData {
                    color_stops: color_stops(),
                    interpolation_method,
            ...
            4 => recorder.fill_rect_with_conic_gradient(
                dest_int_rect,
                &ConicGradientData {
...
pub unsafe extern "C" fn layout_arena_hit_test_item_at_line_edge(
    ...
    abort_on_panic(|| {
        let position_type = if position_type == 1 {
            crate::painting::hit_test::caret::CaretPositionType::Before
        } else {
...
fn caret_mode(mode: u8) -> crate::painting::hit_test::caret::CaretPositionMode {
    match mode {
        1 => crate::painting::hit_test::caret::CaretPositionMode::SelectionStart,
        2 => crate::painting::hit_test::caret::CaretPositionMode::Selection,
        _ => crate::painting::hit_test::caret::CaretPositionMode::Normal,
    }
}
...
pub unsafe extern "C" fn layout_arena_hit_test_caret_item_for_line(
    ...
    abort_on_panic(|| {
        with_hit_test_list(arena, Default::default(), |list, _| {
            match list.caret_item_for_line(line_index, css_point(x_raw, y_raw), caret_mode(mode)) {
...
/// `arena` must be a live handle from `layout_arena_create`, used on the document thread.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn layout_arena_dump_hit_test_display_list(arena: *mut c_void, callbacks: FfiDumpCallbacks) {
    abort_on_panic(|| {
        let arena = unsafe { arena_from_handle(arena) };
        let mut paintables = arena.paintables().borrow_mut();
        let describer = CallbackDescriber { arena, callbacks };
        let text = {
            let crate::painting::paintable_arena::PaintableArena { hit_test_list, .. } = &mut *paintables;
            match hit_test_list.as_mut() {
                Some(list) => {
                    let list_ptr: *mut crate::painting::hit_test::HitTestList = list;
                    // SAFETY: `list` lives inside `paintables` and is disjoint from the arena data
                    // the dump reads; nothing else touches it during the call.
                    crate::painting::dump::dump_hit_test_list(unsafe { &mut *list_ptr }, &paintables, &describer)
                }
                None => "(no hit-test display list)\n".to_string(),
            }
        };
        // SAFETY: append_output copies the bytes synchronously.
        unsafe { (callbacks.append_output)(callbacks.context, text.as_ptr(), text.len()) };
