...
/// `arena` must be a live handle from `layout_arena_create`, used on the document thread.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn layout_arena_paintable_slot_for_node(
    arena: *mut c_void,
    layout_node: NodeSlotId,
) -> PaintableSlotId {
    abort_on_panic(|| {
        let arena = unsafe { arena_from_handle(arena) };
        arena.paintables().borrow().paintable_of_node(layout_node)
    })
}
...
/// `arena` must be a live handle from `layout_arena_create`, used on the document thread.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn layout_arena_paintable_row(arena: *mut c_void, slot: PaintableSlotId) -> *mut PaintableData {
    abort_on_panic(|| {
        let arena = unsafe { arena_from_handle(arena) };
...
pub unsafe extern "C" fn layout_arena_reset_visual_context_state(arena: *mut c_void) {
    abort_on_panic(|| {
        let arena = unsafe { arena_from_handle(arena) };
        arena.paintables().borrow_mut().reset_visual_context_state();
    });
}
