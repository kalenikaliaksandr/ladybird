...
mod tests {
    ...
    #[test]
    fn snapshots_and_retained_raw_pointers_do_not_alias() {
        let mut arena = PaintableArena::new();
        let allocation = arena.allocate(NodeSlotId::INVALID, std::ptr::null_mut());
        let original_pointer = allocation.data;
        let snapshot = arena.data_ref(allocation.slot);

        // Model a write through Paintable::m_rust_data after Rust has taken a snapshot.
        // SAFETY: The allocation remains live and the arena is used on this thread only.
        unsafe { (*original_pointer).selection_state = 3 };
        assert_eq!(snapshot.selection_state, 0);
        assert_eq!(arena.data_ref(allocation.slot).selection_state, 3);

        arena.update_data(allocation.slot, |data| data.flags = 0x1234);
        // SAFETY: The allocation remains live.
        unsafe {
            assert_eq!((*original_pointer).selection_state, 3);
            assert_eq!((*original_pointer).flags, 0x1234);
        }

        for _ in 0..PAINTABLE_SLOTS_PER_CHUNK {
            arena.allocate(NodeSlotId::INVALID, std::ptr::null_mut());
        }
        assert_eq!(arena.data_ptr(allocation.slot), original_pointer);
    }
}
