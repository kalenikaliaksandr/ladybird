...
fn linear_gradient_angle_degrees(
    ...
    if let Some(direction) = direction_value {
        ...
        let angle = match side_or_corner {
            0 => 0.0,
            1 => 180.0,
            2 => 270.0,
            3 => 90.0,
            4 => -corner_angle_degrees(),
            5 => corner_angle_degrees(),
            6 => corner_angle_degrees() + 180.0,
            7 => -(corner_angle_degrees() + 180.0),
...
mod tests {
    ...
    fn stop(color: Color, position: f32) -> GradientStop {
        GradientStop {
            color,
            position,
            transition_hint: None,
    ...
    #[test]
    fn repeat_expansion_covers_the_unit_range() {
        let red = Color::from_rgb(255, 0, 0);
        let blue = Color::from_rgb(0, 0, 255);
        let expanded = expand_repeat_length(&[stop(red, 0.25), stop(blue, 0.75)], 0.5);
        assert_eq!(
            expanded.iter().map(|stop| stop.position).collect::<Vec<_>>(),
            vec![0.0, 0.25, 0.25, 0.75, 0.75, 1.0]
        );
        assert_eq!(expanded.first().unwrap().color, blue.mixed_with(red, 0.5));
        assert_eq!(expanded.last().unwrap().color, red.mixed_with(blue, 0.5));
        assert_eq!(
            expanded.iter().map(|stop| stop.color).collect::<Vec<_>>(),
            vec![
                blue.mixed_with(red, 0.5),
                blue,
                red,
                blue,
                red,
                red.mixed_with(blue, 0.5)
            ]
        );
    }

    #[test]
    fn transition_hint_replacement_samples_five_extra_stops() {
        ...
        let stops = [
            ...
            GradientStop {
                color: white,
                position: 1.0,
                transition_hint: Some(0.5),
            },
        ];
        let replaced = replace_transition_hints_with_normal_color_stops(&stops);
        assert_eq!(replaced.len(), 7);
        assert!(replaced.iter().all(|stop| stop.transition_hint.is_none()));
        assert_eq!(replaced[3].position, 0.5);
        assert_eq!(replaced[3].color, black.mixed_with(white, 0.5));
    }

    #[test]
    fn centered_transition_hint_is_linear() {
        let previous = stop(Color::from_rgb(0, 0, 0), 0.0);
        let next = GradientStop {
            color: Color::from_rgb(255, 255, 255),
            position: 1.0,
            transition_hint: Some(0.5),
        };
        assert_eq!(color_stop_step(&previous, &next, 0.25), 0.25);
        assert_eq!(color_stop_step(&previous, &next, -1.0), 0.0);
        assert_eq!(color_stop_step(&previous, &next, 2.0), 1.0);
    }

    #[test]
    fn degenerate_repeating_gradient_averages_premultiplied() {
        let opaque_red = Color::from_rgb(255, 0, 0);
        let transparent = Color::TRANSPARENT;
        let average = average_color_for_degenerate_repeating_gradient(&[stop(opaque_red, 0.0), stop(transparent, 0.0)]);
        assert_eq!(average, Color::from_rgba(255, 0, 0, 128));
    }

    #[test]
    fn identical_adjacent_stops_deduplicate() {
        let red = Color::from_rgb(255, 0, 0);
        let stops = to_color_stops(&[stop(red, 0.0), stop(red, 0.0), stop(red, 1.0)], false, |position| {
            position
        });
        assert_eq!(stops.positions, vec![0.0, 1.0]);
    }
}
