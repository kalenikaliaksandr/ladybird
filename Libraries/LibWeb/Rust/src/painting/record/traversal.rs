    ...
    fn paint_scope(&mut self, scope: PaintScope) {
        ...
        self.splice_or_record_capture(site, |this| {
            ...
            if plan.establishes_stacking_context {
                ...
                this.with_context(context, |this| this.execute_paint_order(&plan));
            } else {
                this.execute_paint_order(&plan);
            }
        });
    }

    fn execute_paint_order(&mut self, plan: &PaintScopePlan) {
        for item in &plan.items {
            match *item {
                PaintOrderItem::Scope(scope) => self.paint_scope(scope),
                PaintOrderItem::Producer(site) => match site.producer {
                    PaintProducer::BoxPhase(phase) => self.paint_node(site.owner, phase),
                    PaintProducer::Svg(phase) => self.paint_svg(site.owner, phase),
                    PaintProducer::SvgBoxForeground => self.paint_svg_box(site.owner, PaintPhase::Foreground),
                },
            }
        }
    }
