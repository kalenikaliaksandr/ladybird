...
unsafe extern "C" {
    ...
    fn ladybird_gfx_filter_serialized_summary(filter: *const c_void, out_size: *mut usize, out_hash: *mut u64);
}
...
impl FilterHandle {
    ...
    pub fn serialized_summary(&self) -> (usize, u64) {
        let mut size = 0usize;
        let mut hash = 0u64;
        // SAFETY: adopt() took sole ownership of a live Gfx::Filter.
        unsafe { ladybird_gfx_filter_serialized_summary(self.raw, &raw mut size, &raw mut hash) };
        (size, hash)
    }
}
...
impl VisualContextData {
    fn kind_index(&self) -> u8 {
        match self {
            ...
            Self::BackfaceVisibility(_) => 4,
            Self::ClipPath(_) => 5,
            Self::Effects(_) => 6,
            Self::ScrollCompensation(_) => 7,
            Self::AnchorScrollShift(_) => 8,
            Self::Mask(_) => 9,
        }
    }
}
    ...
    pub fn is_compatible_with(&self, other: &Self) -> bool {
        ...
        self.nodes.iter().zip(&other.nodes).all(|(node, other_node)| {
            node.parent_index == other_node.parent_index
                ...
                && node.data.kind_index() == other_node.data.kind_index()
        })
    }
