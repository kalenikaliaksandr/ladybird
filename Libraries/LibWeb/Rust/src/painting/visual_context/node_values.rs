...
mod tests {
    ...
    #[test]
    fn translate_percentage_rounds_to_nearest_not_truncated() {
        let percentage = StyleValueData::Percentage { value: 50.0 };
        let handle = ComputedStyleValueHandle {
            pointer: std::ptr::from_ref(&percentage).cast(),
        };
        let reference = CssPixels::from_raw(107);
        assert_eq!(
            resolved_translate_axis_px(0.0, &handle, reference),
            CssPixels::from_raw(54).to_float()
        );
        std::mem::forget(handle);
    }

    #[test]
    fn translate_without_percentage_keeps_the_precomputed_px() {
        let empty = ComputedStyleValueHandle {
            pointer: std::ptr::null(),
        };
        assert_eq!(resolved_translate_axis_px(12.5, &empty, CssPixels::from_raw(640)), 12.5);
    }
}
