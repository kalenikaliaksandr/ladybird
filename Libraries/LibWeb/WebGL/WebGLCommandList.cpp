...
// All three encoders below share one payload layout: the payload struct at offset 0,
// then each non-empty inline blob at the next command_alignment boundary — the same
// rule first_inline_data_offset()/next_inline_data_offset() hand out to the callers
// that write the matching span fields.
...
// Writes the layout above into `destination`, zeroing the alignment gaps and the tail
// so they cannot leak the buffer's previous contents.
...
ByteBuffer WebGLSyncCall::encode_reply_bytes(ReadonlyBytes reply, ReadonlyBytes inline_data, ReadonlyBytes more_inline_data)
{
    ...
    auto bytes = MUST(ByteBuffer::create_uninitialized(payload_layout_size(reply, inline_data, more_inline_data)));
