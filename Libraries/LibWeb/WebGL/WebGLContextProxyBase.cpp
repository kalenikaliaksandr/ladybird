...
void WebGLContextProxyBase::read_buffer_sub_data(GLenum target, long long offset, Bytes destination)
{
    ...
    if (shared_data_or_error.is_error()) {
        set_pending_local_error(out_of_memory_error);
        return;
    }

    auto shared_data = shared_data_or_error.release_value();
    flush_commands();
    m_transport->read_buffer_sub_data(m_canvas_context_id, target, static_cast<GLintptr>(offset), static_cast<GLintptr>(destination.size()), shared_data);
    if (m_lost)
        return;
...
void WebGLContextProxy::read_pixels_robust_angle(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLsizei bufSize, GLsizei* length, GLsizei* columns, GLsizei* rows, void* pixels)
{
    ...
    if (rows)
        *rows = result.rows;
    if (pixels && bufSize > 0)
        __builtin_memcpy(pixels, shared_pixels.data<void>(), static_cast<size_t>(bufSize));
}
