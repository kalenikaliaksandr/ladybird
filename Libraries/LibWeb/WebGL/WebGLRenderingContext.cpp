...
// Connects to the compositor and creates a host WebGL context. The result is then either
// wrapped in a fresh proxy (context creation) or rebound onto an existing one (restore).
...
OwnPtr<WebGLContextProxy> create_webgl_context_proxy(HTML::HTMLCanvasElement& canvas_element, WebGLVersion webgl_version, WebGLContextAttributes const& context_attributes)
{
    ...
    return make<WebGLContextProxy>(move(remote->transport), remote->result.canvas_id, webgl_version, move(remote->result.supported_extensions));
}
...
bool restore_webgl_context_proxy(WebGLContextProxy& context, HTML::HTMLCanvasElement& canvas_element, WebGLVersion webgl_version, WebGLContextAttributes const& context_attributes)
{
    ...
    context.restore(move(remote->transport), remote->result.canvas_id, move(remote->result.supported_extensions));
    return true;
}
