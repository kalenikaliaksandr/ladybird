...
OwnPtr<WebGLContextProxy> create_webgl_context_proxy(HTML::HTMLCanvasElement& canvas_element, WebGLVersion webgl_version, WebGLContextAttributes const& context_attributes)
{
    ...
    auto canvas_context_id = WebGLContextProxyBase::allocate_canvas_context_id();
    ...
    return make<WebGLContextProxy>(transport.release_nonnull(), canvas_context_id, webgl_version, move(result.supported_extensions));
}
...
bool restore_webgl_context_proxy(WebGLContextProxy& context, HTML::HTMLCanvasElement& canvas_element, WebGLVersion webgl_version, WebGLContextAttributes const& context_attributes)
{
    ...
    context.restore(transport.release_nonnull(), move(result.supported_extensions));
    context.set_size(canvas_element.bitmap_size_for_canvas(1, 1));
    return true;
}

void prepare_remote_context_for_compositing(WebGLContextProxy& context, HTML::HTMLCanvasElement& canvas_element, bool preserve_drawing_buffer)
{
    // Without a compositor context (for example mid-teardown) there is no slot to
    // sample from; flush whatever is recorded so the host stays current.
    if (auto navigable = canvas_element.document().navigable(); navigable && navigable->has_compositor_context()) {
        context.prepare_canvas_surface_for_compositing(
            navigable->compositor_context().id(),
            canvas_element.ensure_canvas_id(),
...
{
    prepare_remote_context_for_compositing(context(), *m_canvas_element, m_context_creation_parameters.preserve_drawing_buffer);
}
