...
// Rebinds an existing (lost) proxy to a freshly created host context after a compositor
// restart, reusing the proxy's context id. Returns false if no compositor is available.
bool restore_webgl_context_proxy(WebGLContextProxy&, HTML::HTMLCanvasElement&, WebGLVersion, WebGLContextAttributes const&);

// Makes the drawing buffer available through the element's canvas surface slot.
void prepare_remote_context_for_compositing(WebGLContextProxy&, HTML::HTMLCanvasElement&, bool preserve_drawing_buffer);
