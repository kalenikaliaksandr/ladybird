...
{
    // While the context is lost, getError() returns CONTEXT_LOST_WEBGL the first time and
    // NO_ERROR thereafter (seeded in lose_context_from_compositor_loss()). The host is gone,
    // so a round trip would only fabricate NO_ERROR from an empty reply.
    if (m_context_lost) {
        auto error = m_error;
        m_error = GL_NO_ERROR;
        return error;
