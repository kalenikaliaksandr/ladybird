...
void Application::register_compositor_context(Web::Compositor::CompositorContextId context_id, Optional<u64> page_id, Web::Compositor::PagePresentationRegistration page_presentation_registration)
{
    if (!m_compositor_client)
        return;
    m_compositor_client->async_create_context(context_id, page_id, page_presentation_registration);
}
