...
void CompositorClient::did_request_cursor_change(Web::Compositor::CompositorContextId context_id, Gfx::Cursor cursor)
{
    auto web_content_client = WebContentClient::client_for_compositor_context_id(context_id);
    if (!web_content_client.has_value())
        return;
    ...
    web_content_client->did_request_cursor_change(*page_id, move(cursor));
}

void CompositorClient::did_lose_compositor()
{
    die();
}
