...
// Mach port handshake for child process IPC:
//
// The parent cannot pass Mach port rights to a child via fork/exec (unlike FDs).
// Instead, a two-phase handshake is used:
//
//  1. Parent creates two port pairs (A and B), keeps (A_recv, B_send) for
//     itself, and stores (B_recv, A_send) as "pending" for the child's PID.
//
//  2. Child starts up and contacts the parent's MachPortServer (via the
//     macOS bootstrap server). The MachPortServer calls
//     handle_child_mach_port_registration(), which sends the pending ports
//     to the child via a Mach message.
//
// Because steps 1 and 2 can race (child might register before or after the
// parent stores the pending ports), both sides check a shared map protected
// by s_pending_ipc_ports_mutex. Whichever side arrives second completes
// the handshake by calling send_ipc_ports_to_child().
#if defined(AK_OS_MACOS)
static Threading::Mutex s_pending_ipc_ports_mutex;
static HashMap<pid_t, Process::PendingIPCPorts> s_pending_ipc_ports;
static HashMap<pid_t, Core::MachPort> s_pending_replies;

void Process::register_pending_ipc_ports(pid_t pid, Core::MachPort receive_right, Core::MachPort send_right)
{
    Optional<Core::MachPort> reply_port;
    {
        Threading::MutexLocker locker(s_pending_ipc_ports_mutex);
        auto it = s_pending_replies.find(pid);
        if (it != s_pending_replies.end()) {
            reply_port = move(it->value);
            s_pending_replies.remove(it);
        } else {
            s_pending_ipc_ports.set(pid, PendingIPCPorts { move(receive_right), move(send_right) });
        }
    }
    if (reply_port.has_value())
        send_ipc_ports_to_child(*reply_port, move(receive_right), move(send_right));
}

void Process::handle_child_mach_port_registration(pid_t pid, Core::MachPort reply_port)
{
    Optional<PendingIPCPorts> ports;
    {
        Threading::MutexLocker locker(s_pending_ipc_ports_mutex);
        ...
        if (it != s_pending_ipc_ports.end()) {
            ...
            s_pending_replies.set(pid, move(reply_port));
        }
    }
    if (ports.has_value())
        send_ipc_ports_to_child(reply_port, move(ports->receive_right), move(ports->send_right));
}
