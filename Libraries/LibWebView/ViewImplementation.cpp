...
void ViewImplementation::enqueue_input_event(Web::InputEvent event)
{
    ...
    if (Application::web_content_options().enable_async_scrolling == EnableAsyncScrolling::Yes
        ...
        if (mouse_event->type == Web::MouseEvent::Type::MouseWheel) {
            ...
            Web::Compositor::AsyncScrollInput input { mouse_event->wheel_delta_precision, mouse_event->scroll_gesture_phase };
            if (client().send_async_scroll_to_compositor(m_client_state.page_index, position, delta_in_device_pixels, input))
