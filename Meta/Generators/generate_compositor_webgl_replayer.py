    ...
    for arg in function["args"]:
        ...
        if arg.get("string"):
            ...
            if arg["type"] == "GLsync":
                ...
                lines.append(
                    f"    GLuint {field} = command.{field} ? TRY(objects.lookup(command.{field})) : gl.{default_getter}();"
                )
            else:
    ...
    for arg in function["args"]:
        ...
        if override is not None:
            ...
            if "payload" in arg:
                ...
                lines.append(
                    f"    if ({field}_byte_size < 0 || static_cast<u64>({field}_byte_size) > max_webgl_sync_reply_size)"
                )
            ...
            expression = rewrite_size_expression(arg["payload"], function, "request")
            lines.append(f"    auto {field} = WebGLCommandList::resolve_typed_span<{element}>(payload, request.{field});")
            lines.append(f"    VERIFY(static_cast<i64>({field}.size() * sizeof({element})) == static_cast<i64>({expression}));")
            call_args.append(f"{field}.data()")
        else:
