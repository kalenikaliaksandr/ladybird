...
def emit_sync_method(out: TextIO, function: dict) -> None:
    ...
    for field in blobs:
        out.write(f"    request.{field} = {{ WebGLCommandList::first_inline_data_offset(sizeof(request)), static_cast<u32>({field}_bytes.size()) }};\n")
    blob_argument = f", {blobs[0]}_bytes" if blobs else ""
    out.write(f"    auto reply_bytes = send_sync_call(WebGLSyncCall::encode_request<{call}>(request{blob_argument}));\n")

    has_return = function["return"] != "void"
    failure = "        return {};\n" if has_return else "        return;\n"
    out.write(f"    auto reply_or_error = WebGLSyncCall::decode_reply<{call}::Reply>(reply_bytes);\n")
    out.write("    if (reply_or_error.is_error())\n")
    out.write(failure)
    if sync_reply_fields(function):
        out.write("    auto reply = reply_or_error.release_value();\n")
    ...
    for _, field_name, arg in sync_reply_fields(function):
        ...
        if "payload" in arg:
            out.write(f"""    if ({name}) {{
        if (auto resolved = WebGLCommandList::resolve_data_span(reply_bytes, reply.{field_name}); !resolved.is_error())
            __builtin_memcpy({name}, resolved.value().data(), min(resolved.value().size(), static_cast<size_t>({arg['payload']})));
    }}
""")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Generate the WebGL context proxy recorder", add_help=False)
