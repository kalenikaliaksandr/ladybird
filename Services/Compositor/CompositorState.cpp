...
void CompositorState::set_canvas_surface(Web::Compositor::CompositorContextId context_id, Web::Painting::CanvasId canvas_id, NonnullRefPtr<Gfx::PaintingSurface> surface)
{
    auto* context = context_if_present(context_id);
    VERIFY(context);
    context->set_canvas_surface(canvas_id, move(surface));
}
