...
ConnectionFromWebContent::ConnectionFromWebContent(NonnullOwnPtr<IPC::Transport> transport, NonnullRefPtr<CompositorState> compositor_state)
    : IPC::ConnectionFromClient<CompositorWebContentClientEndpoint, CompositorWebContentServerEndpoint>(*this, move(transport), 1)
...
void ConnectionFromWebContent::forward_mouse_event(u64 page_id, Web::MouseEvent const& event)
{
    async_mouse_event(page_id, event.clone_without_browser_data());
}

bool ConnectionFromWebContent::set_context_owner(Web::Compositor::CompositorContextId context_id)
{
    switch (m_compositor_state->set_web_content_client_for_context(context_id, *this)) {
    case CompositorState::SetWebContentClientResult::Accepted:
        return true;
    case CompositorState::SetWebContentClientResult::ContextDestroyed:
        return false;
    case CompositorState::SetWebContentClientResult::ConflictingOwner:
...
{
    if (!set_context_owner(context_id))
        return;
...
void ConnectionFromWebContent::did_request_cursor_change(Web::Compositor::CompositorContextId context_id, Gfx::Cursor cursor)
{
    if (!set_context_owner(context_id))
        return;
    m_compositor_state->did_request_cursor_change(context_id, cursor);
}
