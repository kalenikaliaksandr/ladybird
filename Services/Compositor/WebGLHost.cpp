...
ErrorOr<void> HostWebGLContext::execute_commands(ReadonlyBytes bytes, OnPresent const& on_present)
{
    ...
    return WebGLCommandList::for_each_command(bytes, [&]<typename Command>(Command const& command, ReadonlyBytes payload) -> ErrorOr<void> {
        if constexpr (IsSame<Command, Commands::Present>) {
            (void)payload;
            return on_present(command, TRY(snapshot_for_present(command.preserve_drawing_buffer)));
        } else if constexpr (IsSame<Command, Commands::SetDrawingBufferSize>) {
            (void)payload;
...
ErrorOr<NonnullRefPtr<Gfx::PaintingSurface>> HostWebGLContext::snapshot_for_present(bool preserve_drawing_buffer)
{
    ...
    m_publish_surface->copy_from(*drawing_surface);
...
{
    auto source_bytes = TRY(WebGLCommandList::resolve_data_span(payload, command.source));
    if (source_bytes.is_empty() || source_bytes[source_bytes.size() - 1] != 0)
...
{
    auto varyings_bytes = TRY(WebGLCommandList::resolve_data_span(payload, command.varyings));
...
{
    auto names_bytes = TRY(WebGLCommandList::resolve_data_span(payload, request.uniform_names));
