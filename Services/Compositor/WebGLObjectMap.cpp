...
ErrorOr<void> WebGLObjectMap::add(Web::WebGL::WebGLObjectId id, GLuint name)
{
    ...
    m_reverse.set(name, id);
    return {};
}

Web::WebGL::WebGLObjectId WebGLObjectMap::reverse_lookup(GLuint name) const
{
    if (name == 0)
        return 0;
    return m_reverse.get(name).value_or(0);
}
