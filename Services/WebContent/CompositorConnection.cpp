...
{
    // The compositor process owns this WebContent's painting and presentation: losing the connection is unrecoverable
    // because there is no longer anywhere to publish display lists. Crash WebContent so the UI process can replace it.
    VERIFY_NOT_REACHED();
}
...
void CompositorConnection::did_request_cursor_change(Web::Compositor::CompositorContextId context_id, Gfx::Cursor const& cursor)
{
