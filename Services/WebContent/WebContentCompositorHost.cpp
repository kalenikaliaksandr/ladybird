...
public:
    ...
    Web::Compositor::ScreenshotRequestId register_screenshot(Function<void()>&& callback)
    {
        auto request_id = Web::Compositor::ScreenshotRequestId { m_next_screenshot_request_id++ };
        m_screenshots.set(request_id, PendingScreenshot { move(callback) });
        return request_id;
    }
...
private:
    ...
    {
        complete_screenshot(request_id);
    }
    ...
    void complete_screenshot(Web::Compositor::ScreenshotRequestId request_id)
    {
        auto pending_screenshot = m_screenshots.take(request_id);
...
public:
    explicit ActorMainThreadClient(NonnullRefPtr<Core::WeakEventLoopReference> event_loop)
        : m_event_loop(move(event_loop))
    {
    }

    void set_actor(WeakPtr<WebContentCompositorActor> actor) { m_actor = move(actor); }
...
public:
    explicit ActorUIPresentationClient(NonnullRefPtr<Core::WeakEventLoopReference> event_loop, NonnullRefPtr<CompositorConnectionFromClient> connection)
        : m_event_loop(move(event_loop))
    ...
    {
        invoke_on_actor_thread([this, page_id, front_bitmap_id, front_backing_store = move(front_backing_store), back_bitmap_id, back_backing_store = move(back_backing_store)]() mutable {
    ...
    {
        invoke_on_actor_thread([this, page_id, viewport_rect, bitmap_id] {
...
public:
    WebContentCompositorActor(NonnullOwnPtr<IPC::Transport> transport, Web::DisplayListPlayerType display_list_player_type)
        ...
        , m_main_thread_client(adopt_ref(*new ActorMainThreadClient(Core::EventLoop::current_weak())))
        , m_compositor_thread(make<Web::Compositor::CompositorThread>(m_main_thread_client))
    {
        m_main_thread_client->set_actor(make_weak_ptr<WebContentCompositorActor>());
...
private:
    ...
    virtual void update_display_list(Web::Compositor::CompositorContextId context_id, NonnullRefPtr<Web::Painting::DisplayList> display_list, Web::Painting::DisplayListResourceTransactionTransport resource_transaction, Web::Painting::ScrollStateSnapshot scroll_state_snapshot) override
    {
    ...
    virtual void request_screenshot(Web::Compositor::CompositorContextId context_id, Web::Compositor::ScreenshotRequestId request_id, Gfx::SharedImage target) override
    {
        m_compositor_thread->request_screenshot(context_id, move(target), request_id);
    }
...
{
    invoke_on_actor_thread([](auto& actor) {
...
{
    invoke_on_actor_thread([request_id](auto& actor) {
...
public:
    void attach_ui_client(IPC::TransportHandle handle)
    {
        connection().async_attach_ui_client(move(handle));
    }

private:
    virtual void start_impl(Web::DisplayListPlayerType display_list_player_type) override
    {
        if (m_connection)
            return;
    ...
    virtual Web::Compositor::CompositorContextId create_context_impl(Optional<u64> page_id, Web::Compositor::PagePresentationRegistration page_presentation_registration) override
    {
        return connection().create_context(page_id, page_presentation_registration);
    }

    virtual void destroy_context_impl(Web::Compositor::CompositorContextId context_id) override
    {
        connection().async_destroy_context(context_id);
    }

    virtual void stop_presenting_to_client_impl(Web::Compositor::CompositorContextId context_id) override
    {
        connection().async_stop_presenting_to_client(context_id);
    }

    virtual void set_presentation_mode_impl(Web::Compositor::CompositorContextId context_id, Web::Compositor::PresentationMode mode) override
    {
        connection().async_set_presentation_mode(context_id, move(mode));
    }

    virtual void update_display_list_impl(Web::Compositor::CompositorContextId context_id, NonnullRefPtr<Web::Painting::DisplayList> display_list, Web::Painting::DisplayListResourceTransaction&& resource_transaction, Web::Painting::ScrollStateSnapshot&& scroll_state_snapshot) override
    {
        auto resource_transaction_transport = MUST(Web::Painting::create_display_list_resource_transaction_transport(move(resource_transaction)));
        auto encoded_message = MUST(Messages::WebContentCompositorServer::UpdateDisplayList::static_encode(context_id, move(display_list), move(resource_transaction_transport), move(scroll_state_snapshot)));
        MUST(connection().post_message(encoded_message));
    }

    virtual void update_video_frame_impl(Web::Compositor::CompositorContextId context_id, Web::Painting::VideoFrameResourceId frame_id, NonnullRefPtr<Media::VideoFrame const> frame) override
    {
    ...
    virtual void clear_video_frame_impl(Web::Compositor::CompositorContextId context_id, Web::Painting::VideoFrameResourceId frame_id) override
    {
        connection().async_clear_video_frame(context_id, frame_id);
    }

    virtual void update_compositor_surface_impl(Web::Compositor::CompositorContextId context_id, Web::Painting::CompositorSurfaceId surface_id, Gfx::SharedImage&& shared_image) override
    {
        connection().async_update_compositor_surface(context_id, surface_id, move(shared_image));
    }

    virtual void clear_compositor_surface_impl(Web::Compositor::CompositorContextId context_id, Web::Painting::CompositorSurfaceId surface_id) override
    {
        connection().async_clear_compositor_surface(context_id, surface_id);
    }

    virtual void update_scroll_state_impl(Web::Compositor::CompositorContextId context_id, Web::Painting::ScrollStateSnapshot&& scroll_state_snapshot) override
    {
        connection().async_update_scroll_state(context_id, move(scroll_state_snapshot));
    }

    virtual void invalidate_wheel_event_listener_state_impl(Web::Compositor::CompositorContextId context_id, u64 generation) override
    {
        connection().async_invalidate_wheel_event_listener_state(context_id, generation);
    }

    virtual Web::Compositor::AsyncScrollEnqueueResult async_scroll_by_impl(Web::Compositor::CompositorContextId context_id, Web::UniqueNodeID expected_document_id, Gfx::FloatPoint position,
    ...
    virtual bool should_defer_async_scroll_offset_adoption_impl(Web::Compositor::CompositorContextId context_id) const override
    {
    ...
    virtual bool should_defer_main_thread_present_for_async_scroll_impl(Web::Compositor::CompositorContextId context_id) const override
    {
    ...
    virtual Web::Compositor::PendingAsyncScrollUpdates take_pending_async_scroll_updates_impl(Web::Compositor::CompositorContextId context_id) override
    {
    ...
    virtual void viewport_size_updated_impl(Web::Compositor::CompositorContextId context_id, Gfx::IntSize viewport_size, bool is_top_level_traversable, Web::Compositor::WindowResizingInProgress window_resize_in_progress) override
    {
        connection().async_viewport_size_updated(context_id, viewport_size, is_top_level_traversable, window_resize_in_progress);
    }

    virtual void present_frame_impl(Web::Compositor::CompositorContextId context_id, Gfx::IntRect viewport_rect) override
    {
        connection().async_present_frame(context_id, viewport_rect);
    }

    virtual void request_screenshot_impl(Web::Compositor::CompositorContextId context_id, Gfx::SharedImage&& target, Function<void()>&& callback) override
    {
        auto& conn = connection();
        auto request_id = conn.register_screenshot(move(callback));
        conn.async_request_screenshot(context_id, request_id, move(target));
    }
...
void attach_ui_client_to_web_content_compositor_host(Web::Compositor::CompositorHost& compositor_host, IPC::TransportHandle handle)
{
    static_cast<WebContentCompositorHost&>(compositor_host).attach_ui_client(move(handle));
}
